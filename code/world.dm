...
/world/New()
	...
	if(!RoleAuthority)
		...
		to_chat(world, "<span style='color:#FF0000'>\b Job setup complete</span>")
	...
	// Process Scheduler
	to_chat(src, "<span style='color:#FF0000'>\b Scheduler initialized.</span>")
	processScheduler = new

	spawn(0)
		processScheduler.setup()

	to_chat(src, "<span style='color:#FF0000'>\b Scheduler setup complete.</span>")
...
//		world << sound(pick('sound/AI/newroundsexy.ogg','sound/misc/apcdestroyed.ogg','sound/misc/bangindonk.ogg')) // random end sounds!! - LastyBatsy
	...
	for(var/client/C in clients)
		if(config.server)	//if you set a server location in config.txt, it sends you there instead of trying to reconnect to the same world address. -- NeoFite
			C << link("byond://you_adress")//[config.server]")
			...
			for(var/client/C in clients)
				...
				if(C.is_afk(INACTIVITY_KICK))
					if(!istype(C.mob, /mob/dead))
						...
						to_chat(C, "<span style='color:#FF0000'>You have been inactive for more than 10 minutes and have been disconnected.</span>")
						cdel(C)
...
proc/send_map_info(next_map)
	var/http[] = world.Export("http://127.0.0.1:1400?" + next_map)
	if(!http)
