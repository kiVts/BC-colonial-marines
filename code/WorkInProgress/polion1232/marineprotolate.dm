...
/obj/machinery/r_n_d/marineprotolathe/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(istype(O, /obj/item/marineResearch/xenomorp))
		...
		if(max_per_resource["biomass"] <= material_storage["biomass"])
			to_chat(user, "<span style='color:#FF0000'>The protolathe's biomass bin is full.</span>")
			return 1
		user.drop_held_item()
	...
	if(istype(O, /obj/item/marineResearch/sampler))
		...
		if(!samp.filled)
			to_chat(user, "<span style='color:#FF0000'>Sampler is empty!</span>")
			return 1
		if(max_per_resource["biomass"] <= material_storage["biomass"])
			to_chat(user, "<span style='color:#FF0000'>The protolathe's biomass bin is full.</span>")
			return 1
	...
	if(istype(O,/obj/item/stack/sheet))
		...
		if (material_storage[S.name] + S.perunit > max_per_resource[S.name])
			to_chat(user, "<span style='color:#FF0000'>The protolathe's [S.name] material bin is full. Please remove material before adding more.</span>")
			return 1
