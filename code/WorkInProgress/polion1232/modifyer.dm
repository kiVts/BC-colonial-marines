...
/obj/machinery/r_n_d/modifyer/attackby(var/obj/O as obj, var/mob/user as mob)
	...
	if (opened)
		if(istype(O, /obj/item/tool/crowbar))
			...
			to_chat(user, "<span style='color:#FF0000'>You can't load the [src.name] while it's opened.</span>")
			return 1
	if (disabled)
		return
	if (!linked_console)
		to_chat(user, "<span style='color:#FF0000'>The EMU must be linked to an R&D console first!</span>")
		return
	if (busy)
		to_chat(user, "<span style='color:#FF0000'>EMU is busy right now.</span>")
		return
		...
		if(!istype(O, /obj/item/clothing/suit/storage/marine))
			if(!istype(O, /obj/item/clothing/head/helmet/marine))
				if(!istype(O, /obj/item/clothing/shoes/marine))
					to_chat(user, "<span style='color:#FF0000'>Can't do anything with that!</span>")
					return
		loaded_item = O
		user.drop_held_item()
		O.loc = src
		to_chat(user, "<span style='color:#0000FF'>You add the [O.name] to the machine!</span>")
		update_icon()
		return 1
	else
		to_chat(user, "<span style='color:#FF0000'>Can't do anything with that!</span>")
...
/obj/machinery/r_n_d/modifyerfuel
	name = "Fuel Creator Unit"
	icon = 'code/WorkInProgress/polion1232/rnd.dmi'
	icon_state = "mod_unit"
	var/obj/item/rocket_fuel/loaded_item = null // Because armor and helmet too different
	var/obj/item/rocket_fuel/py = null // In future, you will use alien pieces for modification
...
/obj/machinery/r_n_d/modifyer/update_icon()
	if(istype(loaded_item, /obj/item/rocket_fuel))
		icon_state = "mod_unit_fuel"
		return
	icon_state = "mod_unit"
...
/obj/machinery/r_n_d/modifyer/attackby(var/obj/O as obj, var/mob/user as mob)
	...
	if (istype(O, /obj/item) && !loaded_item)
		if(isrobot(user)) //Don't put your module items in there!
			return
		if(!istype(O, /obj/item/rocket_fuel))
			to_chat(user, "<span style='color:#FF0000'>Can't do anything with that!</span>")
			return
		loaded_item = O
		user.drop_held_item()
