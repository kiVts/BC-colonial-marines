...
/obj/machinery/computer/XenoRnD/Topic(href, href_list)				//Brutally teared from rdconsole.dm
	...
	if(href_list["menu"]) //Switches menu screens. Converts a sent text string into a number. Saves a LOT of code.
			...
			if("dissector")
				if(linked_dissector)
					if(linked_dissector.busy)
						to_chat(usr, "<span style='color:#FF0000'>The organic dissector is busy at the moment.</span>")

					else if(linked_dissector.loaded_item)
						if(istype(linked_dissector, /obj/item/marineResearch/xenomorp/weed))
							to_chat(usr, "<span style='color:#FF0000'>You cannot eject that.</span>")
							screen = 2.2
							return
			...
			if("modifyer")
				if(linked_modifyer)
					if(linked_modifyer.busy)
						to_chat(usr, "<span style='color:#FF0000'>The EMU is busy at the moment.</span>")
				...
				spawn(24)
					if(linked_dissector)
						...
						if(!linked_dissector.loaded_item)
							to_chat(usr, "<span style='color:#FF0000'>The organic dissector appears to be empty.</span>")
							screen = 1.0
							return
						linked_dissector.icon_state = "d_analyzer"
		...
		if(linked_modifyer)
			if(linked_modifyer.busy)
				to_chat(usr, "<span style='color:#FF0000'>EMU is busy at the moment.</span>")
			else if(linked_modifyer.loaded_item.is_modifyed)
				to_chat(usr, "<span style='color:#FF0000'>This equipment is already modifyed.</span>")
			else
