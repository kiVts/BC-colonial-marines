			...
			if(istype(P, /obj/item/tool/crowbar))
				if(battery)
					...
					if(do_after(user, 10, TRUE, 5, BUSY_ICON_BUILD))
						battery.loc = loc
						to_chat(user, "<span style='color:#0000FF'>You remove [battery].</span>")
						battery = null
				else
					to_chat(user, "<span style='color:#FF0000'>There's no battery to remove!</span>")
			...
			if(istype(P, /obj/item/cell))
				if(!battery)
					...
					if(do_after(user, 5, TRUE, 5, BUSY_ICON_BUILD))
						battery = P
						P.loc = src
						to_chat(user, "<span style='color:#0000FF'>You insert [battery].</span>")
				else
					to_chat(user, "<span style='color:#FF0000'>There's already \an [battery] in [src]!</span>")
				...
				if(P:amount >= 5)
					...
					if(do_after(user, 20, TRUE, 5, BUSY_ICON_BUILD))
						if(P)
							P:amount -= 5
							if(!P:amount) cdel(P)
							to_chat(user, "<span style='color:#0000FF'>You add cables to the frame.</span>")
							src.state = 3
							src.icon_state = "3"
		if(3)
			...
			if(istype(P, /obj/item/stack/sheet/glass))
				if(P:amount >= 2)
					...
					if(do_after(user, 20, TRUE, 5, BUSY_ICON_BUILD))
						if(P)
							P:use(2)
							to_chat(user, "<span style='color:#0000FF'>You put in the glass panel.</span>")
							src.state = 4
							src.icon_state = "4"
		if(4)
			if(istype(P, /obj/item/tool/crowbar))
...
/obj/structure/computer3frame/proc/remove_peripheral(var/obj/item/I = null)
	if(!components || !components.len)
		to_chat(usr, "<span style='color:#FF0000'>There are no components in [src] to take out!</span>")
		return 0
	if(!I)
	...
	if(I)
		...
		if(do_after(usr, 25, TRUE, 5, BUSY_ICON_BUILD))
			if(I==hdd)
				...
				I.loc = loc
			to_chat(usr, "<span style='color:#0000FF'>You remove [I]</span>")
			return 1
	return 0
