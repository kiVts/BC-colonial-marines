	...
	proc/interactable(var/mob/user)
		...
		if(istype(user,/mob/living/silicon))
			if(!program.ai_allowed)
				to_chat(user, "<span style='color:#0000FF'>You are forbidden from accessing this program.</span>")
				return 0
		else
			if(program.human_controls)
				if(!ishuman(user))
					to_chat(user, "<span style='color:#FF0000'>Your body can't work the controls!</span>")
					return 0
				if(user.is_mob_restrained())
					to_chat(user, "<span style='color:#FF0000'>You need a free hand!</span>")
					return 0
			...
			if(!in_range(src,user))
				...
				if(ishuman(user) && istype(user.get_active_hand(),/obj/item/tk_grab))
					if(program.human_controls)
						to_chat(user, "<span style='color:#FF0000'>It's too complicated to work at a distance!</span>")
						return 0
	...
	*/
	proc/Crash(var/errorcode = PROG_CRASH)
		...
		switch(errorcode)
			if(PROG_CRASH)
				if(usr)
					to_chat(usr, "<span style='color:#FF0000'>The program crashed!</span>")
