...
/obj/machinery/lapvend/proc/scan_card(var/obj/item/card/I)
	if (istype(I, /obj/item/card/id))
		...
		if(!CH || isnull(CH) || !istype(CH))
			to_chat(usr, "<span style='color:#FF0000'>This card has no account data!</span>")
			return
...
/obj/machinery/lapvend/proc/choose_progs(var/obj/item/card/id/C)
	...
	if(ACCESS_MARINE_ENGINEERING in C.access)
	...
	if(ACCESS_MARINE_RESEARCH in C.access)
	...
	if(ACCESS_WY_CORPORATE in C.access)
