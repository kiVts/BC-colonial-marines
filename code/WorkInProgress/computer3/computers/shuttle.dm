			...
			switch(choice)
				if("Authorize")
					...
					if (src.auth_need - src.authorized.len > 0)
						...
						to_chat(world, text("<span style='color:#0000FF'><B>Alert: [] authorizations needed until shuttle is launched early</B></span>", src.auth_need - src.authorized.len))
					else
						message_admins("[key_name_admin(user)] has launched the shuttle")
						log_game("[user.ckey] has launched the shuttle early")
						to_chat(world, "<span style='color:#0000FF'><B>Alert: Shuttle launch time shortened to 10 seconds!</B></span>")
				...
				if("Abort")
					to_chat(world, "<span style='color:#0000FF'><B>All authorizations to shorting time for shuttle launch have been revoked!</B></span>")
