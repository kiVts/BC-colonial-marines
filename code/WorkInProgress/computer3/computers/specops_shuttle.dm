...
/proc/specops_process()
	...
	if (!specops_can_move())
		to_chat(usr, "<span style='color:#FF0000'>The Special Operations shuttle is unable to leave.</span>")
		return
	...
	for(var/turf/T in get_area_turfs(end_location) )
		...
		to_chat(M, "<span style='color:#FF0000'>You have arrived to [station_name]. Commence operation!</span>")
...
/obj/machinery/computer3/specops_shuttle/attackby(I as obj, user as mob)
	if(istype(I,/obj/item/card/emag))
		to_chat(user, "<span style='color:#0000FF'>The electronic systems in this console are far too advanced for your primitive hacking peripherals.</span>")
	else
		return attack_hand(user)
...
/obj/machinery/computer3/specops_shuttle/attack_hand(var/mob/user as mob)
	...
	if (sent_strike_team == 0)
		to_chat(usr, "<span style='color:#FF0000'>The strike team has not yet deployed.</span>")
		return
...
/obj/machinery/computer3/specops_shuttle/Topic(href, href_list)
	...
	if (href_list["sendtodock"])
		...
		to_chat(usr, "<span style='color:#0000FF'>Central Command will not allow the Special Operations shuttle to return.</span>")
		return
		...
		to_chat(usr, "<span style='color:#0000FF'>The Special Operations shuttle will arrive on [station_name] in [(SPECOPS_MOVETIME/10)] seconds.</span>")
