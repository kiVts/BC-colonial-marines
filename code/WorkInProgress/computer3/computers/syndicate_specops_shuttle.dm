...
/proc/syndicate_elite_process()
	...
	if (!syndicate_elite_can_move())
		to_chat(usr, "<span style='color:#FF0000'>The Syndicate Elite shuttle is unable to leave.</span>")
		return
...
/obj/machinery/computer3/syndicate_elite_shuttle/Topic(href, href_list)
	...
	if (href_list["sendtodock"])
		...
		to_chat(usr, "<span style='color:#0000FF'>The Syndicate will not allow the Elite Squad shuttle to return.</span>")
		return
		...
		to_chat(usr, "<span style='color:#0000FF'>The Syndicate Elite shuttle will arrive on [station_name] in [(SYNDICATE_ELITE_MOVETIME/10)] seconds.</span>")
