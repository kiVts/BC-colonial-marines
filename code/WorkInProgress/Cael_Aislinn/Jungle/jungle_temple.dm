		...
		25;"viscerator", \
			...
			if("plants")
				if(prob(25))
					...
					new /obj/item/reagent_container/food/snacks/grown/mushroom/libertycap(src.loc)
				else if(prob(50))
					new /obj/item/reagent_container/food/snacks/grown/ambrosiavulgaris(src.loc)
			if("clothes")
			...
			if("lights")
				...
				var/num = rand(2,6)
				for(var/i=0,i<num,i++)
					var/spawn_type = pick(/obj/item/device/flashlight/flare, /obj/item/trash/candle, /obj/item/tool/candle/, /obj/item/storage/box/matches)
					new spawn_type(C)
			if("engineering")
				var/obj/structure/closet/crate/secure/gear/C = new(src.loc)
...
/obj/effect/step_trigger/trap/Trigger(var/atom/A)
	...
	switch(trap_type)
		if("sawburst")
			to_chat(M, "<span style='color:#FF0000'><b>A sawblade shoots out of the ground and strikes you!</b></span>")
			...
			//flick("sawblade",src)
		if("poison_dart")
			to_chat(M, "<span style='color:#FF0000'><b>You feel something small and sharp strike you!</b></span>")
		...
		if("flame_burst")
			to_chat(M, "<span style='color:#FF0000'><b>A jet of fire comes out of nowhere!</b></span>")
		...
		if("thrower")
			...
			M.visible_message("<span style='color:#FF0000'><b>The floor under [M] suddenly tips upward!</b>","\red <b>The floor tips upward under you!</b></span>")
