...
/obj/structure/bush/attackby(var/obj/I as obj, var/mob/user as mob)
	...
	if(I && (istype(I, /obj/item/tool/hatchet) || istype(I, /obj/item/weapon/combat_knife) || istype(I, /obj/item/weapon/claymore/mercsword) && !stump))
		...
		if(indestructable)
			...
			to_chat(user, "<span style='color:#FF0000'>You flail away at the undergrowth, but it's too thick here.</span>")
		else
			user.visible_message("<span style='color:#FF0000'>[user] flails away at the  [src] with [I].","\red You flail away at the [src] with [I].</span>")
			playsound(src.loc, 'sound/effects/vegetation_hit.ogg', 25, 1)
			health -= damage
			if(health < 0)
				to_chat(user, "<span style='color:#0000FF'>You clear away [src].</span>")
			healthcheck()
	else
		return ..()
...
/obj/structure/jungle_plant/attack_hand(var/mob/user as mob)
	if(fruits_left > 0)
		fruits_left--
		to_chat(user, "<span style='color:#0000FF'>You pick a fruit off [src].</span>")
		...
		to_chat(user, "<span style='color:#FF0000'>There are no fruit left on [src].</span>")
