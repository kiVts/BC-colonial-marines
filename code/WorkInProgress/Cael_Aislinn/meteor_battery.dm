		...
		if(get_turf(M) == M.target && M)
			...
			if( istype(get_turf(M), /turf/space) )
				...
				for(var/mob/P in view(7))
					P.visible_message("<span style='color:#FF0000'>The missile skids to a halt, vibrating and sparking ominously!</span>")
	...
	if(!(stat & BROKEN))
		...
		for(var/mob/O in viewers(src, null))
			if ((O.client && !( O.blinded )))
				O.show_message(text("<span style='color:#FF0000'><B>[] has slashed at []!</B></span>", M, src), 1)
		src.health -= 15
		if (src.health <= 0)
			src.die()
	else
