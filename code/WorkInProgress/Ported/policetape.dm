...
/obj/item/tool/taperoll/attack_self(mob/user as mob)
	if(icon_state == "[icon_base]_start")
		start = get_turf(src)
		to_chat(usr, "<span style='color:#0000FF'>You place the first end of the [src].</span>")
		icon_state = "[icon_base]_stop"
	else
		icon_state = "[icon_base]_start"
		end = get_turf(src)
		if(start.y != end.y && start.x != end.x || start.z != end.z)
			to_chat(usr, "<span style='color:#0000FF'>[src] can only be laid horizontally or vertically.</span>")
			return
		...
		if (!can_place)
			to_chat(usr, "<span style='color:#0000FF'>You can't run \the [src] through that!</span>")
			return
	...
	//is_blocked_turf(var/turf/T)
		to_chat(usr, "<span style='color:#0000FF'>You finish placing the [src].</span>"	)
...
/obj/item/tool/taperoll/afterattack(var/atom/A, mob/user as mob, proximity)
	if (proximity && istype(A, /obj/machinery/door/airlock))
		...
		to_chat(user, "<span style='color:#0000FF'>You finish placing the [src].</span>")
...
/obj/item/tape/proc/breaktape(obj/item/W as obj, mob/user as mob)
	if(user.a_intent == "help" && ((!can_puncture(W) && src.allowed(user))))
		...
		return
	user.show_viewers("<span style='color:#0000FF'>[user] breaks the [src]!</span>")
