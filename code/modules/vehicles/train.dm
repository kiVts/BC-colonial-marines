...
/obj/vehicle/train/proc/attach_to(obj/vehicle/train/T, mob/user)
	if (get_dist(src, T) > 1)
		to_chat(user, "<span style='color:#FF0000'>[src] is too far away from [T] to hitch them together.</span>")
		return

	if (lead)
		to_chat(user, "<span style='color:#FF0000'>[src] is already hitched to something.</span>")
		return

	if (T.tow)
		to_chat(user, "<span style='color:#FF0000'>[T] is already towing something.</span>")
		return
	...
	while (next_car)
		if (next_car == src)
			to_chat(user, "<span style='color:#FF0000'>That seems very silly.</span>")
			return
	...
	if(user)
		to_chat(user, "<span style='color:#0000FF'>You hitch [src] to [T].</span>")
...
/obj/vehicle/train/proc/unattach(mob/user)
	if (!lead)
		to_chat(user, "<span style='color:#FF0000'>[src] is not hitched to anything.</span>")
		return

	lead.tow = null
	lead.update_stats()

	to_chat(user, "<span style='color:#0000FF'>You unhitch [src] from [lead].</span>")
	lead = null
