...
/obj/structure/vehicle_interior/side_door/proc/activate(mob/user)
	...
	var/remove_vehicle_crewman_entered = FALSE
	var/passengers_left = 0
	if(ishuman(user))
		var/mob/living/carbon/human/H = user
		var/obj/item/card/id/I = H.wear_id
		if(I && I.rank == "Vehicle Crewman" && master.vehicle_crewman_entered)
			remove_vehicle_crewman_entered = TRUE
		else
	...
	if(remove_vehicle_crewman_entered)
		master.vehicle_crewman_entered = FALSE
