...
/obj/vehicle/multitile/root/cm_transport/apc
	...
	var/vehicle_crewman_entered = FALSE
...
/obj/effect/multitile_spawner/cm_transport/apc/New()
	...
	R.camera.c_tag = "Armored Personnel Carrier пїЅ[rand(1,10)]" //ARMORED to be at the start of cams list, numbers in case of events with multiple vehicles
...
/obj/vehicle/multitile/root/cm_transport/apc/proc/use_megaphone(mob/living/user)
	...
	if (user.client)
		if(user.client.prefs.muted & MUTE_IC)
			to_chat(user, "<span style='color:#FF0000'>You cannot speak in IC (muted).</span>")
			return
	if(user.silent)
		return

	if(spamcheck)
		to_chat(user, "<span style='color:#FF0000'>\The megaphone needs to recharge!</span>")
		return
...
/obj/vehicle/multitile/root/cm_transport/apc/handle_interior_entrance(var/mob/living/carbon/M)
	...
	var/new_vehicle_crewman_entered = FALSE
	var/new_passengers = 0
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
		var/obj/item/card/id/I = H.wear_id
		if(I && I.rank == "Vehicle Crewman" && !vehicle_crewman_entered)
			new_vehicle_crewman_entered = TRUE
		else
			if(I && I.rank == module_role && !module_role_entered)
				...
				if(passengers >= passengers_max)
					...
					new_passengers++
					to_chat(M, "<span class='notice'>Вы начинаете входить в APC.</span>")
//					to_chat(M, "<span class='debuginfo'>new passengers = [new_passengers].</span>")//don't commit
	else
		if(passengers >= passengers_max)
			to_chat(M, "<span class='warning'>[src] is full.</span>")
			return
...
//			to_chat(M, "<span class='danger'>Check for user done.</span>")
	...
	if(M.pulling && get_dist(entrance, M.pulling) <= 1)
		...
		if(isliving(M.pulling))
			...
			if(ishuman(M.pulling))
				...
				if(I && I.rank == "Vehicle Crewman" && !vehicle_crewman_entered)
					new_vehicle_crewman_entered = TRUE
					to_chat(M, "<span class='debuginfo'>pulling vehicle crewman.</span>")
				else
					if(I && I.rank == module_role && !module_role_entered)
						new_module_role_entered = TRUE
	...
	if(new_vehicle_crewman_entered)
		vehicle_crewman_entered = TRUE
...
/obj/vehicle/multitile/root/cm_transport/apc/proc/choose_module(var/mob/user)
	...
	if(ishuman(user))
		...
		var/obj/item/card/id/I = H.wear_id
		if(!istype(I) || I.registered_name != H.real_name || I.rank != "Vehicle Crewman")
			to_chat(H, "<span class='warning'>[src] door is locked. You need a Vehicle Crewman to unlock it.</span>")
