...
/obj/machinery/mining/drill/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/tool/screwdriver))
		if(active) return
		open = !open
		to_chat(user, "<span style='color:#0000FF'>You [open ? "open" : "close"] the maintenance panel.</span>")
		return
	else
...
/obj/machinery/mining/drill/attack_hand(mob/user as mob)
	...
	if(need_player_check)
		...
		if(use_cell_power())
			active = !active
			if(active)
				to_chat(user, "<span style='color:#0000FF'>You engage \the [src] and it lurches downwards, grinding noisily.</span>")
				need_update_field = 1
			else
				to_chat(user, "<span style='color:#0000FF'>You disengage \the [src] and it shudders to a grinding halt.</span>")
		else
			to_chat(user, "<span style='color:#0000FF'>The drill is unpowered.</span>")
	else
		to_chat(user, "<span style='color:#0000FF'>Turning on a piece of industrial machinery without sufficient bracing is a bad idea.</span>")
...
/obj/machinery/mining/drill/proc/system_error(var/error)
	...
	if(error) src.visible_message("<span style='color:#FF0000'>\The [src] flashes a '[error]' warning.</span>")
	need_player_check = 1
	active = 0
	update_icon()
...
/obj/machinery/mining/drill/verb/unload()
	...
	if(B)
		for(var/obj/item/ore/O in contents)
			O.loc = B
		to_chat(usr, "<span style='color:#FF0000'>You unload the drill's storage cache into the ore box.</span>")
	else
		to_chat(usr, "<span style='color:#FF0000'>You must move an ore box up to the drill before you can unload it.</span>")
*/
...
/obj/machinery/mining/brace/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/tool/wrench))
		...
		if(istype(get_turf(src),/turf/space))
			to_chat(user, "<span style='color:#0000FF'>You can't anchor something to empty space. Idiot.</span>")
			return

		if(connected && connected.active)
			to_chat(user, "<span style='color:#0000FF'>You can't unanchor the brace of a running drill!</span>")
			return

		playsound(src.loc, 'sound/items/Ratchet.ogg', 25, 1)
		to_chat(user, "<span style='color:#0000FF'>You [anchored ? "un" : ""]anchor the brace.</span>")
...
/obj/machinery/mining/brace/proc/connect()
	...
	if(!T.has_resources)
		src.visible_message("<span style='color:#FF0000'>The terrain near the brace is unsuitable!</span>")
		return
