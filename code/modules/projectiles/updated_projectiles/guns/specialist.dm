...
/obj/item/weapon/gun/rifle/sniper/M42A/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 6225)
	else
...
/obj/item/weapon/gun/rifle/sniper/elite/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 1325)
	else
		serial_number = given_serial
	desc += " Серийный номер [serial_number]"
	..(loc)
...
/obj/item/weapon/gun/rifle/sniper/svd/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
...
/obj/item/weapon/gun/rifle/m4ra/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 4225)
	else
		serial_number = given_serial
	desc += " Серийный номер [serial_number]"
	..(loc)
...
/obj/item/weapon/gun/smartgun/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 4235)
	else
		serial_number = given_serial
	desc += " Серийный номер [serial_number]"
	..(loc)
...
/obj/item/weapon/gun/smartgun/dirty/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 9225)
	else
		serial_number = given_serial
	desc += " Серийный номер [serial_number]"
	..(loc)
...
/obj/item/weapon/gun/launcher/m92/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 49225)
	else
		serial_number = given_serial
	desc += " Серийный номер [serial_number]"
	..(loc)
...
/obj/item/weapon/gun/launcher/m92/examine(mob/user)
	...
	if(grenades.len)
		...
		to_chat(user, "<span style='color:#0000FF'>It is loaded with <b>[grenades.len] / [max_grenades]</b> grenades.</span>")
...
/obj/item/weapon/gun/launcher/m81/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 79225)
	else
		serial_number = given_serial
...
/obj/item/weapon/gun/launcher/m81/examine(mob/user)
	...
	if(grenade)
		if (get_dist(user, src) > 2 && user != loc) return
		to_chat(user, "<span style='color:#0000FF'>It is loaded with a grenade.</span>")
...
/obj/item/weapon/gun/launcher/m81/riot/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 79225)
	else
		serial_number = given_serial
...
/obj/item/weapon/gun/launcher/rocket/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 79225)
	else
		serial_number = given_serial
...
/obj/item/weapon/gun/launcher/rocket/able_to_fire(mob/living/user)
	...
	if (. && istype(user)) //Let's check all that other stuff first.
		...
		if (current_turf.z == 3 || current_turf.z == 4) //Can't fire on the Almayer, bub.
			...
			return 0*/
		if(user.mind && user.mind.cm_skills && (flags_gun_features & GUN_SPECIALIST) && user.mind.cm_skills.spec_weapons < SKILL_SPEC_TRAINED && user.mind.cm_skills.spec_weapons != SKILL_SPEC_ROCKET)
			to_chat(user, "<span class='warning'>You don't seem to know how to use [src]...</span>")
			return 0
...
/obj/item/weapon/gun/launcher/rocket/m57a4/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 79225)
	else
		serial_number = given_serial
