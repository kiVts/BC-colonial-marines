...
/obj/item/weapon/gun/flare/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
...
/obj/item/weapon/gun/minigun/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 120)
	else
		serial_number = given_serial
	desc += " Серийный номер [serial_number]"
	..(loc)
...
/obj/item/weapon/gun/syringe/examine(mob/user)
	..()
	if(user != loc) return
	to_chat(user, "<span style='color:#0000FF'>[syringes.len] / [max_syringes] syringes.</span>")
	...
	if(istype(I, /obj/item/reagent_container/syringe))
		...
		if(S.mode != 2)//SYRINGE_BROKEN in syringes.dm
			if(syringes.len < max_syringes)
				...
				syringes += I
				update_icon()
				to_chat(user, "<span style='color:#0000FF'>You put the syringe in [src].</span>")
				to_chat(user, "<span style='color:#0000FF'>[syringes.len] / [max_syringes] syringes.</span>")
			else
				to_chat(usr, "<span style='color:#FF0000'>[src] cannot hold more syringes.</span>")
		else
			to_chat(usr, "<span style='color:#FF0000'>This syringe is broken!</span>")
...
/obj/item/weapon/gun/syringe/Fire(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, params, reflex = 0)
	if(syringes.len)
		...
		to_chat(usr, "<span style='color:#FF0000'>[src] is empty.</span>")
