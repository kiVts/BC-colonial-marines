...
/obj/item/attachable/scope/iftsscope
	...
	icon_state = "iftsscope"
	attach_icon = "iftsscope_a"
	desc = "A small rail iff mounted zoom sight scope. Allows zoom by activating the attachment. Use F12 if your HUD doesn't come back."
	slot = "rail"
	wield_delay_mod = WIELD_DELAY_NORMAL
	zoom_offset = 6
	zoom_viewsize = 8

/obj/item/attachable/scope/iftsscope/New()
	..()
...
/obj/item/attachable/stock/mou53
	name = "M-OU53 Tactical Stock"
	desc = "A sturdy polymer stock for the MOU53 shotgun. Supplied in limited numbers and moderately encumbering, it provides an ergonomic surface to ease perceived recoil and usability."
	icon_state = "m_stock"
...
/obj/item/attachable/stock/l_stock
	...
	size_mod = 1
	icon_state = "l_stock"
	attach_icon = "l_stock_a"
	pixel_shift_x = 41
	pixel_shift_y = 10

/obj/item/attachable/stock/l_stock/New()
	..()
	accuracy_mod = config.min_hit_accuracy_mult
	recoil_mod = -config.med_recoil_value
	scatter_mod = -config.med_scatter_value
...
/obj/item/attachable/fverticalgrip/attackby(obj/item/I, mob/user)
	if(istype(I,/obj/item/tool/screwdriver))
		to_chat(user, "<span class='notice'>You modify the rail flashlight back into a normal flashlight.</span>")
		if(istype(loc, /obj/item/storage))
			var/obj/item/storage/S = loc
...
/obj/item/attachable/fverticalgrip/New()
	..()
	accuracy_mod = config.min_hit_accuracy_mult
...
/obj/item/attachable/fverticalgrip
	name = "Flashlight Grip"
	desc = "A simple Flashlight for Grip used for mounting on a firearm. \nHas no drawbacks, but isn't particuraly useful outside of providing a light source."
	icon_state = "fverticalgrip"
	attach_icon = "fverticalgrip_a"
	light_mod = 7
	slot = "under"
	pixel_shift_x = 20
	matter = list("metal" = 150,"glass" = 40)
	flags_attach_features = ATTACH_REMOVABLE|ATTACH_ACTIVATION
	attachment_action_type = /datum/action/item_action/toggle

/obj/item/attachable/fverticalgrip/activate_attachment(obj/item/weapon/gun/G, mob/living/user, turn_off)
	if(turn_off && !(G.flags_gun_features & GUN_FLASHLIGHT_ON))
		return
	var/flashlight_on = (G.flags_gun_features & GUN_FLASHLIGHT_ON) ? -1 : 1
	var/atom/movable/light_source =  ismob(G.loc) ? G.loc : G
	...
	if(G.flags_gun_features & GUN_FLASHLIGHT_ON)
		icon_state = "fverticalgrip-on"
		attach_icon = "fverticalgrip_a-on"
	else
		icon_state = "fverticalgrip"
		attach_icon = "fverticalgrip_a"
