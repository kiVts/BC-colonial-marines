...
/obj/item/weapon/gun/verb/lower_aim()
	...
	if(target)
		stop_aim()
		usr.visible_message("<span style='color:#0000FF'>\The [usr] lowers \the [src]...</span>")
...
/obj/item/weapon/gun/proc/Aim(var/mob/living/M)
	if(!target || !(M in target))
		...
		if(target && !automatic) //If they're targeting someone and they have a non automatic weapon.
			...
			cdel(target)
			target = null
			usr.visible_message("<span style='color:#FF0000'><b>[usr] turns \the [src] on [M]!</b></span>")
		else
			usr.visible_message("<span style='color:#FF0000'><b>[usr] aims \a [src] at [M]!</b></span>")
		M.Targeted(src)
...
//			to_chat(src, "<span style='color:#FF0000'>Your move intent is now set to walk, as your targeter permits it.</span>")
		...
		if(G.target)
			for(var/mob/living/carbon/human/M in G.target)
				if(target_can_move)
					...
					if(!target_can_run)
						to_chat(M, "<span style='color:#FF0000'>Your move intent is now set to walk, as your targeter permits it.</span>")
