...
/obj/item/device/analyzer/plant_analyzer/afterattack(obj/target, mob/user, flag)
	...
	if(!grown_seed)
		to_chat(user, "<span style='color:#FF0000'>[src] can tell you nothing about [target].</span>")
		return

	var/dat = "<h3>Plant data for [target]</h3>"
	user.visible_message("<span style='color:#0000FF'>[user] runs the scanner over [target].</span>")
