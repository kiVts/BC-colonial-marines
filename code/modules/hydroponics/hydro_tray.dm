...
/obj/machinery/portable_atmospherics/hydroponics/proc/weed_invasion()
	...
	sampled = 0
	update_icon()
	visible_message("<span style='color:#0000FF'>[src] has been overtaken by [seed.display_name].</span>")
...
/obj/machinery/portable_atmospherics/hydroponics/proc/mutate_species()
	...
	update_icon()
	visible_message("<span style='color:#FF0000'>The \blue [previous_plant] \red has suddenly mutated into \blue [seed.display_name]!</span>")
...
/obj/machinery/portable_atmospherics/hydroponics/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(istype(O, /obj/item/tool/wirecutters) || istype(O, /obj/item/tool/surgery/scalpel))
		...
		if(!seed)
			...
			to_chat(user, "<span style='color:#FF0000'>\The [src] already has seeds in it!</span>")

	else if (istype(O, /obj/item/tool/minihoe))  // The minihoe

		if(weedlevel > 0)
			user.visible_message("<span style='color:#FF0000'>[user] starts uprooting the weeds.", "\red You remove the weeds from the [src].</span>")
			weedlevel = 0
			update_icon()
		else
			to_chat(user, "<span style='color:#FF0000'>This plot is completely devoid of weeds. It doesn't need uprooting.</span>")
		...
		if(seed)
			to_chat(user, "<span style='color:#FF0000'>[src] is already occupied!</span>")
		else
			user.drop_held_item()
			cdel(O)
