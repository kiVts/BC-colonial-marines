...
/datum/seed/proc/mutate(var/degree,var/turf/source_turf)
	...
	source_turf.visible_message("<span style='color:#0000FF'>\The [display_name] quivers!</span>")
	...
	for(var/i = 0;i<total_mutations;i++)
		switch(rand(0,11))
			if(0) //Plant cancer!
				...
				source_turf.visible_message("<span style='color:#FF0000'>\The [display_name] withers rapidly!</span>")
			if(1)
			...
			if(5)
				...
				if(prob(degree*5))
					...
					if(carnivorous)
						source_turf.visible_message("<span style='color:#0000FF'>\The [display_name] shudders hungrily.</span>")
			if(6)
				weed_tolerance  =           max(0,  min(10,  weed_tolerance       + (rand(-2,2)   * degree)))
			...
			if(8)
				...
				if(prob(degree*5))
					...
					source_turf.visible_message("<span style='color:#0000FF'>\The [display_name] spasms visibly, shifting in the tray.</span>")
			if(9)
			...
			if(10)
				if(prob(degree*2))
					biolum = !biolum
					if(biolum)
						source_turf.visible_message("<span style='color:#0000FF'>\The [display_name] begins to glow!</span>")
						if(prob(degree*2))
							biolum_colour = "#[pick(list("FF0000","FF7F00","FFFF00","00FF00","0000FF","4B0082","8F00FF"))]"
							source_turf.visible_message("<span style='color:#0000FF'>\The [display_name]'s glow <font color='[biolum_colour]'>changes colour</font>!</span>")
					else
						source_turf.visible_message("<span style='color:#0000FF'>\The [display_name]'s glow dims...</span>")
			if(11)
				if(prob(degree*2))
					flowers = !flowers
					if(flowers)
						source_turf.visible_message("<span style='color:#0000FF'>\The [display_name] sprouts a bevy of flowers!</span>")
						if(prob(degree*2))
							flower_colour = "#[pick(list("FF0000","FF7F00","FFFF00","00FF00","0000FF","4B0082","8F00FF"))]"
						source_turf.visible_message("<span style='color:#0000FF'>\The [display_name]'s flowers <font=[flower_colour]>changes colour</font>!</span>")
					else
						source_turf.visible_message("<span style='color:#0000FF'>\The [display_name]'s flowers wither and fall off.</span>")
	return
...
/datum/seed/proc/harvest(var/mob/user,var/yield_mod,var/harvest_sample)
	...
	if(!got_product && !harvest_sample)
		to_chat(user, "<span style='color:#FF0000'>You fail to harvest anything useful.</span>")
	else
		...
		for(var/i = 0;i<total_yield;i++)
			...
			if(istype(product,/mob/living))
				product.visible_message("<span style='color:#0000FF'>The pod disgorges [product]!</span>")
