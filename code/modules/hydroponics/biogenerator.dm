...
/obj/machinery/biogenerator/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(istype(O, /obj/item/reagent_container/glass))
		if(beaker)
			to_chat(user, "<span style='color:#FF0000'>The biogenerator is already loaded.</span>")
		else
			user.drop_inv_item_to_loc(O, src)
			beaker = O
			updateUsrDialog()
	else if(processing)
		to_chat(user, "<span style='color:#FF0000'>The biogenerator is currently processing.</span>")
	else if(istype(O, /obj/item/storage/bag/plants))
		var/i = 0
		for(var/obj/item/reagent_container/food/snacks/grown/G in contents)
			i++
		if(i >= 10)
			to_chat(user, "<span style='color:#FF0000'>The biogenerator is already full! Activate it.</span>")
		else
			for(var/obj/item/reagent_container/food/snacks/grown/G in O.contents)
				G.loc = src
				i++
				if(i >= 10)
					to_chat(user, "<span style='color:#0000FF'>You fill the biogenerator to its capacity.</span>")
					break
			if(i<10)
				to_chat(user, "<span style='color:#0000FF'>You empty the plant bag into the biogenerator.</span>")


	else if(!istype(O, /obj/item/reagent_container/food/snacks/grown))
		to_chat(user, "<span style='color:#FF0000'>You cannot put this in [src.name]</span>")
	else
		var/i = 0
		for(var/obj/item/reagent_container/food/snacks/grown/G in contents)
			i++
		if(i >= 10)
			to_chat(user, "<span style='color:#FF0000'>The biogenerator is full! Activate it.</span>")
		else
			if(user.drop_inv_item_to_loc(O, src))
				to_chat(user, "<span style='color:#0000FF'>You put [O.name] in [src.name]</span>")
	update_icon()
...
/obj/machinery/biogenerator/proc/activate()
	...
	if (src.stat != 0) //NOPOWER etc
		return
	if(src.processing)
		to_chat(usr, "<span style='color:#FF0000'>The biogenerator is in the process of working.</span>")
		return
	var/S = 0
