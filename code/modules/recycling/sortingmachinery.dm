		...
		if(istype(W, /obj/item/device/destTagger))
			...
			switch(alert("What would you like to alter?",,"Title","Description", "Cancel"))
				...
				if("Description")
					...
					if(!str || !length(str))
						to_chat(usr, "<span style='color:#FF0000'>Invalid text.</span>")
						return
					if(!examtext && !nameset)
						examtext = str
						update_icon()
...
/obj/item/packageWrap
	...
	afterattack(var/obj/target as obj, mob/user as mob, proximity)
		...
		if (istype(target, /obj/item) && !(istype(target, /obj/item/storage) && !istype(target,/obj/item/storage/box)))
			...
			to_chat(user, "<span style='color:#0000FF'>The object you are trying to wrap is unsuitable for the sorting machinery!</span>")
		if (src.amount <= 0)
	...
	examine(mob/user)
		...
		if(get_dist(src, user) < 2)
			to_chat(user, "<span style='color:#0000FF'>There are [amount] units of package wrap left!</span>")
