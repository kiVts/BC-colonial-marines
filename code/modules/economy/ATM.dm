...
/obj/machinery/atm/attackby(obj/item/I as obj, mob/user as mob)
	if(istype(I, /obj/item/card))
		if(emagged > 0)
			...
			to_chat(user, "<span style='color:#FF0000'>\icon[src] CARD READER ERROR. This system has been compromised!</span>")
			return
			...
			to_chat(user, "<span style='color:#FF0000'>\icon[src] The [src] beeps: \"[response]\"</span>")
			return
...
/obj/machinery/atm/attack_hand(mob/user as mob)
	if(istype(user, /mob/living/silicon))
		to_chat(user, "<span style='color:#FF0000'>\icon[src] Artificial unit recognized. Artificial units do not currently receive monetary compensation, as per Weyland Yutani regulation #1005.</span>")
		return
		...
		switch(href_list["choice"])
					...
					if(!authenticated_account)
						...
						if(previous_account_number == tried_account_num)
							if(number_incorrect_tries > max_pin_attempts)
								...
								to_chat(usr, "<span style='color:#FF0000'>\icon[src] Incorrect pin/account combination entered, [max_pin_attempts - number_incorrect_tries] attempts remaining.</span>")
								previous_account_number = tried_account_num
								playsound(src, 'sound/machines/buzz-sigh.ogg', 25, 1)
						else
							to_chat(usr, "<span style='color:#FF0000'>\icon[src] incorrect pin/account combination entered.</span>")
							number_incorrect_tries = 0
					else
						...
						to_chat(usr, "<span style='color:#0000FF'>\icon[src] Access granted. Welcome user '[authenticated_account.owner_name].'</span>")
			...
			if("insert_card")
				if(!held_card)
					...
					if(emagged > 0)
						to_chat(usr, "<span style='color:#FF0000'>\icon[src] The ATM card reader rejected your ID because this machine has been sabotaged!</span>")
					else
	...
	if(!authenticated_account)
		if(human_user.wear_id)
			...
			if(I)
				...
				if(authenticated_account)
					to_chat(human_user, "<span style='color:#0000FF'>\icon[src] Access granted. Welcome user '[authenticated_account.owner_name].'</span>")
