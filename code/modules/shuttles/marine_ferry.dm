	...
	for(var/datum/shuttle/ferry/marine/M in shuttle_controller.process_shuttles)
		if(M.info_tag == "Almayer Evac" || M.info_tag == "Alt Almayer Evac")
			...
			spawn(1)
				M.short_jump()
				to_chat(world, "LAUNCHED THING WITH TAG [M.shuttle_tag]")
		else to_chat(world, "did not launch thing with tag [M.shuttle_tag]")
*/
...
/datum/shuttle/ferry/marine/proc/long_jump_crash()
	...
	for(var/X in equipments)
		...
		if(istype(E, /obj/structure/dropship_equipment/adv_comp/docking))
			...
			else message_admins("<span style='color:#0000FF'>no crash turf found in Almayer Hangar, contact coders.</span>")
			break
	...
	for(var/mob/living/carbon/M in mob_list) //knock down mobs
		...
		if(M.buckled)
			to_chat(M, "<span style='color:#FF0000'>You are jolted against [M.buckled]!</span>")
			shake_camera(M, 3, 1)
		else
			to_chat(M, "<span style='color:#FF0000'>The floor jolts under your feet!</span>")
			shake_camera(M, 10, 1)
			M.KnockDown(3)
