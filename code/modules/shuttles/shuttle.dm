		...
		if(M.client)
			spawn(0)
				if(M.buckled && !iselevator)
					to_chat(M, "<span style='color:#FF0000'>Sudden acceleration presses you into [M.buckled]!</span>")
					shake_camera(M, 3, 1)
				else if (!M.buckled)
					to_chat(M, "<span style='color:#FF0000'>The floor lurches beneath you!</span>")
