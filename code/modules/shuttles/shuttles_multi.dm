...
/obj/machinery/computer/shuttle_control/multi/Topic(href, href_list)
	...
	if (MS.moving_status != SHUTTLE_IDLE)
		to_chat(usr, "<span style='color:#0000FF'>[shuttle_tag] vessel is moving.</span>")
		return

	if(href_list["start"])

		if(MS.at_origin)
			to_chat(usr, "<span style='color:#FF0000'>You are already at your home base.</span>")
			return

		if(!MS.return_warning)
			to_chat(usr, "<span style='color:#FF0000'>Returning to your home base will end your mission. If you are sure, press the button again.</span>")
	...
	if(href_list["toggle_cloak"])
		...
		to_chat(usr, "<span style='color:#FF0000'>Ship stealth systems have been [(MS.cloaked ? "activated. The station will not" : "deactivated. The station will")] be warned of our arrival.</span>")

	if(href_list["move_multi"])
		if((MS.last_move + MS.cooldown*10) > world.time)
			to_chat(usr, "<span style='color:#FF0000'>The ship's drive is inoperable while the engines are charging.</span>")
			return

		var/choice = input("Select a destination.") as null|anything in MS.destinations
		if(!choice) return

		to_chat(usr, "<span style='color:#0000FF'>[shuttle_tag] main computer recieved message.</span>")
