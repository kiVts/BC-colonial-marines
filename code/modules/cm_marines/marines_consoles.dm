...
/obj/machinery/computer/marine_card
	...
	desc = "Вы можете использовать это для изменения идентификаторов."
	icon_state = "id"
	...
	if(istype(O, /obj/item/card/id))
		...
		if(ACCESS_MARINE_LOGISTICS in idcard.access)
			if(!scan)
				...
				to_chat(user, "Оба слота уже заняты. Сначала извлеките карту.")
		else
			if(!modify)
				usr.drop_held_item()
				idcard.loc = src
		...
		if ("assign")
			if (authenticated)
				...
				if(t1 == "Custom")
					...
					if(!jobdatum)
						to_chat(usr, "<span style='color:#FF0000'>No log exists for this job.</span>")
						return

					if(!modify)
						to_chat(usr, "<span style='color:#FF0000'>No card to modify!</span>")
						return
