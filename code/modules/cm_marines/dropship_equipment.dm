...
/obj/effect/attach_point
	...
	desc = "Место где тЯжелое оборудование можно установить с поверлоадера."
	unacidable = TRUE
	anchored = TRUE
	...
	if(istype(I, /obj/item/powerloader_clamp))
		...
		if(istype(PC.loaded, /obj/structure/dropship_equipment))
			...
			if(SE.equip_category != base_category)
				to_chat(user, "<span class='warning'>[SE] на [src].</span>")
				return TRUE
			if(installed_equipment)
				return TRUE
			...
			if(installed_equipment || PC.loaded != SE)
				return TRUE
			to_chat(user, "<span class='notice'>Вы устанавливаете [SE] на [src].</span>")
			SE.forceMove(loc)
			PC.loaded = null
...
/obj/structure/dropship_equipment/attackby(obj/item/I, mob/user)
	...
	if(PC.loaded)
		...
		if(SA.equipment_type != type)
			to_chat(user, "<span class='warning'>[SA] не вписывается в [src].</span>")
			return FALSE
		...
		PC.update_icon()
		to_chat(user, "<span class='notice'>Вы загружаете [SA] в [src].</span>")
		ammo_equipped = SA
		update_equipment()
		...
		if(!ammo_equipped.ammo_count)
			...
			to_chat(user, "<span class='notice'>Вы отбросили пустой [ammo_equipped.name] в [src].</span>")
			cdel(ammo_equipped)
		else
			ammo_equipped.forceMove(PC.linked_powerloader)
			PC.loaded = ammo_equipped
			PC.update_icon()
			to_chat(user, "<span class='notice'>Вы удалили [ammo_equipped] из [src] и загрузили его в [PC].</span>")
		ammo_equipped = null
		update_icon()
		...
		playsound(src, 'sound/machines/hydraulics_1.ogg', 40, 1)
		PC.update_icon()
		to_chat(user, "<span class='notice'>Вы [ship_base ? "uninstalled" : "grabbed"] [PC.loaded] с [PC].</span>")
		if(ship_base)
			ship_base.installed_equipment = null
			ship_base = null
			if(linked_shuttle)
...
/obj/structure/dropship_equipment/proc/equipment_interact(mob/user)
	if(is_interactable)
		...
		to_chat(user, "<span class='notice'>Выберите [src].</span>")
...
/obj/structure/dropship_equipment/sentry_holder
	...
	desc = "Ящик, который разворачивает сторожевую башню. Подходит на оружие прикрепить точек челноков. Вам нужен поверлоадер, чтобы поднять его."
...
/obj/structure/dropship_equipment/sentry_holder/examine(mob/user)
	. = ..()
	if(!deployed_turret)
		to_chat(user, "Его башня отсутствует.")
...
/obj/structure/dropship_equipment/sentry_holder/equipment_interact(mob/user)
	if(!deployed_turret)
		to_chat(user, "<span class='warning'>[src] не отвечает.</span>")
		return
	if(deployment_cooldown > world.time)
		to_chat(user, "<span class='warning'>[src] занят.</span>")
		return //prevents spamming deployment/undeployment
	if(deployed_turret.loc == src) //not deployed
		if(z == LOW_ORBIT_Z_LEVEL)
			to_chat(user, "<span class='warning'>[src] не может развернуться в полёте.</span>")
		else
			to_chat(user, "<span class='notice'>Вы развертываете [src].</span>")
			deploy_sentry()
	else
		to_chat(user, "<span class='notice'>Вы убираете [src].</span>")
		undeploy_sentry()
...
/obj/structure/dropship_equipment/mg_holder
	...
	desc = "Коробка, которая развертывает модифицированный пулемет m56d crewserved. На дропшипы прикрепляют орудия. Вам нужен поверлоадер, чтобы поднять его."
...
/obj/structure/dropship_equipment/mg_holder/examine(mob/user)
	. = ..()
	if(!deployed_mg)
		to_chat(user, "Его пулемет пропал.")
...
/obj/structure/dropship_equipment/fuel/fuel_enhancer
	name = "fuel enhancer"
	desc = "Топливная система для шаттлов. Оно улучшает тягу произведенную сгоранием топлива для более быстрых перемещений. Пригонки внутри точек присоединения двигателя. Вам нужен поверлоадер, чтобы поднять его."
	icon_state = "fuel_enhancer"
	point_cost = 800

/obj/structure/dropship_equipment/fuel/cooling_system
	name = "cooling system"
	desc = "Система охлаждения шаттлов. Оно производит дополнительный охлаждать уменьшая задержки между стартом. Пригонки внутри точек присоединения двигателя. Вам нужно поверлоадер, чтобы поднять его."
...
/obj/structure/dropship_equipment/electronics/spotlights
	...
	desc = "Набор мощных прожекторов для освещения больших площадей. Подходит на электронику пристроить точки челноков. Перемещение этого потребует поверлоадер."
	is_interactable = TRUE
	point_cost = 300
...
/obj/structure/dropship_equipment/electronics/spotlights/equipment_interact(mob/user)
	...
	if(luminosity != brightness)
		...
		to_chat(user, "<span class='notice'>Вы включаете [src].</span>")
	else
		SetLuminosity(0)
		icon_state = "spotlights_off"
		to_chat(user, "<span class='notice'>Вы выключаете [src].</span>")
...
/obj/structure/dropship_equipment/electronics/targeting_system
	...
	desc = "Система наведения на шаттлы. Повышает точность стрельбы по лазерным мишеням. Пригонки на точках присоединения электроники. Вам нужен поверлоадер, чтобы поднять это."
	point_cost = 800
...
/obj/structure/dropship_equipment/electronics/laser_detector
	name = "laser detector"
	desc = "Электронное устройство, связанное с системой камеры шаттла, которая позволяет наблюдать лазерные цели на земле в середине полета."
...
/obj/structure/dropship_equipment/electronics/landing_zone_detector
	name = "LZ detector"
	desc = "Электронное устройство, связанное с системой камеры шаттла, которая позволяет наблюдать за вашей посадочной зоны в середине полета."
...
/obj/structure/dropship_equipment/weapon/heavygun
	...
	desc = "Спаренная 30-мм роторная пушка ГАУ-21 'гремучая змея'. Кажется, что он пропускает свои каналы питания и имеет открытые соединительные провода. Способен производить 5200 выстрелов в минуту, многие боятся за его мощность. Получил прозвище 'гремучая змея' из-за вибраций, которые она вызывала на шаттлах во время своего начального производства."
...
/obj/structure/dropship_equipment/weapon/rocket_pod
	...
	desc = "Система ракетного оружия, способная запускать одну ракету с лазерным наведением. Переезд этого потребует какой-то лифтер."
...
/obj/structure/dropship_equipment/weapon/minirocket_pod
	...
	desc = "Мини - ракетный блок, способный запускать шесть мини-ракет с лазерным наведением. Переезд этого потребует какой-то лифтер."
	icon = 'icons/Marine/almayer_props64.dmi'
	firing_sound = 'sound/weapons/gun_flare_explode.ogg'
...
/obj/structure/dropship_equipment/weapon/laser_beam_gun
	...
	desc = "Современная технология, недавно приобретенная USCM, она стреляет импульсным лазерным лучом с батарейным питанием со скоростью света, поджигая все, к чему прикасается. Переезд этого потребует какой-то лифтер."
	icon = 'icons/Marine/almayer_props64.dmi'
...
/obj/structure/dropship_equipment/weapon/launch_bay
	...
	desc = "A launch bay to drop special ordnance. Fits inside the dropship's crew weapon emplacement. Переезд этого потребует какой-то лифтер."
	icon = 'icons/Marine/almayer_props.dmi'
	firing_sound = 'sound/weapons/gun_flare_explode.ogg'
	firing_delay = 10 //1 seconds
...
/obj/structure/dropship_equipment/medevac_system
	name = "medevac system"
	desc = "Лебедка для подъема раненых морских пехотинцев на медицинских носилках на шаттл. Получить цель подъема через консоль оборудования шаттла."
	equip_category = DROPSHIP_CREW_WEAPON
...
/obj/structure/dropship_equipment/medevac_system/equipment_interact(mob/user)
	...
	if(linked_shuttle.moving_status != SHUTTLE_INTRANSIT)
		to_chat(user, "<span class='warning'>[src] может использоваться только в полете.</span>")
		return

	if(!linked_shuttle.transit_gun_mission)
		to_chat(user, "<span class='warning'>[src] требует, чтобы полет был использован.</span>")
		return

	if(busy_winch)
		to_chat(user, "<span class='warning'> Лебедка уже в движении.</span>")
		return

	if(world.time < medevac_cooldown)
		to_chat(user, "<span class='warning'>[src] после использовался вам нужно немного подождать прежде чем использовать его снова.</span>")
		return
	...
	if(!possible_stretchers.len)
		to_chat(user, "<span class='warning'>Никаких активных медицинских носилок не обнаружено.</span>")
		return

	var/stretcher_choice = input("С какими носилками вы хотели бы связаться?", "Доступные носилки") as null|anything in possible_stretchers
	if(!stretcher_choice)
		return
	...
	if(!selected_stretcher.buckled_mob && !selected_stretcher.buckled_bodybag)
		to_chat(user, "<span class='warning'>Эти носилки пусты.</span>")
		return

	if(selected_stretcher.linked_medevac && selected_stretcher.linked_medevac != src)
		to_chat(user, "<span class='warning'>Еще один шаттл парит над носилками.Еще один шаттл парит над носилками.</span>")
		return
	...
	if(selected_stretcher == linked_stretcher) //already linked to us, unlink it
		to_chat(user, "<span class='notice'> Уберите свой шаттл подальше от маяка носилок.</span>")
		linked_stretcher.visible_message("<span class='notice'>[linked_stretcher] обнаруживает, что шаттл больше не находится над головой.</span>")
		linked_stretcher.linked_medevac = null
		linked_stretcher = null
		return

	to_chat(user, "<span class='notice'> Вы перемещаете свой шаттл над маяком выбранного носилки.</span>")
	...
	linked_stretcher.visible_message("<span class='notice'>[linked_stretcher] обнаруживает шаттл над головой.</span>")
...
/obj/structure/dropship_equipment/medevac_system/attack_hand(mob/user)
	...
	if(user.mind && user.mind.cm_skills && user.mind.cm_skills.pilot < SKILL_PILOT_TRAINED)
		to_chat(user, "<span class='warning'> Вы не знаете, как использовать [src].</span>")
		return
	...
	if(linked_shuttle.moving_status != SHUTTLE_INTRANSIT)
		to_chat(user, "<span class='warning'>[src] может использоваться только во время полета</span>")
		return
	...
	if(!linked_stretcher)
		to_chat(user, "<span class='warning'>Похоже, что к [src] не подключены медицинские носилки.</span>")
		return
...
/obj/structure/dropship_equipment/medevac_system/proc/activate_winch(mob/user)
	...
	user.visible_message("<span class='notice'>[user] активирует лебёдку [src]'s.</span>", \
						"<span class='notice'>Вы активировали лебёдку [src]'s.</span>")
	sleep(30)
	...
	if(fail)
		if(linked_stretcher)
			linked_stretcher.linked_medevac = null
			linked_stretcher = null
		to_chat(user, "<span class='warning'>Лебедка заканчивает подъем, но, похоже, нет носилок медэвака, подключенных к [src].</span>")
		return
	...
	if(lifted_object)
		...
		to_chat(user, "<span class='warning'>Лебедка заканчивает поднимать носилки, но они пусты!</span>")
		linked_stretcher.linked_medevac = null
		linked_stretcher = null
		return

	flick("winched_stretcher", linked_stretcher)
	linked_stretcher.visible_message("<span class='notice'>Крюк лебедки падает с неба и начинает поднимать [linked_stretcher] вверх.</span>")
