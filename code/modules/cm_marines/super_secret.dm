...
/obj/structure/nobugs
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if(istype(W,/obj/item/coin))
			if(coins < 4 && !state)
				user.visible_message("[user] inserts \the [W] into \the [src].","<span style='color:#0000FF'>You insert \the [W] inside \the [src].</span>")
				coins++
				cdel(W)
			else
				to_chat(user, "<span style='color:#FF0000'>\the [W] does not fit anymore.</span>")
		else
			to_chat(user, "<span style='color:#FF0000'>\the [W] does not fit inside that rectangular hole.</span>")
	...
	attack_hand(mob/user)
		...
		if(!state)
			if(coins)
				user.visible_message("You hear a click from somewhere near...","<span style='color:#0000FF'>You press the button and hear a click coming from inside...</span>")
				state = 1
				spawn(rand(100,200))
					switch(coins)
						if(1)
				...
				to_chat(usr, "<span style='color:#FF0000'>Nothing happened...</span>")
		else
			to_chat(usr, "<span style='color:#FF0000'>Nothing happened...</span>")