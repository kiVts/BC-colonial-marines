...
/obj/item/ammo_magazine/sentry/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
...
/obj/item/storage/box/sentry/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 79225)
	else
		serial_number = given_serial
	...
	switch(href_list["op"])
		...
		if("burst")
			...
			if(burst_fire)
				...
				to_chat(usr, "<span style='color:#0000FF'>You deactivate the burst fire mode.</span>")
			else
				burst_fire = 1
				fire_delay = 15
...
/obj/machinery/marine_turret/premadeu
	name = "UA-577 Gauss Turret"
	immobile = TRUE
	on = TRUE
	burst_fire = TRUE
...
/obj/machinery/marine_turret/premadeu/New()
	spark_system = new /datum/effect_system/spark_spread
	spark_system.set_up(5, 0, src)
	spark_system.attach(src)
	var/obj/item/cell/super/H = new(src) //Better cells in these ones.
...
/obj/machinery/marine_turret/premade/dumb/attack_hand(mob/user as mob)
	...
	if(!on)
		...
		visible_message("<span style='color:#0000FF'>[src] hums to life and emits several beeps.</span>")
