...
/obj/machinery/r_n_d/bioprinter/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if (!linked_console)
		...
		return 1
	if (busy)
		to_chat(user, "<span style='color:#FF0000'>The Weyland Yutani Brand Bioprinter(TM) is busy. Please wait for completion of previous operation.</span>")
		return 1
	...
	if (do_after(user, 16, TRUE, 5, BUSY_ICON_GENERIC))
		to_chat(user, "<span style='color:#0000FF'>You add a [O] to the [src.name].</span>")
		icon_state = "protolathe"
