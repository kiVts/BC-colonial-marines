...
/obj/machinery/computer/rdconsole/attackby(var/obj/item/D as obj, var/mob/user as mob)
	...
	if(istype(D, /obj/item/disk))
		...
		if(istype(D, /obj/item/disk/tech_disk)) t_disk = D
			...
			to_chat(user, "<span style='color:#FF0000'>Machine cannot accept disks in that format.</span>")
			return
		user.drop_held_item()
		D.loc = src
		to_chat(user, "<span style='color:#0000FF'>You add the disk to the machine!</span>")
	else if(istype(D, /obj/item/card/emag) && !emagged)
		playsound(src.loc, 'sound/effects/sparks4.ogg', 75, 1)
		emagged = 1
		to_chat(user, "<span style='color:#0000FF'>You you disable the security protocols</span>")
	else
	...
	if(href_list["menu"]) //Switches menu screens. Converts a sent text string into a number. Saves a LOT of code.
		...
		if(linked_destroy)
			if(linked_destroy.busy)
				to_chat(usr, "<span style='color:#FF0000'>The destructive analyzer is busy at the moment.</span>")
					...
					if(linked_destroy)
						...
						if(!linked_destroy.hacked)
							if(!linked_destroy.loaded_item)
								to_chat(usr, "<span style='color:#FF0000'>The destructive analyzer appears to be empty.</span>")
								screen = 1.0
								return
		...
		screen = 0.0
		if(!sync)
			to_chat(usr, "<span style='color:#FF0000'>You must connect to the network first!</span>")
		else
	...
	// CM HREFS FOR THE NEW MACHINES
		...
		if(linked_organic)
			if(linked_organic.busy)
				to_chat(usr, "<span style='color:#FF0000'>The Weyland Brand Organic Analyzer(TM) is busy at the moment.</span>")
			else
					...
					if(linked_organic)
						...
						if(!linked_organic.hacked)
							if(!linked_organic.loaded_item)
								to_chat(usr, "<span style='color:#FF0000'>The Weyland Brand Organic Analyzer(TM) appears to be empty.</span>")
								screen = 1.0
								return
