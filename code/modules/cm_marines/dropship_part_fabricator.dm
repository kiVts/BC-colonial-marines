...
/obj/machinery/dropship_part_fabricator
	...
	desc = "Большой автоматизированный 3D-принтер для производства новых деталей шаттла."
	density = 1
	anchored = 1
	use_power = 1
	idle_power_usage = 20
...
/obj/machinery/dropship_part_fabricator/proc/build_dropship_part(part_type, cost, mob/user)
	...
	if(dropship_points < cost)
		to_chat(user, "<span class='warning'>У вас недостаточно поинтов, чтобы напечатать это.</span>")
		return
	visible_message("<span class='notice'>[src] начинает печатать что-то.</span>")
...
/obj/machinery/dropship_part_fabricator/Topic(href, href_list)
	...
	if(busy)
		to_chat(usr, "<span class='warning'>Автолат занят. Пожалуйста, дождитесь завершения предыдущей операции.</span>")
		return
