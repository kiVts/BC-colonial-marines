/obj/item/weapon/stunprod
	...
	suicide_act(mob/user)
		viewers(user) << "<span style='color:#FF0000'><b>[user] is putting the live [src.name] in \his mouth! It looks like \he's trying to commit suicide.</b></span>"
		return (FIRELOSS)
...
/obj/item/weapon/stunprod/attack_self(mob/user as mob)
	if(status && (CLUMSY in user.mutations) && prob(50))
		to_chat(user, "<span style='color:#FF0000'>You grab the [src] on the wrong side.</span>")
		user.KnockDown(30)
		charges--
		if(charges < 1)
			status = 0
