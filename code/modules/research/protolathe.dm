...
/obj/machinery/r_n_d/protolathe/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if (!linked_console)
		...
		return 1
	if (busy)
		to_chat(user, "<span style='color:#FF0000'>The protolathe is busy. Please wait for completion of previous operation.</span>")
		return 1
	if (!istype(O, /obj/item/stack/sheet))
		to_chat(user, "<span style='color:#FF0000'>You cannot insert this item into the protolathe!</span>")
		return 1
	if (stat)
		return 1
	if(istype(O,/obj/item/stack/sheet))
		...
		if (TotalMaterials() + S.perunit > max_material_storage)
			to_chat(user, "<span style='color:#FF0000'>The protolathe's material bin is full. Please remove material before adding more.</span>")
			return 1
