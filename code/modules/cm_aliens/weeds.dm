...
/obj/effect/alien/weeds/attackby(obj/item/W, mob/living/user)
	...
	if(istype(W, /obj/item/marineResearch/sampler)) // Taking samples
		...
		if(A.filled)
			return TRUE
		to_chat(user, "Вы взЯли образец из [src].")
	...
	if(istype(src, /obj/effect/alien/weeds/node)) //The pain is real
		to_chat(user, "<span class='warning'>Вы нажимаете \the [src] с помощью \the [W].</span>")
	else
		to_chat(user, "<span class='warning'>Вы вырезаете \the [src] с помощью \the [W].</span>")
