...
/obj/ovipositor/attack_alien(mob/living/carbon/Xenomorph/M)
	switch(M.a_intent)
		if("help")
			M.visible_message("<span class='notice'>\The [M] ласки [src] коса-как рука.</span>", \
			"<span class='notice'>Вы ласкаете [src] своей косой рукой.</span>")
			return 0
		...
		if("hurt")
			...
			M.visible_message("<span class='danger'>\The [M] укусы [src]!</span>", \
			"<span class='danger'>Вы кусаете [src]!</span>")
			health -= damage
			return 1

		if("disarm")
			to_chat(M, "<span class='warning'>Нет ничего, чтобы разоружить!</span>")
...
/obj/ovipositor/attack_larva(mob/living/carbon/Xenomorph/Larva/M)
	...
	"<span class='danger'>Ты прижимаешьсЯ головой к [src].</span>")
