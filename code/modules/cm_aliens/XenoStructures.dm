...
/obj/effect/alien/resin
	name = "resin"
	desc = "Похоже на какой-то слизистый нарост."
	icon_state = "Resin1"
	anchored = 1
	health = 200
	unacidable = 1
...
/obj/effect/alien/resin/hitby(AM as mob|obj)
	...
	if(istype(AM,/mob/living/carbon/Xenomorph))
		return
	visible_message("<span class='danger'>\The [src] был сбит \the [AM].</span>", \
	"<span class='danger'>Ты попал \the [src].</span>")
	var/tforce = 0
	if(ismob(AM))
		tforce = 10
	else
...
/obj/effect/alien/resin/attack_alien(mob/living/carbon/Xenomorph/M)
	if(isXenoLarva(M)) //Larvae can't do shit
		return 0
	M.visible_message("<span class='xenonotice'>\The [M] когти \the [src]!</span>", \
	"<span class='xenonotice'>Вы когтЯми \the [src].</span>")
	if(istype(src, /obj/effect/alien/resin/sticky))
		playsound(loc, "alien_resin_move", 25)
	else
		playsound(loc, "alien_resin_break", 25)
...
/obj/effect/alien/resin/attack_animal(mob/living/M as mob)
	M.visible_message("<span class='danger'>[M] слезы \the [src]!</span>", \
	"<span class='danger'>Вы разрываете \the [name].</span>")
	if(istype(src, /obj/effect/alien/resin/sticky))
		playsound(loc, "alien_resin_move", 25)
	else
		playsound(loc, "alien_resin_break", 25)
...
/obj/effect/alien/resin/attack_hand()
	to_chat(usr, "<span class='warning'>Вы неэффективно царапаете \the [src].</span>")
...
/*/obj/effect/alien/resin/speedy/thin
	...
	desc = "Тонкий слой странного желе."
	health = 7
	speed_amt = 4 */

//Carrier trap
/obj/effect/alien/resin/trap
	desc = "Похоже что-то, прЯчетсЯ в отверстие."
	name = "resin hole"
	icon_state = "trap0"
	density = 0
	opacity = 0
...
/obj/effect/alien/resin/trap/examine(mob/user)
	if(isXeno(user))
		to_chat(user, "Нора длЯ маленького чего-то, чтобы спрЯтатьсЯ в засаде.")
		if(hugger)
			to_chat(user, "Там внутри что-то маленькое.")
		else
			to_chat(user, "Оно пусто.")
	else
		..()
...
/obj/effect/alien/resin/trap/HasProximity(atom/movable/AM)
	if(hugger)
		if(CanHug(AM) && !isYautja(AM) && !isSynth(AM))
			var/mob/living/L = AM
			L.visible_message("<span class='warning'>[L] поездки на [src]!</span>",\
							"<span class='danger'>Вы спотыкаетесь на [src]!</span>")
			L.KnockDown(1)
			if(carrier_number)
				for(var/mob/living/carbon/Xenomorph/X in living_mob_list)
					if(X.nicknumber == carrier_number)
						if(!X.stat)
							...
							if(A)
								to_chat(X, "<span class='xenoannounce'>Вы чувствуете, что одна из ваших ловушек в [A.name] была запущена!</span>")
						break
			drop_hugger()
...
/obj/effect/alien/resin/trap/proc/drop_hugger()
	...
	hugger = FALSE
	icon_state = "trap0"
	visible_message("<span class='warning'>[FH] выходит из [src]!</span>")
	sleep(15)
...
/obj/effect/alien/resin/trap/attack_alien(mob/living/carbon/Xenomorph/M)
	if(M.a_intent != "hurt")
		...
		if(allowed_castes.Find(M.caste))
			if(!hugger)
				to_chat(M, "<span class='warning'>[src] пусто.</span>")
			else
				hugger = FALSE
				icon_state = "trap0"
				var/obj/item/clothing/mask/facehugger/F = new ()
				M.put_in_active_hand(F)
				to_chat(M, "<span class='xenonotice'>Удалить facehugger из [src].</span>")
		return
	..()
...
/obj/effect/alien/resin/trap/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/clothing/mask/facehugger) && isXeno(user))
		...
		if(FH.stat == DEAD)
			to_chat(user, "<span class='warning'>Вы не можете поместить мертвого лицехват в [src].</span>")
		else
			hugger = TRUE
			icon_state = "trap1"
			to_chat(user, "<span class='xenonotice'>Вы помещаете лицехват в [src].</span>")
			cdel(FH)
	else
		. = ..()
...
/obj/structure/mineral_door/resin/attack_paw(mob/user as mob)
	if(user.a_intent == "hurt")
		user.visible_message("<span class='xenowarning'>\The [user] когти на \the [src].</span>", \
		"<span class='xenowarning'>Вы цеплЯетесь за \the [src].</span>")
		playsound(loc, "alien_resin_break", 25)
		health -= rand(40, 60)
		if(health <= 0)
			user.visible_message("<span class='xenodanger'>\The [user] разрезает \the [src] на части.</span>", \
			"<span class='xenodanger'>Вы разрезаете \the [src] на части.</span>")
		healthcheck()
		return
	else
...
/obj/structure/mineral_door/resin/attack_alien(mob/living/carbon/Xenomorph/M)
	...
	M.visible_message("<span class='warning'>\The [M] копаетсЯ в \the [src] и начинает разрывать его.</span>", \
	"<span class='warning'>Вы копаетесь в \the [src] и начинаете разрывать его.</span>", null, 5)
	...
	if(do_after(M, 80, FALSE, 5, BUSY_ICON_HOSTILE))
		...
		M.visible_message("<span class='danger'>[M] рипы вниз \the [src]!</span>", \
		 "<span class='danger'>Вы промчитесь вниз \the [src]!</span>", null, 5)
		cdel(src)
...
/obj/structure/mineral_door/resin/proc/check_resin_support()
	...
	for(var/i in cardinal)
		...
		if(locate(/obj/structure/mineral_door/resin) in T)
			...
			break
	if(!.)
		visible_message("<span class = 'notice'>[src] рушитсЯ из-за отсутствиЯ поддержки.</span>")
		cdel(src)
...
/obj/effect/alien/egg
	desc = "Это выглЯдит как странное Яйцо"
	name = "egg"
...
/obj/effect/alien/egg/attack_alien(mob/living/carbon/Xenomorph/M)
	...
	if(M.hivenumber != hivenumber)
		...
		M.visible_message("<span class='xenowarning'>[M] давит \the [src]","<span class='xenowarning'>Вы давите \the [src]")
		Burst(1)
		return
		...
		if(BURST, DESTROYED)
			switch(M.caste)
				if("Queen","Drone","Hivelord","Carrier")
					M.visible_message("<span class='xenonotice'>\The [M] очищает вылупившеесЯ Яйцо.</span>", \
					"<span class='xenonotice'>Вы очищаете вылупившеесЯ Яйцо.</span>")
					playsound(src.loc, "alien_resin_break", 25)
					M.plasma_stored++
					cdel(src)
		if(GROWING)
			to_chat(M, "<span class='xenowarning'>Ребенок еще не развит.</span>")
		if(GROWN)
			if(isXenoLarva(M))
				to_chat(M, "<span class='xenowarning'>Уложить Яйцо, но ничего не происходит.</span>")
				return
			to_chat(M, "<span class='xenonotice'>Вы забираете ребенка.</span>")
			Burst(0)
...
/obj/effect/alien/egg/attackby(obj/item/W, mob/living/user)
	...
	if(istype(W,/obj/item/clothing/mask/facehugger))
		...
		if(F.stat != DEAD)
			switch(status)
				if(BURST)
					if(user)
						visible_message("<span class='xenowarning'>[user] слайды [F] обратно в [src].</span>","<span class='xenonotice'>Вы помещаете ребенка обратно в [src].</span>")
						user.temp_drop_inv_item(F)
					else
						visible_message("<span class='xenowarning'>[F] заползает обратно в [src]!</span>") //Not sure how, but let's roll with it for now.
					status = GROWN
					icon_state = "Egg"
					cdel(F)
				if(DESTROYED) to_chat(user, "<span class='xenowarning'>Это Яйцо больше нельзЯ использовать.</span>")
				if(GROWING,GROWN) to_chat(user, "<span class='xenowarning'>Эта занЯта ребенком.</span>")
		else
			to_chat(user, "<span class='xenowarning'>Этот ребенок мертв.</span>")
		return
	...
	if(W.attack_verb.len)
		visible_message("<span class='danger'>\The [src] был [pick(W.attack_verb)] с \the [W][(user ? " by [user]." : ".")]</span>")
	else
		visible_message("<span class='danger'>\The [src] был атакован с \the [W][(user ? " by [user]." : ".")]</span>")
	var/damage = W.force
	if(W.w_class < 4 || !W.sharp || W.force < 20) //only big strong sharp weapon are adequate
		damage /= 4
...
/obj/structure/tunnel
	name = "tunnel"
	desc = "Вход в туннель. Похоже, его выкопал какой-то зверь с когтЯми."
	icon = 'icons/Xeno/effects.dmi'
	icon_state = "hole"
...
/obj/structure/tunnel/examine(mob/user)
	...
	if(!other)
		to_chat(user, "<span class='warning'>КажетсЯ, она никуда не ведет.</span>")
	else
		var/area/A = get_area(other)
		to_chat(user, "<span class='info'>КажетсЯ это ведет к <b>[A.name]</b>.</span>")
		if(tunnel_desc)
			to_chat(user, "<span class='info'>Запах Hivelord гласит: \'[tunnel_desc]\'</span>")

/obj/structure/tunnel/proc/healthcheck()
	if(health <= 0)
		visible_message("<span class='danger'>[src] внезапно рушитсЯ!</span>")
		if(other && isturf(other.loc))
			visible_message("<span class='danger'>[other] внезапно рушитсЯ!</span>")
			cdel(other)
			other = null
		cdel(src)
...
/obj/structure/tunnel/attack_alien(mob/living/carbon/Xenomorph/M)
	...
	if(ticker && ticker.mode && ticker.mode.flags_round_type & MODE_FOG_ACTIVATED)
		...
		if(!hive.living_xeno_queen)
			to_chat(M, "<span class='xenowarning'>Нет никакой королевы. Сначала ты должен стать Королевой.</span>")
			return FALSE
		else if(isXenoQueen(M))
			to_chat(M, "<span class='xenowarning'>Пока нет причин покидать безопасные пещеры.</span>")
			return FALSE

	if(M.anchored)
		to_chat(M, "<span class='xenowarning'>В неподвижном состоЯнии по туннелю не пролезешь.</span>")
		return FALSE
	...
	if(!other || !isturf(other.loc))
		to_chat(M, "<span class='warning'>\The [src] похоже никуда не ведет.</span>")
		return

	var/area/A = get_area(other)

	if(tunnel_time <= 50)
		M.visible_message("<span class='xenonotice'>\The [M] начинает ползти вниз в \the [src].</span>", \
		"<span class='xenonotice'>Вы начинаете ползти вниз в \the [src] к <b>[A.name]</b>.</span>")
	else
		M.visible_message("<span class='xenonotice'>[M]  начинает поднимать свою огромную массу вниз в \the [src].</span>", \
		"<span class='xenonotice'>Вы начинаете поднимать свою чудовищную массу в \the [src] к <b>[A.name]</b>.</span>")
	...
	if(do_after(M, tunnel_time, FALSE, 5, BUSY_ICON_GENERIC))
		if(other && isturf(other.loc)) //Make sure the end tunnel is still there
			...
			M.visible_message("<span class='xenonotice'>\The [M] выскакивает из \the [src].</span>", \
			"<span class='xenonotice'>Ты выскакиваешь с другой стороны!</span>")
		else
			to_chat(M, "<span class='warning'>\The [src] закончилсЯ неожиданно,поэтому вы возвращаетесь обратно.</span>")
	else
		to_chat(M, "<span class='warning'>Ваше ползание было прервано!</span>")
