...
/obj/item/clothing/suit/armor/reactive/attack_self(mob/user as mob)
	...
	if (src.active)
		to_chat(user, "<span style='color:#0000FF'>The reactive armor is now active.</span>")
		src.icon_state = "reactive"
		src.item_state = "reactive"
	else
		to_chat(user, "<span style='color:#0000FF'>The reactive armor is now inactive.</span>")
...
/obj/item/clothing/suit/armor/tactical/verb/holster()
	...
	if(!holstered)
		...
		if (W.w_class > 3)
			to_chat(usr, "<span style='color:#FF0000'>This gun won't fit in \the belt!</span>")
			return
		holstered = usr.get_active_hand()
		usr.drop_held_item()
		holstered.loc = src
		usr.visible_message("<span style='color:#0000FF'>\The [usr] holsters \the [holstered].", "You holster \the [holstered].</span>")
	else
		if(istype(usr.get_active_hand(),/obj) && istype(usr.get_inactive_hand(),/obj))
			to_chat(usr, "<span style='color:#FF0000'>You need an empty hand to draw the gun!</span>")
		else
			if(usr.a_intent == "hurt")
				usr.visible_message("<span style='color:#FF0000'>\The [usr] draws \the [holstered], ready to shoot!</span>", \
				"<span style='color:#FF0000'>You draw \the [holstered], ready to shoot!</span>")
			else
				usr.visible_message("<span style='color:#0000FF'>\The [usr] draws \the [holstered], pointing it at the ground.</span>", \
				"<span style='color:#0000FF'>You draw \the [holstered], pointing it at the ground.</span>")
