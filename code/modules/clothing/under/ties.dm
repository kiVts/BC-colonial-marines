...
/obj/item/clothing/tie/holster/proc/holster(obj/item/I, mob/user as mob)
	if(holstered)
		to_chat(user, "<span style='color:#FF0000'>There is already a [holstered] holstered here!</span>")
		return

	if (!istype(I, /obj/item/weapon/gun))
		to_chat(user, "<span style='color:#FF0000'>Only guns can be holstered!</span>")
		return

	var/obj/item/weapon/gun/W = I
	if (!can_holster(W))
		to_chat(user, "<span style='color:#FF0000'>This [W] won't fit in the [src]!</span>")
		return

	holstered = W
	user.drop_inv_item_to_loc(holstered, src)
	holstered.add_fingerprint(user)
	user.visible_message("<span style='color:#0000FF'>[user] holsters the [holstered].", "You holster the [holstered].</span>")
...
/obj/item/clothing/tie/holster/verb/holster_verb()
	...
	if (!H)
		to_chat(usr, "<span style='color:#FF0000'>Something is very wrong.</span>")

	if(!H.holstered)
		if(!istype(usr.get_active_hand(), /obj/item/weapon/gun))
			to_chat(usr, "<span style='color:#0000FF'>You need your gun equiped to holster it.</span>")
			return
...
/obj/item/clothing/tie/holobadge/attack_self(mob/user as mob)
	if(!stored_name)
		...
		return
	if(isliving(user))
		user.visible_message("<span style='color:#FF0000'>[user] displays their W-Y Internal Security Legal Authorization Badge.\nIt reads: [stored_name], W-Y Security.","\red You display your W-Y Internal Security Legal Authorization Badge.\nIt reads: [stored_name], W-Y Security.</span>")

/obj/item/clothing/tie/holobadge/attackby(var/obj/item/O as obj, var/mob/user as mob)

	if (istype(O, /obj/item/card/emag))
		if (emagged)
			to_chat(user, "<span style='color:#FF0000'>[src] is already cracked.</span>")
			return
		else
			emagged = 1
			to_chat(user, "<span style='color:#FF0000'>You swipe [O] and crack the holobadge security checks.</span>")
			return
...
/obj/item/clothing/tie/holobadge/attack(mob/living/carbon/human/M, mob/living/user)
	if(isliving(user))
		user.visible_message("<span style='color:#FF0000'>[user] invades [M]'s personal space, thrusting [src] into their face insistently.","\red You invade [M]'s personal space, thrusting [src] into their face insistently. You are the law.</span>")
