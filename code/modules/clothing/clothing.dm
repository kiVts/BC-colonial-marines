/obj/item/clothing
	...
	*/
	var/is_modifyed = 0 //var for checking modification
		...
		if(species_restricted)
			...
			if(H.species)
				...
				if(!wearable && (slot != 15 && slot != 16)) //Pockets.
					to_chat(M, "<span style='color:#FF0000'>Your species cannot wear [src].</span>")
					return 0
...
/obj/item/clothing/gloves/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/tool/wirecutters) || istype(W, /obj/item/tool/surgery/scalpel))
		...
		user.visible_message("<span style='color:#FF0000'>[user] cuts the fingertips off of the [src].","\red You cut the fingertips off of the [src].</span>")
