...
/obj/item/clothing/suit/space/rig/verb/toggle_helmet()
	...
	if(H.head == helmet)
		helmet.flags_item &= ~NODROP
		H.drop_inv_item_to_loc(helmet, src)
		to_chat(H, "<span style='color:#0000FF'>You retract your hardsuit helmet.</span>")
	else
		if(H.head)
			to_chat(H, "<span style='color:#FF0000'>You cannot deploy your helmet while wearing another helmet.</span>")
			return
		//TODO: Species check, skull damage for forcing an unfitting helmet on?
		helmet.loc = H
		helmet.pickup(H)
		H.equip_to_slot(helmet, WEAR_HEAD)
		helmet.flags_item |= NODROP
		to_chat(H, "<span style='color:#0000FF'>You deploy your hardsuit helmet, sealing you off from the world.</span>")
...
/obj/item/clothing/head/helmet/space/rig/syndi/attack_self(mob/user)
	if(camera)
		...
		to_chat(user, "<span style='color:#0000FF'>User scanned as [camera.c_tag]. Camera activated.</span>")
