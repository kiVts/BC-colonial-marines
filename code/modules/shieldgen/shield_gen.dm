...
/obj/machinery/shield_gen/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/card/id))
		...
		src.visible_message("<span style='color:#0000FF'>\icon[src] [src] has been [anchored?"bolted to the floor":"unbolted from the floor"] by [user].</span>")
...
/obj/machinery/shield_gen/Topic(href, href_list[])
	...
	if( href_list["close"] )
		...
		if (!active && !anchored)
			to_chat(usr, "<span style='color:#FF0000'>The [src] needs to be firmly secured to the floor first.</span>")
			return
		toggle()
