...
/obj/item/device/assembly_holder/attackby(obj/item/W as obj, mob/user as mob)
	if(isscrewdriver(W))
		if(!a_left || !a_right)
			to_chat(user, "<span style='color:#FF0000'>BUG:Assembly part missing, please report this!</span>")
			return
		a_left.toggle_secure()
		a_right.toggle_secure()
		secured = !secured
		if(secured)
			to_chat(user, "<span style='color:#0000FF'>\The [src] is ready!</span>")
		else
			to_chat(user, "<span style='color:#0000FF'>\The [src] can now be taken apart!</span>")
		update_icon()
		return
...
/obj/item/device/assembly_holder/attack_self(mob/user)
	...
	if(src.secured)
		if(!a_left || !a_right)
			to_chat(user, "<span style='color:#FF0000'>Assembly part missing!</span>")
			return
