...
/obj/item/weapon/fuel/proc/injest(mob/M as mob)
	switch(content)
		if("Anti-Hydrogen")
			M.gib()
		if("Hydrogen")
			to_chat(M, "<span style='color:#0000FF'>You feel very light, as if you might just float away...</span>")
	del(src)
	return
...
/obj/item/weapon/fuel/attack(mob/M as mob, mob/user as mob)
	if (user != M)
		...
		for(var/mob/O in viewers(M, null))
			O.show_message(text("<span style='color:#FF0000'>[M] ate the [content ? content : "empty canister"]!</span>"), 1)
		src.injest(M)
