...
/obj/machinery/power/am_control_unit/attackby(obj/item/W, mob/user)
	...
	if(istype(W, /obj/item/tool/wrench))
		if(!anchored)
			...
			to_chat(user, "<span style='color:#FF0000'>Once bolted and linked to a shielding unit it the [src.name] is unable to be moved!</span>")
		return

	if(istype(W, /obj/item/am_containment))
		if(fueljar)
			to_chat(user, "<span style='color:#FF0000'>There is already a [fueljar] inside!</span>")
			return
