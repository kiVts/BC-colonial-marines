...
/obj/machinery/power/port_gen/examine(mob/user)
	..()
	if(active)
		to_chat(user, "<span style='color:#0000FF'>The generator is on.</span>")
	else
		to_chat(user, "<span style='color:#0000FF'>The generator is off.</span>")
...
/obj/machinery/power/port_gen/pacman/examine(mob/user)
	..()
	to_chat(user, "<span style='color:#0000FF'>The generator has [sheets] units of [sheet_name] fuel left, producing [power_gen] per cycle.</span>")
	if(crit_fail) to_chat(user, "<span style='color:#FF0000'>The generator seems to have broken down.</span>")
...
/obj/machinery/power/port_gen/pacman/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(istype(O, sheet_path))
		...
		if(amount < 1)
			to_chat(user, "<span style='color:#0000FF'>The [src.name] is full!</span>")
			return
		to_chat(user, "<span style='color:#0000FF'>You add [amount] sheets to the [src.name].</span>")
		sheets += amount
		addstack.use(amount)
		updateUsrDialog()
		return
		...
		if(istype(O, /obj/item/tool/wrench))
			...
			if(!anchored)
				connect_to_network()
				to_chat(user, "<span style='color:#0000FF'>You secure the generator to the floor.</span>")
			else
				disconnect_from_network()
				to_chat(user, "<span style='color:#0000FF'>You unsecure the generator from the floor.</span>")
			...
			if(open)
				to_chat(user, "<span style='color:#0000FF'>You open the access panel.</span>")
			else
				to_chat(user, "<span style='color:#0000FF'>You close the access panel.</span>")
