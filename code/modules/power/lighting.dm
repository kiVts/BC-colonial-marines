...
/obj/machinery/light/attack_animal(mob/living/M)
	...
	if(status == LIGHT_EMPTY||status == LIGHT_BROKEN)
		to_chat(M, "<span style='color:#FF0000'>That object is useless to you.</span>")
		return
	else if (status == LIGHT_OK||status == LIGHT_BURNED)
		for(var/mob/O in viewers(src))
			O.show_message("<span style='color:#FF0000'>[M.name] smashed the light!", 3, "You hear a tinkle of breaking glass</span>", 2)
		broken()
	return
...
/obj/machinery/light/attack_hand(mob/user)
	...
	if(istype(user,/mob/living/carbon/human))
		...
		if(H.species.can_shred(H))
			for(var/mob/M in viewers(src))
				M.show_message("<span style='color:#FF0000'>[user.name] smashed the light!", 3, "You hear a tinkle of breaking glass</span>", 2)
			broken()
			return
...
/obj/item/light_bulb/proc/shatter()
	if(status == LIGHT_OK || status == LIGHT_BURNED)
		src.visible_message("<span style='color:#FF0000'>[name] shatters.","\red You hear a small glass object shatter.</span>")
		status = LIGHT_BROKEN
		force = 5
