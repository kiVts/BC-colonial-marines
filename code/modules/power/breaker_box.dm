...
/obj/machinery/power/breakerbox/examine(mob/user)
	...
	if(on)
		...
		to_chat(user, "<span style='color:#FF0000'>It seems to be offline</span>")

/obj/machinery/power/breakerbox/attack_ai(mob/user)
	if(busy)
		to_chat(user, "<span style='color:#FF0000'>System is busy. Please wait until current operation is finished before changing power settings.</span>")
		return
...
/obj/machinery/power/breakerbox/attack_hand(mob/user)
	...
	for(var/mob/O in viewers(user))
		O.show_message(text("<span style='color:#FF0000'>[user] started reprogramming [src]!</span>"), 1)
