...
/obj/machinery/power/rad_collector/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/tank/phoron))
		if(!src.anchored)
			to_chat(user, "<span style='color:#FF0000'>The [src] needs to be secured to the floor first.</span>")
			return 1
		if(src.P)
			to_chat(user, "<span style='color:#FF0000'>There's already a phoron tank loaded.</span>")
			return 1
		...
		if(P)
			to_chat(user, "<span style='color:#0000FF'>Remove the phoron tank first.</span>")
			return 1
		...
		if (src.allowed(user))
			if(active)
				...
				to_chat(user, "<span style='color:#FF0000'>The controls can only be locked when the [src] is active</span>")
		else
