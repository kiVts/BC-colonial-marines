...
/mob/living/carbon/Xenomorph/proc/create_s_shriekwave()
	overlays_standing[X_SUIT_LAYER] = image("icon"='icons/Xeno/2x2_Xenos.dmi', "icon_state" = "s_shriek_waves") //Ehh, suit layer's not being used.
	apply_overlay(X_SUIT_LAYER)
	spawn(30)
		remove_overlay(X_SUIT_LAYER)

/mob/living/carbon/Xenomorph/proc/create_s_shriekwave_l()
	overlays_standing[X_SUIT_LAYER] = image("icon"='icons/Xeno/2x2_Xenos.dmi', "icon_state" = "s_shriek_waves_l") //Ehh, suit layer's not being used.
	apply_overlay(X_SUIT_LAYER)
	spawn(30)
		remove_overlay(X_SUIT_LAYER)
...
/mob/living/carbon/Xenomorph/proc/update_wounds()
	...
	if(health < maxHealth * 0.5) //Injuries appear at less than 50% health
		...
		if(lying)
			if((resting || sleeping) && (!knocked_down && !knocked_out && health > 0))
				I = image("icon"='icons/Xeno/2x2_Xenos.dmi', "icon_state"="overlay", "layer"=-X_WOUND_LAYER)
			else
				I = image("icon"='icons/Xeno/2x2_Xenos.dmi', "icon_state"="overlay", "layer"=-X_WOUND_LAYER)
		else
			I = image("icon"='icons/Xeno/2x2_Xenos.dmi', "icon_state"="overlay", "layer"=-X_WOUND_LAYER)
