	...
	switch(stage)
		...
		if(3)
			if(prob(2))
				...
				to_chat(affected_mob, "<span class='warning'>ћышцы болят.</span>")
				if(prob(20))
		...
		if(4)
			if(prob(1))
				if(affected_mob.knocked_out < 1)
					affected_mob.visible_message("<span class='danger'>\The [affected_mob] начинает неудержимо трястись!</span>", \
												 "<span class='danger'>Ќачинает неудержимо трястись!</span>")
...
/obj/item/alien_embryo/proc/become_larva()
	...
	if(picked)
		...
		to_chat(new_xeno, "<span class='xenoannounce'>“ы личинка ксеноморфа внутри хозяина! ƒвигайтесь, чтобы вырваться из него!</span>")
...
/mob/living/carbon/Xenomorph/Larva/proc/chest_burst(mob/living/carbon/victim)
	...
	if(victim.knocked_out < 1)
		victim.KnockOut(20)
	victim.visible_message("<span class='danger'>\The [victim] начинает неудержимо трястись!</span>", \
								 "<span class='danger'>“ы чувствуешь, как что-то разрывает тебя изнутри!</span>")
	...
	if((!key || !client) && loc.z == 1 && (locate(/obj/structure/bed/nest) in loc) && hivenumber == XENO_HIVE_NORMAL && hive.living_xeno_queen && hive.living_xeno_queen.z == src.loc.z)
		visible_message("<span class='xenodanger'>[src] зарываетс€ в резину.</span>")
