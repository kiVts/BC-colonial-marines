...
/mob/living/carbon/Xenomorph/Boiler/proc/bomb_turf(var/turf/T)
	if(!istype(T) || T.z != src.z || T == get_turf(src))
		src << "<span class='warning'>Это недопустимая цель.</span>"
		return
	...
	if(!is_bombarding)
		src << "<span class='warning'>Вы должны окопаться, прежде чем сможете это сделать.</span>"
		return

	if(bomb_cooldown)
		src << "<span class='warning'>Ты все еще готовишь еще один плевок. Будьте терпеливы!</span>"
		return

	if(get_dist(T, U) <= 5) //Magic number
		src << "<span class='warning'>Ты слишком близко! Вы должны быть на расстояние не менее 7 метров от цели из-за дуги траектории.</span>"
		return
	...
	src << "<span class='xenonotice'>Вы начинаете накапливать кислоту.</span>"
	if(client)
		client.mouse_pointer_icon = initial(client.mouse_pointer_icon) //Reset the mouse pointer.
	bomb_cooldown = 1
	is_bombarding = 0
	...
	if(do_after(src, 50, FALSE, 5, BUSY_ICON_HOSTILE))
		if(!check_state())
			...
			return
		bomb_turf = null
		visible_message("<span class='xenowarning'>\The [src] запускает огромный шар кислоты, плюя в даль!</span>", \
		"<span class='xenowarning'>Вы запускаете огромный шар кислоты бросая вдаль!</span>", null, 5)
		...
		spawn(bomb_delay) //20 seconds cooldown.
			bomb_cooldown = 0
			src << "<span class='notice'>Вы чувствуете, как набухают ваши ядовитые железы. Вы можете снова плюнуть огромным шаром кислоты.</span>"
		...
		bomb_cooldown = 0
		src << "<span class='warning'>Вы решили не запускать кислоту.</span>"
	return
...
/mob/living/carbon/Xenomorph/Boiler/proc/acid_spray(atom/T)
	...
	if(!isturf(loc) || istype(loc, /turf/open/space))
		src << "<span class='warning'>Ты не можешь сделать это оттуда.</span>"
		return
	...
	if(T)
		...
		if(target == loc)
			src << "<span class='warning'>Это слишком близко!</span>"
			return
		...
		visible_message("<span class='xenowarning'>\The [src] извергает ядовитую струю кислоты!</span>", \
		...
		spawn(acid_delay) //12 second cooldown.
			acid_cooldown = 0
			src << "<span class='warning'>Вы чувствуете, как ваши кислотные железы наполняются. Вы можете распылить <B>acid</b> снова.</span>"
			for(var/X in actions)
				var/datum/action/A = X
				A.update_button_icon()
	else
		src << "<span class='warning'>Вы не видите ничего, плюнуть на!</span>"
	...
	if(!locate(/obj/effect/xenomorph/spray) in target) //No stacking flames!
		...
		for(var/mob/living/carbon/M in target)
			if(ishuman(M) || ismonkey(M))
				...
				M << "<span class='xenodanger'>\The [src] осыпает тебя едкой кислотой!</span>"
				if(!isYautja(M))
					M.emote("scream")
					M.KnockDown(rand(3, 4))

/mob/living/carbon/Xenomorph/Boiler/Topic(href, href_list)
...
/mob/living/carbon/Xenomorph/Boiler/proc/set_xeno_overwatch(mob/living/carbon/Xenomorph/target, stop_overwatch)
	if(stop_overwatch)
		observed_xeno = null
	else
