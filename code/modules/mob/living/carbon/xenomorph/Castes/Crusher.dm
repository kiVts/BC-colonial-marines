/mob/living/carbon/Xenomorph/Crusher
	caste = "Crusher"
	name = "Crusher"
	desc = "Огромный инопланетянин с огромным бронированным гребнем на голове."
	...
	caste_desc = "Огромный танковый ксеноморф."
	speed = 0.1
...
/mob/living/carbon/Xenomorph/Crusher/proc/stomp()
	...
	if(world.time < has_screeched + CRUSHER_STOMP_COOLDOWN) //Sure, let's use this.
		to_chat(src, "<span class='xenowarning'>Вы не готовы снова топать.</span>")
		return FALSE

	if(legcuffed)
		to_chat(src, "<span class='xenodanger'>Ты не можешь встать на дыбы с этой штукой на ноге!</span>")
		return
	...
	visible_message("<span class='xenodanger'>[src] врезается в землю!</span>", \
	"<span class='xenodanger'>Ты врезаешься в землю!</span>")
	...
	for(var/mob/living/M in range(1,loc))
		...
		if(!isXeno(M))
			if(M.loc == loc)
				...
				to_chat(M, "<span class='highdanger'>Тебя топчут [src]!</span>")
			shake_camera(M, 2, 2)
		i--
...
/obj/machinery/vending/charge_act(mob/living/carbon/Xenomorph/X)
	if(X.charge_speed > X.charge_speed_max/2) //Halfway to full speed or more
		if(unacidable)
			...
			return FALSE
		X.visible_message("<span class='danger'>[X] врезается прямо в [src]!</span>",
		"<span class='xenodanger'>Ты врезаешься прямо в [src]!</span>")
		playsound(loc, "punch", 25, 1)
		tip_over()
...
/mob/living/carbon/Xenomorph/Crusher/Topic(href, href_list)
...
/mob/living/carbon/Xenomorph/Crusher/proc/set_xeno_overwatch(mob/living/carbon/Xenomorph/target, stop_overwatch)
	if(stop_overwatch)
		observed_xeno = null
	else
