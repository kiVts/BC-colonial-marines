/mob/living/carbon/Xenomorph/say(var/message)
	...
	if(client)
		if(client.prefs.muted & MUTE_IC)
			to_chat(src, "<span class='warning'>Вы не можете говорить в IC (заглушён).</span>")
			return
	...
	for (var/mob/S in player_list)
		if(!isnull(S) && (isXeno(S) || S.stat == DEAD) && !istype(S,/mob/new_player))
			if(istype(S,/mob/dead/observer))
				...
				if(hive.disrupted)
					to_chat(S, "<span class='xenodanger'>Вы можете почувствовать, что тревожный шум где-то [A? " from [sanitize(A.name)]":""]..</span>")
