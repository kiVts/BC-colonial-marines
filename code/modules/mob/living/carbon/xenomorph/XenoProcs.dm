...
/mob/living/carbon/Xenomorph/Stat()
	...
	if(hivenumber && hivenumber <= hive_datum.len)
		...
		if(upgrade != -1 && upgrade != 4) //upgrade possible
...
/mob/living/carbon/Xenomorph/proc/check_state()
	if(is_mob_incapacitated() || lying || buckled)
		to_chat(src, "<span class='warning'>¬ы не можете сделать это в своем нынешнем состоянии.</span>")
		return 0
	return 1
...
/mob/living/carbon/Xenomorph/proc/check_plasma(value)
	...
	if(value)
		if(plasma_stored < value)
			if(is_robotic)
				to_chat(src, "<span class='warning'>Ѕиип. ” вас недостаточно плазмы для этого. ¬ам требуется [value] пласма, но хранится только [plasma_stored].</span>")
			else
				to_chat(src, "<span class='warning'>” вас недостаточно плазмы для этого. ¬ам требуется плазма [value], но хранится только [plasma_stored].</span>")
			return 0
	return 1
...
/mob/living/carbon/Xenomorph/movement_delay()
	...
	if(is_charging)
		if(legcuffed)
			is_charging = 0
			stop_momentum()
			to_chat(src, "<span class='xenodanger'>“ы не можешь атаковать с этой штукой на ноге!</span>")
		else
			. -= charge_speed
			charge_timer = 2
			if(charge_speed == 0)
		...
		switch(charge_type) //Determine how to handle it depending on charge type.
			...
			if(3 to 4)
				if(istype(O, /obj/structure/table) || istype(O, /obj/structure/rack))
					...
					visible_message("<span class='danger'>[src] пашет насквозь [S]!</span>", null, null, 5)
					...
					if(ishuman(M) && M.dir in reverse_nearby_direction(dir))
						...
						if(isYautja(H))
							if(H.check_shields(0, "the pounce", 1))
								visible_message("<span class='danger'>[H] блокирует набег [src] с комбостик!</span>",
												"<span class='xenodanger'>[H] блокирует ваш набег с комбостик</span>", null, 5)
								KnockDown(5)
								throwing = FALSE
								return FALSE
							else if(prob(75)) //Body slam the fuck out of xenos jumping at your front.
								visible_message("<span class='danger'>[H] тело хлопает [src]!</span>",
												"<span class='xenodanger'>[H] тело бьет тебя.!</span>", null, 5)
								KnockDown(4)
								throwing = FALSE
								return FALSE

					visible_message("<span class='danger'>[src] набрасывается на [M]!</span>",
									"<span class='xenodanger'>“ы набрасываешься [M]!</span>", null, 5)
...
/mob/living/carbon/Xenomorph/proc/bite_attack(var/mob/living/carbon/human/M, var/damage)
	...
	visible_message("<span class='danger'>\The [M] злобно измельчается острыми зубами \the [src]'s !</span>", \
	"<span class='danger'>¬ы злобно рвЄте \the [M] зубами!</span>", null, 5)
...
/mob/living/carbon/Xenomorph/proc/tail_attack(mob/living/carbon/human/M, var/damage)
	...
	visible_message("<span class='danger'>\The [M] внезапно пронзает \the [src]'s острым хвостом!</span>", \
	"<span class='danger'>“ы яростно пронзаешь хвостом \the [M]!</span>", null, 5)
...
/mob/living/carbon/Xenomorph/proc/check_alien_construction(var/turf/current_turf)
	...
	for(var/obj/O in current_turf)
		if(istype(O, /obj/item/clothing/mask/facehugger))
			to_chat(src, "<span class='warning'>«десь уже есть маленький. Ћучше шевелись.</span>")
			return
		if(istype(O, /obj/effect/alien/egg))
			to_chat(src, "<span class='warning'>яйцо уже есть.</span>")
			return
	...
	if(current_turf.density || has_obstacle)
		to_chat(src, "<span class='warning'>«десь уже что-то построено.</span>")
		return
...
/mob/living/carbon/Xenomorph/drop_held_item()
	...
	if(istype(F))
		if(locate(/turf/closed/wall/resin) in loc)
			to_chat(src, "<span class='warning'>¬ конце концов, вы решаете не отбрасывать [F].</span>")
			return
...
/mob/living/carbon/Xenomorph/proc/handle_xeno_leader_pheromones(var/mob/living/carbon/Xenomorph/Queen/Q)
	...
	if(!Q || !Q.anchored || !queen_chosen_lead || !Q.current_aura || Q.loc.z != loc.z) //We are no longer a leader, or the Queen attached to us has dropped from her ovi, disabled her pheromones or even died
		...
		leader_current_aura = ""
		to_chat(src, "<span class='xenowarning'>“вои феромоны слабеют.  оролева больше не дает тебе свои феромоны.</span>")
	else
