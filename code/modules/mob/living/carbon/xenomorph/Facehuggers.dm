...
/obj/item/clothing/mask/facehugger
	name = "alien"
	desc = "Ќа конце хвоста у него что-то вроде трубки."
...
/obj/item/clothing/mask/facehugger/dropped()
	...
	for(F in get_turf(src))
		...
		if(count > 2) //Was 5, our rules got much tighter
			visible_message("<span class='xenowarning'>¬ лицехват яростно поглощены соседнии ќрды чужих!</span>")
			cdel(src)
			return
...
/obj/item/clothing/mask/facehugger/attack(mob/M, mob/user)
	if(CanHug(M))
		...
		to_chat(user, "<span class='warning'>Ћицехват отказывается прикрепляться.</span>")
		..()
...
/obj/item/clothing/mask/facehugger/examine(mob/user)
	...
	switch(stat)
		if(DEAD, UNCONSCIOUS)
			to_chat(user, "<span class='danger'>[src] не двигается.</span>")
		if(CONSCIOUS)
			to_chat(user, "<span class='danger'>[src] кажется активен.</span>")
	if(sterile)
		to_chat(user, "<span class='danger'>ѕохоже, хоботок удалили.</span>")
	...
	if(isturf(loc))
		...
		for(M in view(4,src))
			if(!i)
				break
			if(CanHug(M))
				M.visible_message("<span class='warning'>\The Ѕегущий [src] прыгает на [M]!</span>", \
				"<span class='warning'>Ѕегущий [src] прыгает на [M]!</span>")
				leaping = TRUE
				throw_at(M, 4, 1)
				break
			i--
...
/obj/item/clothing/mask/facehugger/proc/Attach(mob/living/M)
	...
	M.visible_message("<span class='danger'>[src] прыгает на [M]'s лицо!</span>")
	if(throwing)
		throwing = FALSE
	...
	if(ishuman(M))
		...
		if(!H.has_limb("head"))
			visible_message("<span class='warning'>[src] ищет лицо, чтобы обнять [H], но не находит!</span>")
			GoIdle()
			return
		...
		if(isYautja(M))
			...
			if(!H.stat && H.dir != dir && prob(catch_chance)) //Not facing away
				H.visible_message("<span class='notice'>[H] выхватывает [src] из воздуха и раздавливает его!")
				Die()
				loc = H.loc
				return
		...
		if(H.head && !(H.head.flags_item & NODROP))
			...
			if(istype(D))
				if(D.anti_hug > 1)
					H.visible_message("<span class='danger'>[src] разбивается о [H]'s [D.name]!")
					cannot_infect = 1
				else
					H.visible_message("<span class='danger'>[src] разбивается о [H]'s [D.name] и срывает его!")
	...
	if(iscarbon(M))
		...
		if(target.wear_mask)
			...
			if(istype(W))
				...
				if(W.anti_hug > 1)
					target.visible_message("<span class='danger'>[src] разбивается о [target]'s [W.name]!</span>")
					cannot_infect = 1
				else
					target.visible_message("<span class='danger'>[src] разбивается о [target]'s [W.name] и срывает его!</span>")
...
/obj/item/clothing/mask/facehugger/proc/Impregnate(mob/living/carbon/target)
	...
	if(!sterile)
		...
		target.visible_message("<span class='danger'>[src] падает обмякнув после нарушения [target]'s лицо!</span>")
		Die()

	else
		target.visible_message("<span class='danger'>[src] нарушать [target]'s лицо!</span>")
...
/obj/item/clothing/mask/facehugger/proc/check_lifecycle()
	if(lifecycle - 50 <= 0)
		if(isturf(loc))
			...
			if(E && E.status == BURST)
				visible_message("<span class='xenowarning'>[src] вползает обратно в [E]!</span>")
				E.status = GROWN
				E.icon_state = "Egg"
			...
			if(T && !T.hugger)
				visible_message("<span class='xenowarning'>[src] вползает в [T]!</span>")
				T.hugger = TRUE
				T.icon_state = "trap1"
				cdel(src)
				return
...
/obj/item/clothing/mask/facehugger/proc/Die()
	...
	visible_message("\icon[src] <span class='danger'>\The [src] свернуться в клубок!</span>")
	...
	visible_message("\icon[src] <span class='danger'>\The [src] распадается на массу кислоты и хитина.</span>")
	cdel(src)
