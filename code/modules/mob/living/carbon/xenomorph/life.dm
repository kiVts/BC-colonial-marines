			...
			if(hivenumber && hivenumber <= hive_datum.len)
				...
				if(evolution_allowed && evolution_stored < evolution_threshold && hive.living_xeno_queen && hive.living_xeno_queen.ovipositor)
					...
					if(evolution_stored == evolution_threshold - 1)
						to_chat(src, "<span class='xenodanger'>“вой ѕанцирь потрескивает, а сухожилия укрепляются. ¬ы готовы развиваться!</span>")
		...
		if(stomach_contents.len)
			for(var/atom/movable/M in stomach_contents)
				if(world.time > devour_timer && ishuman(M))
					...
					if(limb_name)
						to_chat(H, "<span class='danger'>¬аш [limb_name] растворяется в кислоте!</span>")
					if(prob(50))
				...
				if(M.acid_damage > 300)
					to_chat(src, "<span class='xenodanger'>\The [M] растворяется в кишечнике с бульканьем.</span>")
					stomach_contents.Remove(M)
					cdel(M)
	return 1
...
/mob/living/carbon/Xenomorph/proc/handle_environment()
	...
	if(!fire_immune)
		if(env_temperature > (T0C + 66))
			...
			if(prob(20))
				to_chat(src, "<span class='warning'>¬ы чувствуете обжигающий жар!</span>")
		else
			if(hud_used && hud_used.fire_icon)
	...
	if(!is_robotic && !hardcore) //Robot no heal
		...
		if(isXenoHivelord(src))
			...
			if(H.speed_activated)
				...
				if(plasma_stored < 0)
					H.speed_activated = 0
					to_chat(src, "<span class='warning'>¬ы чувствуете головокружение, когда мир замедляется.</span>")
	...
	if(plasma_stored < 0)
		...
		if(current_aura)
			current_aura = null
			to_chat(src, "<span class='warning'>” вас закончились феромоны, и вы перестали излучать феромоны.</span>")
...
/mob/living/carbon/Xenomorph/proc/handle_stagger()
	if(stagger)
		#if DEBUG_XENO_LIFE
		world << "<span class='debuginfo'>–еген: начальная в шахматном порядке: <b>[stagger]</b></span>"
		#endif
		adjust_stagger(-1)
		#if DEBUG_XENO_LIFE
		world << "<span class='debuginfo'>–еген: начальная в шахматном порядке: <b>[stagger]</b></span>"
...
/mob/living/carbon/Xenomorph/proc/handle_slowdown()
	if(slowdown)
		#if DEBUG_XENO_LIFE
		world << "<span class='debuginfo'>Regen: первоначальное замедление: <b>[slowdown]</b></span>"
		#endif
