		...
		switch(caste)
			if("Queen")
				...
				if(hive.living_xeno_queen == src)
					xeno_message("<span class='xenoannounce'>Внезапно по улью пробегает дрожь... Королева убита! Месть!</span>",3, hivenumber)
					xeno_message("<span class='xenoannounce'>Теперь разрешено рубить хостов.</span>",2)
...
/mob/living/carbon/Xenomorph/gib()
	...
	switch(caste) //This will need to be changed later, when we have proper xeno pathing. Might do it on caste or something.
		if("Boiler")
			...
			visible_message("<span class='danger'>[src] начинает гротескно выпячиваться и взрывается в облаке едкого газа!</span>")
