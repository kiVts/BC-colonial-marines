...
/mob/verb/debug_xeno_mind()
	...
	if(!ticker || ticker.current_state != GAME_STATE_PLAYING || !ticker.mode)
		to_chat(src, "<span class='warning'>–аунд либо не запущен, либо уже закончился.</span>")
		return
	if(mind in ticker.mode.xenomorphs)
		to_chat(src, "<span class='debuginfo'>[src] разум в списке ксеноморфов.  люч разума [mind.key].</span>")
		to_chat(src, "<span class='debuginfo'>“екущая толпа: [mind.current]. ќригинальный моб is: [mind.original].</span>")
	else
		to_chat(src, "<span class='debuginfo'>Ётого ксеноморфа нет в списке ксеноморфов.</span>")
#endif
...
/mob/living/carbon/Xenomorph/proc/generate_name()
	...
	if(caste == "Queen")
		switch(upgrade)
			...
			if(3) name = "[name_prefix]Queen-Mother" //Ancient
			if(4) name = "[name_prefix]Queen-Mother TITAN" //TITAN
...
/mob/living/carbon/Xenomorph/examine(mob/user)
	...
	if(stat == DEAD)
		to_chat(user, "ќн мертв.")
	else if(stat == UNCONSCIOUS)
		to_chat(user, "ќна слегка дрожит, но едва шевелится.")
	else
		var/percent = (health / maxHealth * 100)
		switch(percent)
			if(95 to 101)
				to_chat(user, "¬ыглядит вполне здоровым.")
			if(75 to 94)
				to_chat(user, "ќн слегка ранен.")
			if(50 to 74)
				to_chat(user, "ѕохоже, он ранен.")
			if(25 to 49)
				to_chat(user, "ќн истекает кровью с шипящими ранами.")
			if(1 to 24)
				to_chat(user, "ќн тяжело ранен и сильно хромает.")

	if(hivenumber != XENO_HIVE_NORMAL)
		if(hivenumber && hivenumber <= hive_datum.len)
			var/datum/hive_status/hive = hive_datum[hivenumber]
			to_chat(user, "This is [hive.prefix].")
	return
...
/mob/living/carbon/Xenomorph/start_pulling(atom/movable/AM, lunge, no_msg)
	...
	if(!isXenoDrone(usr) && !isXenoHivelord(usr) && !isXenoQueen(usr))
...
/mob/living/carbon/Xenomorph/pull_response(mob/puller)
	if(stat != DEAD && has_species(puller,"Human")) // If the Xeno is alive, fight back against a grab/pull
		...
		puller.visible_message("<span class='warning'>[puller] пыталась вытащить [src] но вместо этого получает удар хвостом по голове!</span>")
		puller.stop_pulling()
		return FALSE
	return TRUE
...
/mob/living/carbon/Xenomorph/resist_grab(moving_resist)
	if(pulledby.grab_level)
		visible_message("<span class='danger'>[src] вырвался из захвата [pulledby]'s !</span>", null, null, 5)
