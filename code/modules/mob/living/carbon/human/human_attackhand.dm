/mob/living/carbon/human/attack_hand(mob/living/carbon/human/M)
	...
	if((M != src) && check_shields(0, M.name))
		visible_message("<span style='color:#FF0000'><B>[M] attempted to touch [src]!</B></span>", null, null, 5)
		return 0
	...
	if(M.gloves && istype(M.gloves, /obj/item/clothing/gloves/boxing/hologlove))
		...
		if(!damage)
			...
			visible_message("<span style='color:#FF0000'><B>[M] has attempted to punch [src]!</B></span>")
			return 0
		...
		visible_message("<span style='color:#FF0000'><B>[M] has punched [src]!</B></span>")

		apply_damage(damage, HALLOSS, affecting, armor_block)
		if(damage >= 9)
			visible_message("<span style='color:#FF0000'><B>[M] has weakened [src]!</B></span>")
...
//			if(M.health < -75)	return 0
			...
			if((M.head && (M.head.flags_inventory & COVERMOUTH)) || (M.wear_mask && (M.wear_mask.flags_inventory & COVERMOUTH)))
				to_chat(M, "<span style='color:#0000FF'><B>Remove your mask!</B></span>")
				return 0
			if((head && (head.flags_inventory & COVERMOUTH)) || (wear_mask && (wear_mask.flags_inventory & COVERMOUTH)))
				to_chat(M, "<span style='color:#0000FF'><B>Remove his mask!</B></span>")
				return 0

			//CPR
			if(M.action_busy)
				return 1
			M.visible_message("<span style='color:#FF0000'><B>[M] is trying perform CPR on [src]!</B></span>", null, null, 4)
			...
			if(do_mob(M, src, HUMAN_STRIP_DELAY, BUSY_ICON_GENERIC, BUSY_ICON_MEDICAL))
				if(health > config.health_threshold_dead && health < config.health_threshold_crit)
					...
					adjustOxyLoss(-suff)
					updatehealth()
					visible_message("<span style='color:#FF0000'>[M] performs CPR on [src]!</span>", null, null, 3)
					to_chat(src, "<span style='color:#0000FF'><b>You feel a breath of fresh air enter your lungs. It feels good.</b></span>")
					to_chat(M, "<span style='color:#FF0000'>Repeat at least every 7 seconds.</span>")
		...
		if("disarm")
			...
			if (randn <= 25)
				...
				visible_message("<span style='color:#FF0000'><B>[M] has pushed [src]!</B></span>", null, null, 5)
				return

			if(randn <= 60)
				//BubbleWrap: Disarming breaks a pull
				if(pulling)
					visible_message("<span style='color:#FF0000'><b>[M] has broken [src]'s grip on [pulling]!</B></span>", null, null, 5)
					stop_pulling()
				else
					drop_held_item()
					visible_message("<span style='color:#FF0000'><B>[M] has disarmed [src]!</B></span>", null, null, 5)
				playsound(loc, 'sound/weapons/thudswoosh.ogg', 25, 1, 7)
				return


			playsound(loc, 'sound/weapons/punchmiss.ogg', 25, 1, 7)
			visible_message("<span style='color:#FF0000'><B>[M] attempted to disarm [src]!</B></span>", null, null, 5)
	return
	...
	if (health >= config.health_threshold_crit)
		if(src == M)
			...
			for(var/datum/limb/org in limbs)
				...
				to_chat(src, "\t [status=="OK"?"<span style='color:#0000FF'>":"<span style='color:#FF0000'>"]My [org.display_name] is [status].</span></span>")
