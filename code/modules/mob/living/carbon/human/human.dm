...
/mob/living/carbon/human/Topic(href, href_list)
	...
	if(href_list["internal"])
		...
		if(!usr.action_busy)
			...
			if(internal)
				usr.visible_message("<span style='color:#FF0000'><B>[usr] is trying to disable [src]'s internals</B></span>", null, null, 3)
			else
				usr.visible_message("<span style='color:#FF0000'><B>[usr] is trying to enable [src]'s internals.</B></span>", null, null, 3)
			...
			if(w_uniform && istype(w_uniform, /obj/item/clothing/under))
				...
				if(U.hastie)
					...
					if(istype(U.hastie, /obj/item/clothing/tie/holobadge) || istype(U.hastie, /obj/item/clothing/tie/medal))
						visible_message("<span style='color:#FF0000'><B>[usr] tears off \the [U.hastie] from [src]'s [U]!</B></span>", null, null, 5)
					else
						visible_message("<span style='color:#FF0000'><B>[usr] is trying to take off \a [U.hastie] from [src]'s [U]!</B></span>", null, null, 5)
						if(do_mob(usr, src, HUMAN_STRIP_DELAY, BUSY_ICON_GENERIC, BUSY_ICON_GENERIC))
	...
	if(href_list["sensor"])
		if(!usr.action_busy)
			...
			if(U.has_sensor >= 2)
				...
				visible_message("<span style='color:#FF0000'><B>[usr] is trying to modify [src]'s sensors!</B></span>", null, null, 4)
				if(do_mob(usr, src, HUMAN_STRIP_DELAY, BUSY_ICON_GENERIC, BUSY_ICON_GENERIC))
					if(U == w_uniform)
						if(U.has_sensor >= 2)
							to_chat(usr, "The controls are locked.")
	...
	if (href_list["criminal"])
		if(hasHUD(usr,"security"))
			...
			if(!modified)
				to_chat(usr, "<span style='color:#FF0000'>Unable to locate a data core entry for this person.</span>")
...
/mob/living/carbon/human/proc/play_xylophone()
	if(!src.xylophone)
		visible_message("<span style='color:#FF0000'>[src] begins playing his ribcage like a xylophone. It's quite spooky.","<span style='color:#0000FF'>You begin to play a spooky refrain on your ribcage.","\red You hear a spooky xylophone melody.</span></span>")
...
/mob/living/carbon/human/proc/morph()
	...
	visible_message("<span style='color:#0000FF'>\The [src] morphs and changes [get_visible_gender() == MALE ? "his" : get_visible_gender() == FEMALE ? "her" : "their"] appearance!", "\blue You change your appearance!", "<span style='color:#FF0000'>Oh, god!  What the hell was that?  It sounded like flesh getting squished and bone ground into a different shape!</span></span>")
...
/mob/living/carbon/human/proc/remotesay()
	...
	if(mRemotetalk in target.mutations)
		target.show_message("<span style='color:#0000FF'>You hear [src.real_name]'s voice: [say]</span>")
	else
		target.show_message("<span style='color:#0000FF'>You hear a voice that seems to echo around the room: [say]</span>")
	usr.show_message("<span style='color:#0000FF'>You project your mind into [target.real_name]: [say]</span>")
...
/mob/living/carbon/human/verb/check_pulse()
	...
	if(usr == src)
		self = 1
	if(!self)
		usr.visible_message("<span style='color:#0000FF'>[usr] kneels down, puts \his hand on [src]'s wrist and begins counting their pulse.</span>",\
		"You begin counting [src]'s pulse", null, 3)
	else
		usr.visible_message("<span style='color:#0000FF'>[usr] begins counting their pulse.</span>",\
		"You begin counting your pulse.", null, 3)

	if(src.pulse)
		to_chat(usr, "<span style='color:#0000FF'>[self ? "You have a" : "[src] has a"] pulse! Counting...</span>")
	else
		to_chat(usr, "<span style='color:#FF0000'>[src] has no pulse!</span>"	)
		return
	...
	if(usr.l_move_time >= time)	//checks if our mob has moved during the sleep()
		...
		to_chat(usr, "<span style='color:#0000FF'>[self ? "Your" : "[src]'s"] pulse is [src.get_pulse(GETPULSE_HAND)].</span>")
