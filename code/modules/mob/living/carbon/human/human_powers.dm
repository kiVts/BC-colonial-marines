...
/mob/living/carbon/human/proc/tackle()
	...
	for(var/mob/O in viewers(src, null))
		if ((O.client && !( O.blinded )))
			O.show_message(text("<span style='color:#FF0000'><B>[] [failed ? "tried to tackle" : "has tackled"] down []!</B></span>", src, T), 1)
...
/mob/living/carbon/human/proc/leap()
	...
	if(!src.Adjacent(T))
		to_chat(src, "<span style='color:#FF0000'>You miss!</span>")
		return
...
/mob/living/carbon/human/proc/gut()
	...
	if(is_mob_incapacitated(TRUE) || lying)
		to_chat(src, "<span style='color:#FF0000'>You cannot do that in your current state.</span>")
		return

	var/obj/item/grab/G = locate() in src
	if(!G || !istype(G))
		to_chat(src, "<span style='color:#FF0000'>You are not grabbing anyone.</span>")
		return

	if(usr.grab_level < GRAB_AGGRESSIVE)
		to_chat(src, "<span style='color:#FF0000'>You must have an aggressive grab to gut your prey!</span>")
		return
...
/mob/living/carbon/human/proc/commune()
	...
	to_chat(M, "<span style='color:#0000FF'>Like lead slabs crashing into the ocean, alien thoughts drop into your mind: [text]</span>")
	if(istype(M,/mob/living/carbon/human))
		var/mob/living/carbon/human/H = M
		if(H.species.name == src.species.name)
			return
		to_chat(H, "<span style='color:#FF0000'>Your nose begins to bleed...</span>")
		H.drip(1)
