...
/mob/living/carbon/monkey/Topic(href, href_list)
	...
	if(href_list["internal"])
		...
		if(!usr.action_busy)
			...
			if(internal)
				usr.visible_message("<span style='color:#FF0000'><B>[usr] is trying to disable [src]'s internals</B></span>", null, 4)
			else
				usr.visible_message("<span style='color:#FF0000'><B>[usr] is trying to enable [src]'s internals.</B></span>", null, 4)
		...
		if(G.cell)
			if(M.a_intent == "hurt")//Stungloves. Any contact will stun the alien.
				if(G.cell.charge >= 2500)
					...
					for(var/mob/O in viewers(src, null))
						if (O.client)
							O.show_message("<span style='color:#FF0000'><B>[src] has been touched with the stun gloves by [M]!</B>", 1, "\red You hear someone fall</span>", 2)
					return
				else
					to_chat(M, "<span style='color:#FF0000'>Not enough charge! </span>")
					return
		...
		if (M.a_intent == "hurt")
			...
			if ((prob(75) && health > 0))
				...
				if (prob(40))
					...
					if (knocked_out < 5)
						KnockOut(rand(10, 15))
						visible_message("<span style='color:#FF0000'><B>[M] has knocked out [src]!</B></span>")
			...
			if (M.a_intent == "grab")
				...
				if (!( knocked_out ))
					if (prob(25))
						...
						for(var/mob/O in viewers(src, null))
							if ((O.client && !( O.blinded )))
								O.show_message(text("<span style='color:#FF0000'><B>[] has disarmed [name]!</B></span>", M), 1)
	return
