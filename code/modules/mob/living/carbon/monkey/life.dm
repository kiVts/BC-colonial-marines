...
/mob/living/carbon/monkey
	...
	proc/handle_disabilities()
		...
		if (disabilities & EPILEPSY)
			if ((prob(1) && knocked_out < 10))
				to_chat(src, "<span style='color:#FF0000'>You have a seizure!</span>")
				KnockOut(10)
	...
	proc/handle_mutations_and_radiation()
		...
		if ((HULK in mutations) && health <= 25)
			...
			to_chat(src, "<span style='color:#FF0000'>You suddenly feel very weak.</span>")
			KnockDown(3)
			emote("collapse")
		...
		if (radiation)
			...
			switch(radiation)
				...
				if(75 to 100)
					...
					adjustToxLoss(3)
					if(prob(1))
						to_chat(src, "<span style='color:#FF0000'>You mutate!</span>")
						randmutb(src)
						domutcheck(src,null)
						emote("gasp")
	...
	proc/handle_breath(list/air_info)
		...
		if(air_info[2] > (T0C+66)) // Hot air hurts :(
			if(prob(20))
				to_chat(src, "<span style='color:#FF0000'>You feel a searing heat in your lungs!</span>")
