...
/mob/living/carbon/hellhound/New()
	...
	for(var/mob/dead/observer/M in player_list)
		to_chat(M, "<span style='color:#FF0000'><B>A hellhound is now available to play!</b> Please be sure you can follow the rules.</span>")
		to_chat(M, "<span style='color:#FF0000'>Click 'Join as hellhound' in the ghost panel to become one. First come first serve!</span>")
		to_chat(M, "<span style='color:#FF0000'>If you need help during play, click adminhelp and ask.</span>")
...
/mob/living/carbon/hellhound/proc/bite_human(var/mob/living/carbon/human/H)
	...
	if(a_intent == "help")
			...
			if (!(H.knocked_out ))
				...
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message(text("<span style='color:#FF0000'><B>[] knocks down [H]!</B></span>", src), 1)
		return
	else if(a_intent == "grab")
		playsound(loc, 'sound/weapons/thudswoosh.ogg', 25, 1)
		for(var/mob/O in viewers(src, null))
			O.show_message(text("<span style='color:#FF0000'>[] has grabbed [H] in their jaws!</span>", src), 1)
		src.start_pulling(H)
	else
		if(isYautja(H))
		...
		visible_message("<span style='color:#FF0000'><B>[src] mauls [H]!</b>","\red <B>You maul [H]!</b></span>")
	...
	if(a_intent == "help")
		...
		if (!(X.knocked_out ) && X.mob_size != MOB_SIZE_BIG)
			if(prob(40))
				X.KnockOut(4)
				playsound(loc, 'sound/weapons/thudswoosh.ogg', 25, 1)
				visible_message("<span style='color:#FF0000'>[src] knocks down [X]!","\red You knock down [X]!</span>")
				return
		visible_message("<span style='color:#FF0000'>[src] shoves at [X]!","\red You shove at [X]!</span>")
		return
	else if(a_intent == "grab")
		playsound(loc, 'sound/weapons/thudswoosh.ogg', 25, 1)
		visible_message("<span style='color:#FF0000'><B>[src] grabs [X] in their jaws!</B>","\red <B>You grab [X] in your jaws!</b></span>")
		src.start_pulling(X)
	else
		var/dmg = rand(20,32)
		X.apply_damage(dmg,BRUTE,edge = 1) //Does NOT check armor.
		visible_message("<span style='color:#FF0000'><B>[src] mauls [X]!</b>","\red <B>You maul [X]!</b></span>")
		playsound(loc, 'sound/weapons/bite.ogg', 25, 1)
	return
	...
	if(a_intent == "help")
		...
		if(istype(H,/mob/living/carbon/monkey))
			if (!(H.knocked_out ))
				H.KnockOut(8)
				playsound(loc, 'sound/weapons/thudswoosh.ogg', 25, 1)
				visible_message("<span style='color:#FF0000'>[src] knocks down [H]!","\red You knock down [H]!</span>")
				return
		visible_message("<span style='color:#FF0000'>[src] shoves at [H]!","\red You shove at [H]!</span>")
		return
	else if(a_intent == "grab")
		playsound(loc, 'sound/weapons/thudswoosh.ogg', 25, 1)
		visible_message("<span style='color:#FF0000'><B>[src] grabs [H] in their jaws!</B>","\red <B>You grab [H] in your jaws!</b></span>")
		src.start_pulling(H)
		return
	else
	...
	if (M.a_intent == "help")
		...
		if (M.a_intent == "hurt")
			if ((prob(75) && health > 0))
				playsound(loc, 'sound/weapons/bite.ogg', 25, 1)
				for(var/mob/O in viewers(src, null))
					O.show_message("<span style='color:#FF0000'><B>[M.name] has bit [name]!</B></span>", 1)
				var/damage = rand(2, 4)
				adjustBruteLoss(damage)
			else
				for(var/mob/O in viewers(src, null))
					O.show_message("<span style='color:#FF0000'><B>[M.name] has attempted to bite [name]!</B></span>", 1)
	return
...
/mob/living/carbon/hellhound/attack_hand(mob/living/carbon/human/M as mob)
	...
	if (M.a_intent == "help")
		...
		if (M.a_intent == "hurt")
			...
			if ((prob(75) && health > 0))
				visible_message("<span style='color:#FF0000'><B>[M] [pick(attack.attack_verb)]ed [src]!</B></span>")
				...
				visible_message("<span style='color:#FF0000'><B>[M] tried to [pick(attack.attack_verb)] [src]!</B></span>")
		else
			if (M.a_intent == "grab")
				...
				if (!( knocked_out ))
					if (prob(25))
						...
						for(var/mob/O in viewers(src, null))
							if ((O.client && !( O.blinded )))
								O.show_message(text("<span style='color:#FF0000'><B>[] has pushed down [name]!</B></span>", M), 1)
					else
						for(var/mob/O in viewers(src, null))
							if ((O.client && !( O.blinded )))
								O.show_message(text("<span style='color:#FF0000'><B>[] shoves at [name]!</B></span>", M), 1)
	return
...
/mob/living/carbon/hellhound/attack_animal(mob/living/M as mob)
	...
	if(M.melee_damage_upper == 0)
		...
		for(var/mob/O in viewers(src, null))
			O.show_message("<span style='color:#FF0000'><B>[M]</B> [M.attacktext] [src]!</span>", 1)
...
/mob/living/carbon/hellhound/say(var/message)
	if(client)
		if(client.prefs.muted & MUTE_IC)
			to_chat(src, "<span style='color:#FF0000'>You cannot speak in IC (Muted).</span>")
			return
	...
	if(length(message) >= 2 && radio)
		...
		if(radio_prefix == ":" || radio_prefix == ";") //Hellhounds do not actually get to talk on the radios, only listen.
			...
			for(var/mob/living/carbon/hellhound/M in living_mob_list)
				to_chat(M, "<span style='color:#0000FF'><B>\[RADIO\]</b>: [src.name] [verb_used], '<B>[message]<B>'.</span>")
			return
	...
	to_chat(src, "<span style='color:#0000FF'>You say, '<B>[message]</b>'.</span>")
	for(var/mob/living/carbon/hellhound/H in orange(9))
		to_chat(H, "<span style='color:#0000FF'>[src.name] [verb_used], '[message]'.</span>")

	for(var/mob/living/carbon/C in orange(6))
		if(!istype(C,/mob/living/carbon/hellhound))
			to_chat(C, "<span style='color:#0000FF'>[src.name] [verb_used].</span>")
	return
