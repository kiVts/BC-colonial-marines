...
/mob/living/Bump(atom/movable/AM, yes)
	...
	if(isliving(AM))
		...
		if(isXeno(src) && !isXenoLarva(src) && ishuman(L)) //We are a Xenomorph and pushing a human
			...
			if(has_species(L, "Human") && (X.tier == 3 || X.tier == 0))
				var/mob/living/carbon/human/H = L
				var/turf/T = H.loc
				var/facing = pick(turn(src.dir, - 90), turn(src.dir, 90))
				T = get_step(T, facing)
				H.throw_at(T, 1, 1, src, 0)
				H.drop_held_item()
				H.swap_hand()
				H.drop_held_item()
				H.swap_hand()
		...
		if(ishuman(L))
			...
			if(HULK in L.mutations)
				if(prob(70))
					to_chat(usr, "<span style='color:#FF0000'><B>You fail to push [L]'s fat ass out of the way.</B></span>")
					now_pushing = 0
					return
