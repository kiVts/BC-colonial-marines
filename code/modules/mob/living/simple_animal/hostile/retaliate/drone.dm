...
/mob/living/simple_animal/hostile/retaliate/malf_drone/Life()
	...
	if(prob(1))
		src.visible_message("<span style='color:#FF0000'>\icon[src] [src] shudders and shakes as some of it's damaged systems come back online.</span>")
	...
	if(prob(disabled ? 0 : 1))
		if(hostile_drone)
			src.visible_message("<span style='color:#0000FF'>\icon[src] [src] retracts several targetting vanes, and dulls it's running lights.</span>")
			hostile_drone = 0
		else
			src.visible_message("<span style='color:#FF0000'>\icon[src] [src] suddenly lights up, and additional targetting vanes slide into place.</span>")
			hostile_drone = 1
	...
	if(health / maxHealth > 0.9)
		...
		if(!disabled)
			if(prob(50))
				src.visible_message("<span style='color:#0000FF'>\icon[src] [src] suddenly shuts down!</span>")
			else
				src.visible_message("<span style='color:#0000FF'>\icon[src] [src] suddenly lies still and quiet.</span>")
			disabled = rand(150, 600)
			walk(src,0)

	if(exploding && prob(20))
		if(prob(50))
			src.visible_message("<span style='color:#FF0000'>\icon[src] [src] begins to spark and shake violenty!</span>")
		else
			src.visible_message("<span style='color:#FF0000'>\icon[src] [src] sparks and shakes like it's about to explode!</span>")
		var/datum/effect_system/spark_spread/s = new /datum/effect_system/spark_spread
		s.set_up(3, 1, src)
		s.start()
