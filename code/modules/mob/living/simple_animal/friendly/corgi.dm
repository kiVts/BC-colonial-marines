		...
		if(!stat)
			for(var/mob/M in viewers(user, null))
				if ((M.client && !( M.blinded )))
					M.show_message("<span style='color:#0000FF'>[user] baps [name] on the nose with the rolled up [O]</span>")
			spawn(0)
...
/mob/living/simple_animal/corgi/puppy/Topic(href, href_list)
	if(href_list["remove_inv"] || href_list["add_inv"])
		to_chat(usr, "<span style='color:#FF0000'>You can't fit this on [src]</span>")
		return
	..()
...
/mob/living/simple_animal/corgi/Lisa/Topic(href, href_list)
	if(href_list["remove_inv"] || href_list["add_inv"])
		to_chat(usr, "<span style='color:#FF0000'>[src] already has a cute bow!</span>")
		return
	..()
