...
/mob/living/simple_animal/hostile/retaliate/goat/Life()
	...
	if(.)
		...
		if(enemies.len && prob(10))
			enemies = list()
			LoseTarget()
			src.visible_message("<span style='color:#0000FF'>[src] calms down.</span>")
...
/mob/living/simple_animal/hostile/retaliate/goat/Retaliate()
	..()
	src.visible_message("<span style='color:#FF0000'>[src] gets an evil-looking gleam in their eye.</span>")
...
/mob/living/simple_animal/hostile/retaliate/goat/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(stat == CONSCIOUS && istype(G) && G.is_open_container())
		...
		if(G.reagents.total_volume >= G.volume)
			to_chat(user, "<span style='color:#FF0000'>The [O] is full.</span>")
		if(!transfered)
			to_chat(user, "<span style='color:#FF0000'>The udder is dry. Wait a bit longer...</span>")
	else
		..()
//cow
...
/mob/living/simple_animal/chicken/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(istype(O, /obj/item/reagent_container/food/snacks/grown/wheat)) //feedin' dem chickens
		if(!stat && eggsleft < 8)
			user.visible_message("<span style='color:#0000FF'>[user] feeds [O] to [name]! It clucks happily.","\blue You feed [O] to [name]! It clucks happily.</span>")
			user.drop_held_item()
			cdel(O)
			eggsleft += rand(1, 4)
			//to_chat(world, eggsleft)
		else
			to_chat(user, "<span style='color:#0000FF'>[name] doesn't seem hungry!</span>")
	else
		..()
