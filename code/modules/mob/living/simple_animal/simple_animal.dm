...
/mob/living/simple_animal/attack_hand(mob/living/carbon/human/M as mob)
	...
	switch(M.a_intent)
			...
			if (health > 0)
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message("<span style='color:#0000FF'>[M] [response_help] [src]</span>")
		...
		if("hurt", "disarm")
			...
			for(var/mob/O in viewers(src, null))
				if ((O.client && !( O.blinded )))
					O.show_message("<span style='color:#FF0000'>[M] [response_harm] [src]</span>")
				...
				if(MED.get_amount() >= 1)
					...
					for(var/mob/M in viewers(src, null))
						if ((M.client && !( M.blinded )))
							M.show_message("<span style='color:#0000FF'>[user] applies the [MED] on [src]</span>")
		else
			to_chat(user, "<span style='color:#0000FF'>this [src] is dead, medical items won't bring it back to life.</span>")
