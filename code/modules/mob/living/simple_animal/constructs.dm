	...
	if(O.force)
		...
		for(var/mob/M in viewers(src, null))
			if ((M.client && !( M.blinded )))
				M.show_message("<span style='color:#FF0000'>\b [src] has been attacked with [O] by [user]. </span>")
	else
		to_chat(usr, "<span style='color:#FF0000'>This weapon is ineffective, it does no damage.</span>")
		for(var/mob/M in viewers(src, null))
			if ((M.client && !( M.blinded )))
				M.show_message("<span style='color:#FF0000'>[user] gently taps [src] with [O]. </span>")
...
/mob/living/simple_animal/construct/armoured/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(O.force)
		if(O.force >= 11)
			...
			for(var/mob/M in viewers(src, null))
				if ((M.client && !( M.blinded )))
					M.show_message("<span style='color:#FF0000'>\b [O] bounces harmlessly off of [src]. </span>")
	else
		to_chat(usr, "<span style='color:#FF0000'>This weapon is ineffective, it does no damage.</span>")
		for(var/mob/M in viewers(src, null))
			if ((M.client && !( M.blinded )))
...
/client/proc/summon_cultist()
	...
	if (istype(usr,/mob/living/simple_animal/constructbehemoth))
		...
		if(usr.energy<300)
			to_chat(usr, "<span style='color:#FF0000'>You do not have enough power stored!</span>")
			return
	...
	for(var/datum/mind/H in ticker.mode.cult)
		if (istype(H.current,/mob/living))
			...
			if (cultist == usr) //just to be sure.
				return
			cultist.loc = usr.loc
			usr.visible_message("<span style='color:#FF0000'>[cultist] appears in a flash of red light as [usr] glows with power</span>")*/
