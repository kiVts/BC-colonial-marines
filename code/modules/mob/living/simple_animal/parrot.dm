	...
	if(ishuman(usr) || ismonkey(usr) || isrobot(usr))
			...
			switch(remove_from)
				if("ears")
					if(ears)
						...
						to_chat(usr, "<span style='color:#FF0000'>There is nothing to remove from its [remove_from].</span>")
						return
		...
		//Adding things to inventory
			...
			if(!usr.get_active_hand())
				to_chat(usr, "<span style='color:#FF0000'>You have nothing in your hand to put on its [add_to].</span>")
				return
			switch(add_to)
				if("ears")
					if(ears)
						to_chat(usr, "<span style='color:#FF0000'>It's already wearing something.</span>")
						return
					else
						...
						if( !istype(item_to_add,  /obj/item/device/radio/headset) )
							to_chat(usr, "<span style='color:#FF0000'>This object won't fit.</span>")
							return
		...
		if(in_range(src, parrot_interest))
			...
			if(isliving(parrot_interest))
				...
				if(!parrot_perch || parrot_interest.loc != parrot_perch.loc)
					...
					visible_message("[src] grabs the [held_item]!", "<span style='color:#0000FF'>You grab the [held_item]!", "You hear the sounds of wings flapping furiously.</span>")
...
//-----ATTACKING
		...
		if(in_range(src, parrot_interest))
			...
			if(ishuman(parrot_interest))
				...
				var/datum/limb/affecting = H.get_limb(ran_zone(pick(parrot_dam_zone)))
...
/mob/living/simple_animal/parrot/proc/steal_from_ground()
	...
	if(held_item)
		to_chat(src, "<span style='color:#FF0000'>You are already holding the [held_item]</span>")
		return 1
	...
	for(var/mob/living/carbon/C in view(1,src))
		...
		if(stolen_item)
			if(C.drop_inv_item_to_loc(stolen_item, src))
				...
				visible_message("[src] grabs the [held_item] out of [C]'s hand!", "<span style='color:#0000FF'>You snag the [held_item] out of [C]'s hand!", "You hear the sounds of wings flapping furiously.</span>")
				return held_item
		...
		for(var/atom/movable/AM in view(src,1))
			for(var/perch_path in desired_perches)
				if(istype(AM, perch_path))
					...
					icon_state = "parrot_sit"
					return
	to_chat(src, "<span style='color:#FF0000'>There is no perch nearby to sit on.</span>")
	return
