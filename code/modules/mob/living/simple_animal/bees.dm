...
/mob/living/simple_animal/bee/Life()
	...
	if(stat == CONSCIOUS)
		...
		if(M in view(src,1)) // Can I see my target?
			if(prob(max(feral * 10, 0)))	// Am I mad enough to want to sting? And yes, when I initially appear, I AM mad enough
				...
				if( prob(sting_prob) && (M.stat == CONSCIOUS || (M.stat == UNCONSCIOUS && prob(25))) ) // Try to sting! If you're not moving, think about stinging.
					...
					to_chat(M, "<span style='color:#FF0000'>You have been stung!</span>")
					M.flash_pain()
		...
		//make some noise
		if(prob(0.5))
			src.visible_message("<span style='color:#0000FF'>[pick("Buzzzz.","Hmmmmm.","Bzzz.")]</span>")
		...
		if(calming)
			if(feral > 0)
				src.visible_message("<span style='color:#0000FF'>The bees calm down!</span>")
			feral = -10
			target_mob = null
			target_turf = null
			wander = 1
		...
		if(target_turf)
			if (!(DirBlocked(get_step(src, get_dir(src,target_turf)),get_dir(src,target_turf)))) // Check for windows and doors!
				...
				if (prob(0.1))
					src.visible_message("<span style='color:#0000FF'>The bees swarm after [target_mob]!</span>")
