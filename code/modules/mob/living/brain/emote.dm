/mob/living/brain/emote(var/act,var/m_type=1,var/message = null, player_caused)
	...
	switch(act)
			...
			if(player_caused)
				if (src.client)
					if (client.prefs.muted & MUTE_IC)
						to_chat(src, "<span style='color:#FF0000'>You cannot send IC messages (muted).</span>")
						return
		...
		if ("help")
			...
			to_chat(src, "<span style='color:#0000FF'>Unusable emote '[act]'. Say *help for a list.</span>")
