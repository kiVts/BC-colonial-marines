	...
	if (radiation)
		if (radiation > 100)
			...
			if(!container)//If it's not in an MMI
				to_chat(src, "<span style='color:#FF0000'>You feel weak.</span>")
			else//Fluff-wise, since the brain can't detect anything itself, the MMI handles thing like that
				to_chat(src, "<span style='color:#FF0000'>STATUS: CRITICAL AMOUNTS OF RADIATION DETECTED.</span>")
			...
			if(50 to 74)
				...
				if(prob(5))
					...
					if(!container)
						...
						to_chat(src, "<span style='color:#FF0000'>STATUS: DANGEROUS LEVELS OF RADIATION DETECTED.</span>")
				updatehealth()
			...
			switch(emp_damage)
				...
				if(21 to 30)//High level of EMP damage, unable to see, hear, or speak
					...
					if(!alert)//Sounds an alarm, but only once per 'level'
						emote("alarm")
						to_chat(src, "<span style='color:#FF0000'>Major electrical distruption detected: System rebooting.</span>")
						alert = 1
					if(prob(75))
						emp_damage -= 1
				if(20)
				...
				if(11 to 19)//Moderate level of EMP damage, resulting in nearsightedness and ear damage
					...
					if(!alert)
						emote("alert")
						to_chat(src, "<span style='color:#FF0000'>Primary systems are now online.</span>")
						alert = 1
					if(prob(50))
						emp_damage -= 1
				if(10)
				...
				if(2 to 9)//Low level of EMP damage, has few effects(handled elsewhere)
					if(!alert)
						emote("notice")
						to_chat(src, "<span style='color:#FF0000'>System reboot nearly complete.</span>")
						alert = 1
					if(prob(25))
						emp_damage -= 1
				if(1)
					alert = 0
					to_chat(src, "<span style='color:#FF0000'>All systems restored.</span>")
					emp_damage -= 1
