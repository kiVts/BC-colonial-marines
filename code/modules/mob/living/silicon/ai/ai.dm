...
//		to_chat(src, text ("Switching Law [L]'s report status to []", lawcheck[L+1]))
	...
	if (href_list["track"])
		...
		if(target && (!istype(target, /mob/living/carbon/human) || html_decode(href_list["trackname"]) == target:get_face_name()))
			...
			to_chat(src, "<span style='color:#FF0000'>System error. Cannot locate [html_decode(href_list["trackname"])].</span>")
		return
	...
	if(isnull(network))
		...
		for(var/obj/machinery/camera/C in cameranet.cameras)
			...
			if(network in C.network)
				...
				break
	to_chat(src, "<span style='color:#0000FF'>Switched to [network] camera network.</span>")
...
/mob/living/silicon/ai/proc/ai_hologram_change()
	...
	if(alert("Would you like to select a hologram based on a crew member or switch to unique avatar?",,"Crew Member","Unique")=="Crew Member")
		...
		if(personnel_list.len)
			...
			if(character_icon)
				cdel(holo_icon)//Clear old icon so we're not storing it in memory.
...
/*/mob/living/silicon/ai/proc/corereturn()
	...
	if(!istype(apc))
		to_chat(src, "<span style='color:#0000FF'>You are already in your Main Core.</span>")
		return
	apc.malfvacate()*/
...
/mob/living/silicon/ai/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W, /obj/item/tool/wrench))
		if(anchored)
			user.visible_message("<span style='color:#0000FF'>\The [user] starts to unbolt \the [src] from the plating...</span>")
			if(!do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
				user.visible_message("<span style='color:#0000FF'>\The [user] decides not to unbolt \the [src].</span>")
				return
			user.visible_message("<span style='color:#0000FF'>\The [user] finishes unfastening \the [src]!</span>")
			anchored = 0
			return
		else
			user.visible_message("<span style='color:#0000FF'>\The [user] starts to bolt \the [src] to the plating...</span>")
			if(!do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
				user.visible_message("<span style='color:#0000FF'>\The [user] decides not to bolt \the [src].</span>")
				return
			user.visible_message("<span style='color:#0000FF'>\The [user] finishes fastening down \the [src]!</span>")
			anchored = 1
			return
	else
		return ..()
...
/mob/living/silicon/ai/proc/check_unable(var/flags = 0)
	if(stat == DEAD)
		to_chat(usr, "<span style='color:#FF0000'>You are dead!</span>")
		return 1

	if((flags & AI_CHECK_WIRELESS) && src.control_disabled)
		to_chat(usr, "<span style='color:#FF0000'>Wireless control is disabled!</span>")
		return 1
	if((flags & AI_CHECK_RADIO) && src.aiRadio.disabledAi)
		to_chat(src, "<span style='color:#FF0000'>System Error - Transceiver Disabled!</span>")
		return 1
	return 0
