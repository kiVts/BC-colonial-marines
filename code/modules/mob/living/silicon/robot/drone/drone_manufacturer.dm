...
/mob/dead/verb/join_as_drone()
	...
	if(ticker.current_state < GAME_STATE_PLAYING)
		to_chat(src, "<span style='color:#FF0000'>The game hasn't started yet!</span>")
		return
	...
	if(jobban_isbanned(src,"Cyborg"))
		to_chat(usr, "<span style='color:#FF0000'>You are banned from playing synthetics and cannot spawn as a drone.</span>")
		return
...
//			return
	...
	for(var/obj/machinery/drone_fabricator/DF in machines)
		...
		if(DF.count_drones() >= config.max_maint_drones)
			to_chat(src, "<span style='color:#FF0000'>There are too many active drones in the world for you to spawn.</span>")
			return
