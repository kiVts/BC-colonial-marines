...
/obj/machinery/computer/drone_control/Topic(href, href_list)
	...
	if (href_list["setarea"])
		...
		to_chat(usr, "<span style='color:#0000FF'>You set the area selector to [drone_call_area].</span>")

	else if (href_list["ping"])

		to_chat(usr, "<span style='color:#0000FF'>You issue a maintenance request for all active drones, highlighting [drone_call_area].</span>")
		for(var/mob/living/silicon/robot/drone/D in mob_list)
		...
		if(D.stat != 2)
			to_chat(usr, "<span style='color:#FF0000'>You issue a law synchronization directive for the drone.</span>")
			D.law_resync()
		...
		if(D.stat != 2)
			to_chat(usr, "<span style='color:#FF0000'>You issue a kill command for the unfortunate drone.</span>")
		...
		for(var/obj/machinery/drone_fabricator/fab in oview(3,src))
			...
			dronefab = fab
			to_chat(usr, "<span style='color:#0000FF'>Drone fabricator located.</span>")
			return

		to_chat(usr, "<span style='color:#FF0000'>Unable to locate drone fabricator.</span>")
		...
		to_chat(usr, "<span style='color:#0000FF'>You [dronefab.produce_drones ? "enable" : "disable"] drone production in the nearby fabricator.</span>")
