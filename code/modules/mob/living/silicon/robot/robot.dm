...
/mob/living/silicon/robot/verb/self_diagnosis_verb()
	...
	if(!is_component_functioning("diagnosis unit"))
		to_chat(src, "<span style='color:#FF0000'>Your self-diagnosis component isn't functioning.</span>")

	var/datum/robot_component/CO = get_component("diagnosis unit")
	if (!cell_use_power(CO.active_usage))
		to_chat(src, "<span style='color:#FF0000'>Low Power.</span>")
...
/mob/living/silicon/robot/verb/toggle_component()
	...
	if(C.toggled)
		C.toggled = 0
		to_chat(src, "<span style='color:#FF0000'>You disable [C.name].</span>")
	else
		C.toggled = 1
		to_chat(src, "<span style='color:#FF0000'>You enable [C.name].</span>")
		...
		for(var/V in components)
			...
			if(!C.installed && istype(W, C.external_type))
				...
				if(user.drop_held_item())
					...
					to_chat(usr, "<span style='color:#0000FF'>You install the [W.name].</span>")
	...
	if (istype(W, /obj/item/tool/weldingtool))
		...
		if (WT.remove_fuel(0))
			...
			for(var/mob/O in viewers(user, null))
				O.show_message(text("<span style='color:#FF0000'>[user] has fixed some of the dents on [src]!</span>"), 1)
		else
		...
		if (coil.use(1))
			...
			updatehealth()
			for(var/mob/O in viewers(user, null))
				O.show_message(text("<span style='color:#FF0000'>[user] has fixed some of the burnt wires on [src]!</span>"), 1)
		...
		if(opened)
			...
			if(allowed(usr))
				...
				to_chat(user, "<span style='color:#FF0000'>Access denied.</span>")
		...
		if(opened)//Cover is open
			...
			if(wiresexposed)
				...
				if(prob(50))
					...
					to_chat(src, "<span style='color:#FF0000'>ALERT: Foreign software detected.</span>")
					sleep(5)
					to_chat(src, "<span style='color:#FF0000'>Initiating diagnostics...</span>")
					sleep(20)
					to_chat(src, "<span style='color:#FF0000'>SynBorg v1.7.1 loaded.</span>")
					sleep(5)
					to_chat(src, "<span style='color:#FF0000'>LAW SYNCHRONISATION ERROR</span>")
					sleep(5)
					to_chat(src, "<span style='color:#FF0000'>Would you like to send a report to NanoTraSoft? Y/N</span>")
					sleep(10)
					to_chat(src, "<span style='color:#FF0000'>> N</span>")
					sleep(20)
					to_chat(src, "<span style='color:#FF0000'>ERRORERRORERROR</span>")
					to_chat(src, "<b>Obey these laws:</b>")
					laws.show_laws(src)
					to_chat(src, "<span style='color:#FF0000'>\b ALERT: [user.real_name] is your new master. Obey your new laws and his commands.</span>")
					update_icons()
				else
...
/mob/living/silicon/robot/proc/installed_modules()
	if(weapon_lock)
		to_chat(src, "<span style='color:#FF0000'>Weapon lock active, unable to use modules! Count:[weaponlock_time]</span>")
		return
				...
				for(var/A in tile)
					if(istype(A, /obj/effect))
						...
						if(cleaned_human.lying)
							...
							to_chat(cleaned_human, "<span style='color:#FF0000'>[src] cleans your face!</span>")
		return
