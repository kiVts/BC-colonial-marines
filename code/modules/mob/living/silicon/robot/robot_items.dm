...
/obj/item/tool/kitchen/tray/robotray
	name = "RoboTray"
	desc = "An autoloading tray specialized for carrying refreshments."

/obj/item/tool/kitchen/tray/robotray/afterattack(atom/target, mob/user as mob, proximity)
	if(!proximity)
		return
	if ( !target )
		return
	...
	if ( istype(target,/obj/item))
		...
		for(var/obj/item/reagent_container/food/I in pickup)
			...
			if( I != src && !I.anchored && !istype(I, /obj/item/clothing/under) && !istype(I, /obj/item/clothing/suit) && !istype(I, /obj/item/projectile) )
				...
				addedSomething = 1
		if ( addedSomething )
			user.visible_message("<span style='color:#0000FF'>[user] load some items onto their service tray.</span>")
	...
	if ( isturf(target) || istype(target,/obj/structure/table) )
		...
		if ( droppedSomething )
			if ( foundtable )
				user.visible_message("<span style='color:#0000FF'>[user] unloads their service tray.</span>")
			else
				user.visible_message("<span style='color:#0000FF'>[user] drops all the items on their tray.</span>")
...
/obj/item/form_printer/proc/deploy_paper(var/turf/T)
	T.visible_message("<span style='color:#0000FF'>\The [src.loc] dispenses a sheet of crisp white paper.</span>")
