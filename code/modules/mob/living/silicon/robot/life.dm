...
/mob/living/silicon/robot/proc/use_power()
	...
	if ( cell && is_component_functioning("power cell") && src.cell.charge > 0 )
		...
		if (src.has_power)
			to_chat(src, "<span style='color:#FF0000'>You are now running on emergency backup power.</span>")
		src.has_power = 0
...
/mob/living/silicon/robot/proc/process_killswitch()
	if(killswitch)
		...
		if(killswitch_time <= 0)
			if(src.client)
				to_chat(src, "<span style='color:#FF0000'><B>Killswitch Activated</span>")
			killswitch = 0
			spawn(5)
				gib()
...
/mob/living/silicon/robot/proc/process_locks()
	if(weapon_lock)
		...
		if(weaponlock_time <= 0)
			if(src.client)
				to_chat(src, "<span style='color:#FF0000'><B>Weapon Lock Timed Out!</span>")
			weapon_lock = 0
			weaponlock_time = 120
