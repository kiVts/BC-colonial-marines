...
/mob/new_player/proc/poll_player(var/pollid = -1)
	...
	if(dbcon.IsConnected())
		...
		if(!found)
			to_chat(usr, "<span style='color:#FF0000'>Poll question details not found.</span>")
			return
...
/mob/new_player/proc/vote_on_poll(var/pollid = -1, var/optionid = -1, var/multichoice = 0)
	...
	if(dbcon.IsConnected())
		...
		if(!validpoll)
			to_chat(usr, "<span style='color:#FF0000'>Poll is not valid.</span>")
			return
		...
		if(!validoption)
			to_chat(usr, "<span style='color:#FF0000'>Poll option is not valid.</span>")
			return
		...
		if(!multichoice && alreadyvoted)
			to_chat(usr, "<span style='color:#FF0000'>You already voted in this poll.</span>")
			return

		if(multichoice && (alreadyvoted >= multiplechoiceoptions))
			to_chat(usr, "<span style='color:#FF0000'>You already have more than [multiplechoiceoptions] logged votes on this poll. Enough is enough. Contact the database admin if this is an error.</span>")
			return
		...
		to_chat(usr, "<span style='color:#0000FF'>Vote successful.</span>")
...
/mob/new_player/proc/log_text_poll_reply(var/pollid = -1, var/replytext = "")
	...
	if(dbcon.IsConnected())
		...
		if(alreadyvoted)
			to_chat(usr, "<span style='color:#FF0000'>You already sent your feedback for this poll.</span>")
			return
		...
		to_chat(usr, "<span style='color:#0000FF'>Feedback logging successful.</span>")
		usr << browse(null,"window=playerpoll")
