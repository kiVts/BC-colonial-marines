...
/client/Southwest()
	if(iscarbon(usr))
		...
		to_chat(usr, "<span style='color:#FF0000'>This mob type cannot throw items.</span>")
	return
...
/client/Northwest()
	if(iscarbon(usr))
		...
		if(!C.get_active_hand())
			to_chat(usr, "<span style='color:#FF0000'>You have nothing to drop in your hand.</span>")
			return
		C.drop_held_item()
	else
		to_chat(usr, "<span style='color:#FF0000'>This mob type cannot drop items.</span>")
	return
...
/client/verb/delete_key_pressed()
	...
	if(!usr.pulling)
		to_chat(usr, "<span style='color:#0000FF'>You are not pulling anything.</span>")
		return
	usr.stop_pulling()
...
/mob/proc/Process_Spacemove(var/check_drift = 0)
	...
	if(prob(Process_Spaceslipping(5)))
		to_chat(src, "<span style='color:#0000FF'><B>You slipped!</B></span>")
