...
/mob/dead/observer/New(mob/body)
	...
	if(ticker && ticker.mode && ticker.mode.flags_round_type & MODE_PREDATOR)
		spawn(20)
			to_chat(src, "<span style='color:#FF0000'>This is a <b>PREDATOR ROUND</b>! If you are whitelisted, you may Join the Hunt!</span>")
			return
...
/mob/dead/observer/verb/toggle_HUDs()
	...
	switch(hud_choice)
		if("Medical HUD")
			...
			to_chat(src, (ghost_medhud ? "<span style='color:#0000FF'><B>[hud_choice] Enabled</B>" : "\blue <B>[hud_choice] Disabled</B></span>"))
		if("Security HUD")
			...
			client.prefs.save_preferences()
			to_chat(src, (ghost_sechud ? "<span style='color:#0000FF'><B>[hud_choice] Enabled</B>" : "\blue <B>[hud_choice] Disabled</B></span>"))
		if("Squad HUD")
			...
			client.prefs.save_preferences()
			to_chat(src, (ghost_squadhud ? "<span style='color:#0000FF'><B>[hud_choice] Enabled</B>" : "\blue <B>[hud_choice] Disabled</B></span>"))
		if("Xeno Status HUD")
			...
			client.prefs.save_preferences()
			to_chat(src, (ghost_xenohud ? "<span style='color:#0000FF'><B>[hud_choice] Enabled</B>" : "\blue <B>[hud_choice] Disabled</B></span>"))
...
/mob/dead/observer/verb/follow_xeno()
	...
	if(mobs.len == 0)
		to_chat(usr, "<span style='color:#FF0000'>There aren't any living Xenos.</span>")
		return
...
/mob/dead/observer/verb/follow_pred()
	...
	if(mobs.len == 0)
		to_chat(usr, "<span style='color:#FF0000'>There aren't any living Predators.</span>")
		return
...
/mob/dead/observer/verb/follow_human()
	...
	if(mobs.len == 0)
		to_chat(usr, "<span style='color:#FF0000'>There aren't any living Humans.</span>")
		return
...
/mob/dead/observer/proc/ManualFollow(var/atom/movable/target)
	if(target && target != src)
		...
		following = target
		loc = target.loc
		to_chat(src, "<span style='color:#0000FF'>Now following [target]</span>")
		spawn(0) //Backup
...
/mob/dead/observer/memory()
	set hidden = 1
	to_chat(src, "<span style='color:#FF0000'>You are dead! You have no mind to store memory!</span>")
...
/mob/dead/observer/verb/analyze_air()
	...
	to_chat(src, "<span style='color:#0000FF'><B>Results:</B></span>")
	if(abs(pressure - ONE_ATMOSPHERE) < 10)
		to_chat(src, "<span style='color:#0000FF'>Pressure: [round(pressure,0.1)] kPa</span>")
	else
		to_chat(src, "<span style='color:#FF0000'>Pressure: [round(pressure,0.1)] kPa</span>")

	to_chat(src, "<span style='color:#0000FF'>Gas type: [env_gas]</span>")
	to_chat(src, "<span style='color:#0000FF'>Temperature: [round(env_temperature-T0C,0.1)]&deg;C</span>")
...
/*/mob/dead/verb/bloody_doodle()
	...
	if(!(config.cult_ghostwriter))
		to_chat(src, "<span style='color:#FF0000'>That verb is not currently permitted.</span>")
		return
	...
	if(!ghosts_can_write)
		to_chat(src, "<span style='color:#FF0000'>The veil is not thin enough for you to do that.</span>")
		return
	...
	if (message)
		...
		W.visible_message("<span style='color:#FF0000'>Invisible fingers crudely paint something in blood on [T]...</span>")*/
...
/mob/dead/verb/join_as_hellhound()
	...
	if(hellhound_list.len == 0)
		to_chat(usr, "<span style='color:#FF0000'>There aren't any available Hellhounds.</span>")
		return
	...
	if(!istype(L, /mob/living/carbon/hellhound))
		to_chat(usr, "<span style='color:#FF0000'>That's not a Hellhound.</span>")
		return

	if(L.stat == DEAD)  // DEAD
		to_chat(usr, "<span style='color:#FF0000'>It's dead.</span>")
		return

	if(L.client) // Larva player is still online
		to_chat(usr, "<span style='color:#FF0000'>That player is still connected.</span>")
		return
	...
	if (alert(usr, "Everything checks out. Are you sure you want to transfer yourself into this hellhound?", "Confirmation", "Yes", "No") == "Yes")
		...
		if(L.client || L.stat == DEAD) // Do it again, just in case
			to_chat(usr, "<span style='color:#FF0000'>Oops. That mob can no longer be controlled. Sorry.</span>")
			return
		...
		if( isobserver(ghostmob) )
			cdel(ghostmob)
		spawn(15)
			to_chat(L, "<span style='color:#FF0000'><B>Attention!! You are playing as a hellhound. You can get server banned if you are shitty so listen up!</b></span>")
			to_chat(L, "<span style='color:#FF0000'>You MUST listen to and obey the Predator's commands at all times. Die if they demand it. Not following them is unthinkable to a hellhound.</span>")
			to_chat(L, "<span style='color:#FF0000'>You are not here to go hog wild rambo. You're here to be part of something rare, a Predator hunt.</span>")
			to_chat(L, "<span style='color:#FF0000'>The Predator players must follow a strict code of role-play and you are expected to as well.</span>")
			to_chat(L, "<span style='color:#FF0000'>The Predators cannot understand your speech. They can only give you orders and expect you to follow them. They have a camera that allows them to see you remotely, so you are excellent for scouting missions.</span>")
			to_chat(L, "<span style='color:#FF0000'>Hellhounds are fiercely protective of their masters and will never leave their side if under attack.</span>")
			to_chat(L, "<span style='color:#FF0000'>Note that ANY Predator can give you orders. If they conflict, follow the latest one. If they dislike your performance they can ask for another ghost and everyone will mock you. So do a good job!</span>")
	return
...
/mob/dead/verb/drop_vote()
	...
	if(!ticker || ticker.current_state < GAME_STATE_PLAYING || !ticker.mode)
		to_chat(usr, "<span style='color:#FF0000'>The game hasn't started yet!</span>")
		return
