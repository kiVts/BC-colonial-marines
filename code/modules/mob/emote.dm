...
/mob/proc/emote_dead(var/message)
	...
	if(client.prefs.muted & MUTE_DEADCHAT)
		to_chat(src, "<span style='color:#FF0000'>You cannot send deadchat emotes (muted).</span>")
		return

	if(!(client.prefs.toggles_chat & CHAT_DEAD))
		to_chat(src, "<span style='color:#FF0000'>You have deadchat muted.</span>")
		return

	if(!src.client.holder)
		if(!dsay_allowed)
			to_chat(src, "<span style='color:#FF0000'>Deadchat is globally muted</span>")
			return
