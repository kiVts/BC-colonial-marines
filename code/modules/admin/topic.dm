		...
		switch(href_list["evac_authority"])
			if("init_evac")
				if(!EvacuationAuthority.initiate_evacuation())
					...
					message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] called an evacuation.</span>", 1)
			...
			if("cancel_evac")
				if(!EvacuationAuthority.cancel_evacuation())
					...
					message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] canceled an evacuation.</span>", 1)
			...
			if("force_evac")
				if(!EvacuationAuthority.begin_launch())
					...
					message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] force-launched the escape pods.</span>", 1)
			...
			if("init_dest")
				if(!EvacuationAuthority.enable_self_destruct())
					...
					message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] force-enabled the self-destruct system.</span>", 1)
			...
			if("cancel_dest")
				if(!EvacuationAuthority.cancel_self_destruct(1))
					...
					message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] canceled the self-destruct system.</span>", 1)
			...
			if("use_dest")
				...
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] forced the self-destrust system, destroying the [MAIN_SHIP_NAME].</span>", 1)
...
//======================================================
		...
		message_admins("<span style='color:#0000FF'>[key_name(usr)] [ticker.delay_end ? "delayed the round end" : "has made the round end normally"].</span>", 1)
		href_list["secretsadmin"] = "check_antagonist"
		...
		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] has used rudimentary transformation on [key_name_admin(M)]. Transforming to [href_list["simplemake"]]; deletemob=[delmob]</span>", 1)
	...
	/////////////////////////////////////new ban stuff
		...
		if(alert(usr, "Are you sure you want to unban [key]?", "Confirmation", "Yes", "No") == "Yes")
			if((Banlist["minutes"] - CMinutes) > 10080)
				...
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] removed [key]'s permaban.</span>", 1)
		...
		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] upgraded [banned_key]'s ban to a permaban. Reason: [sanitize(reason)]</span>", 1)
		Banlist.cd = "/base/[banfolder]"
		...
		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] edited [banned_key]'s ban. Reason: [sanitize(reason)] Duration: [duration]</span>", 1)
		Banlist.cd = "/base/[banfolder]"
		Banlist["reason"] << sanitize(reason)
		Banlist["temp"] << temp
		Banlist["minutes"] << minutes
...
//Regular jobs
	...
	//JOBBAN'S INNARDS
		...
		if(notbannedlist.len) //at least 1 unbanned job exists in joblist so we have stuff to ban.
			...
			// 			to_chat(usr, "<span style='color:#FF0000'>Your server is using the legacy banning system, which does not support temporary job bans. Consider upgrading. Aborting ban.</span>")
			// 			return
			...
			// 		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] banned [key_name_admin(M)] from [msg] for [mins] minutes</span>", 1)
			// 		to_chat(M, "\red<BIG><B>You have been jobbanned by [usr.client.ckey] from: [msg].</B></BIG>")
			// 		to_chat(M, "<span style='color:#FF0000'><B>The reason is: [reason]</B></span>")
			// 		to_chat(M, "<span style='color:#FF0000'>This jobban will be lifted in [mins] minutes.</span>")
			// 		jobban_savebanfile()
			...
			if(reason)
				...
				notes_add(M.ckey, "Banned  from [msg] - [reason]")
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] banned [key_name_admin(M)] from [msg]</span>", 1)
				to_chat(M, "\red<BIG><B>You have been jobbanned by [usr.client.ckey] from: [msg].</B></BIG>")
				to_chat(M, "<span style='color:#FF0000'><B>The reason is: [reason]</B></span>")
				to_chat(M, "<span style='color:#FF0000'>Jobban can be lifted only upon request.</span>")
				jobban_savebanfile()
				href_list["jobban2"] = 1 // lets it fall through and refresh
				return 1
				// if("Cancel")
			...
			for(var/job in joblist)
				...
				switch(alert("Job: '[job]' Reason: '[reason]' Un-jobban?","Please Confirm","Yes","No"))
					if("Yes")
						...
						continue
			if(msg)
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] unbanned [key_name_admin(M)] from [msg]</span>", 1)
		...
		if (ismob(M))
			...
			if(!reason)
				to_chat(M, "<span style='color:#FF0000'>You have been kicked from the server</span>")
			else
				to_chat(M, "<span style='color:#FF0000'>You have been kicked from the server: [reason]</span>")
			log_admin("[key_name(usr)] booted [key_name(M)].")
			message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] booted [key_name_admin(M)].</span>", 1)
			//M.client = null
			cdel(M.client)
/*
	//Player Notes
...
*/
		...
		if(t)
			if((alert("Do you want to unjobban [t]?","Unjobban confirmation", "Yes", "No") == "Yes") && t) //No more misclicks! Unless you do it twice.
				...
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] removed [t]</span>", 1)
				jobban_remove(t)
				jobban_savebanfile()
		...
		if(!M.ckey)
			to_chat(usr, "<span style='color:#FF0000'><B>Warning: Mob ckey for [M.name] not found.</b></span>")
			return
		var/mob_key = M.ckey
		...
		to_chat(M, "<span style='color:#FF0000'>This is a temporary ban, it will be removed in [mins] minutes.</span>")
		feedback_inc("ban_tmp",1)
		DB_ban_record(BANTYPE_TEMP, M, mins, reason)
		feedback_inc("ban_tmp_mins",mins)
		if(config.banappeals)
			to_chat(M, "<span style='color:#FF0000'>To try to resolve this matter head to [config.banappeals]</span>")
		else
			to_chat(M, "<span style='color:#FF0000'>No ban appeals URL has been set.</span>")
		...
		to_chat(M, "\red<BIG><B>You have been banned by [usr.client.ckey].\nReason: [reason].</B></BIG>")
		to_chat(M, "<span style='color:#FF0000'>This is a temporary ban, it will be removed in [mins] minutes.</span>")
		to_chat(M, "<span style='color:#0000FF'>This ban was made using a one-click ban system. If you think an error has been made, please visit our forums' ban appeal section.</span>")
		to_chat(M, "<span style='color:#0000FF'>If you make sure to mention that this was a one-click ban, MadSnailDisease will personally double-check this code for you.</span>")
		if(config.banappeals)
			to_chat(M, "<span style='color:#0000FF'>The ban appeal forums are located here: [config.banappeals]</span>")
		else
			to_chat(M, "<span style='color:#0000FF'>Unfortunately, no ban appeals URL has been set.</span>")
		feedback_inc("ban_tmp", 1)
		DB_ban_record(BANTYPE_TEMP, M, mins, reason)
		feedback_inc("ban_tmp_mins", mins)
		...
		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] set the mode as [master_mode].</span>", 1)
		to_chat(world, "<span style='color:#0000FF'><b>The mode is now: [master_mode]</b></span>")
		...
		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] set the forced secret mode as [secret_force_mode].</span>", 1)
		Game() // updates the main game menu
		...
		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] attempting to monkeyize [key_name_admin(H)]</span>", 1)
		H.monkeyize()
		...
		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] attempting to corgize [key_name_admin(H)]</span>", 1)
		H.corgize()
		...
		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] forced [key_name_admin(M)] to say: [speech]</span>")
		...
		to_chat(M, "<span style='color:#FF0000'>You have been sent to the prison station!</span>")
		log_admin("[key_name(usr)] sent [key_name(M)] to the prison station.")
		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] sent [key_name_admin(M)] to the prison station.</span>", 1)
		...
		M.loc = pick(tdome1)
		spawn(50)
			to_chat(M, "<span style='color:#0000FF'>You have been sent to the Thunderdome.</span>")
		...
		if(config.allow_admin_rev)
			L.revive()
			message_admins("<span style='color:#FF0000'>Admin [key_name_admin(usr)] healed / revived [key_name_admin(L)]!</span>", 1)
		...
		message_admins("<span style='color:#FF0000'>Admin [key_name_admin(usr)] AIized [key_name_admin(H)]!</span>", 1)
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
		...
		to_chat(H, "<span style='color:#0000FF'>Your prayers have been answered!! You received the <b>best cookie</b>!</span>")
		...
		to_chat(H, "<span style='color:#FF0000'>You hear something crackle in your headset before a voice speaks, \"Please stand by for a message from USCM:\" \blue <b>\"[input]\"</b></span>")
		...
		for(var/obj/machinery/faxmachine/F in machines)
			if(F == fax)
				...
				return
		to_chat(src.owner, "<span style='color:#FF0000'>Unable to locate fax!</span>")
		...
		switch(href_list["secretsfun"])
			if("gravity")
				...
				if(gravity_is_on)
					...
					message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] toggled gravity on.</span>", 1)
					...
					message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] toggled gravity off.</span>", 1)
			...
			if("traitor_all")
				...
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] used everyone is a traitor secret. Objective is [objective]</span>", 1)
			...
			if("power")
				...
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] powered all SMESs and APCs</span>", 1)
				power_restore()
			if("unpower")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","UP")
				log_admin("[key_name(usr)] unpowered all SMESs and APCs", 1)
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] unpowered all SMESs and APCs</span>", 1)
				power_failure()
			if("quickpower")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","QP")
				log_admin("[key_name(usr)] powered all SMESs", 1)
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] powered all SMESs</span>", 1)
				power_restore_quick()
			if("powereverything")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","PE")
				log_admin("[key_name(usr)] powered all SMESs and APCs everywhere", 1)
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] powered all SMESs and APCs everywhere</span>", 1)
			...
			if("gethumans")
				...
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] mass-teleported all humans.</span>", 1)
				get_all_humans()
			if("getxenos")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","GX")
				log_admin("[key_name(usr)] mass-teleported all Xenos.", 1)
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] mass-teleported all Xenos.</span>", 1)
				get_all_xenos()
			if("getall")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","GA")
				log_admin("[key_name(usr)] mass-teleported everyone.", 1)
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] mass-teleported everyone.</span>", 1)
				get_all()
			if("rejuvall")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","RA")
				log_admin("[key_name(usr)] mass-rejuvenated everyone.", 1)
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] mass-rejuvenated everyone.</span>", 1)
				rejuv_all()
		if(usr)
		...
		switch(href_list["secretsadmin"])
			...
			if("launchshuttle")
				...
				if (S.can_launch())
					S.launch(usr)
					message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] launched the [shuttle_tag] shuttle</span>", 1)
			...
			if("moveshuttle")
				...
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] moved the [shuttle_tag] shuttle</span>", 1)
	...
	if(href_list["mark"])
		...
		if(!istype(ref_person))
			to_chat(usr, "<span style='color:#0000FF'>Ётот игрок куда-то пропал!</span>")
			return
		...
		var/msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> пометил ваш јдминхелп и готовится ответить | Your request was marked, please wait...</b></span>"
	...
	if(href_list["noresponse"])
		...
		var/msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> пометил ваш ахелп и '¬мешательство не требуется'. Ћибо его уже расследуют, решили или в нем нет смысла. | <font color=red>[usr.key]</font> marked your Adminhelp as 'No response necessary'. Either your Adminhelp is being handled, it's fixed, or it's nonsensical.</font></b></span>"
	...
	if(href_list["warning"])
		...
		var/msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> выдал вам <font color=red>предупреждение</font> за неправильное использование системы јдминхелпа. | <font color=red>[usr.key]</font> has given you a <font color=red>warning</font> for abusing Adminhelp system.</b></span>"
	...
	if(href_list["autoresponse"]) // new verb on the Ahelp.  Will tell the person their message was received, and they probably won't get a response
		...
		var/choice = input(" акой јвто-ответ вы хотите отправить игроку?\n\n L - —сылка.\n A - ќтвет на распространенный вопрос", "Autoresponse", "--ќ“ћ≈Ќј--") in list ("--ќ“ћ≈Ќј--", "¬нутри-»гровая —итуация", "”же ѕроверяется", "ћеры Ѕыли ѕриняты", "—пасибо", "¬иновен", "L: ѕервичное –уководство  сеноморфа", "L: ѕервичное –уководство ћорпеха", "L: Ёкипировка ћорпехов", "L: “екущая  арта", "A: Ќет –егенерации ѕлазмы", "A: ѕроглатывание «а  сеноморфа", "A: »спользование ћедэвака", "J: ƒжоб Ѕаны", "E: »дет »вент", "R: –ации", "S: ѕредохранитель", "D: ѕрисоединение ќтключено", "M: ћакросы", "IC Issue", "Being Handled", "Fixed", "Thanks", "Guilty", "L: Xeno Quickstart Guide", "L: Marine quickstart guide", "L: Current Map", "A: No plasma regen", "A: Devour as Xeno", "J: Job bans", "E: Event in progress", "R: Radios", "S: Safety", "D: Joining disabled", "M: Macros")

		var/msgplayer
		switch(choice)
			if("¬нутри-»гровая —итуация")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. Ёто является внутри-игровой ситуацией и вмешательства со стороны администрации не требуется. ≈сли это важно, вы можете сообщить старшему по званию, если уверены, что был нарушен закон.</b></span>"
			if("”же ѕроверяется")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ѕроблемой уже занимаются.</b></span>"
			if("ћеры Ѕыли ѕриняты")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ћеры уже были приняты.</b></span>"
			if("—пасибо")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>! ѕриятной игры!</b></span>"
			if("¬иновен")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ¬ы нарушили закон.</b></span>"
			if("L: ѕервичное –уководство  сеноморфа")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ќтвет на ваш вопрос может быть найден в ѕервичном –уководстве  сеноморфа. <font color='#009900'><a href='http://wiki.ss13marines.ru/index.php?title=Xeno_Quickstart_Guide'>—сылка на руководство</a></font>.</b></span>"
			if("L: ѕервичное –уководство ћорпеха")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ќтвет на ваш вопрос может быть найден в ѕервичном –уководстве ћорпеха. <font color='#009900'><a href='wiki.ss13marines.ru/index.php?title=Marine_Quickstart_Guide'>—сылка на руководство</a></font>.</b></span>"
			if("L: Ёкипировка ћорпехов")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ќтвет на ваш вопрос может быть найден на странице вики \"Ёкипировка ћорпехов\". <font color='#009900'><a href='http://wiki.ss13marines.ru/index.php?title=%D0%AD%D0%BA%D0%B8%D0%BF%D0%B8%D1%80%D0%BE%D0%B2%D0%BA%D0%B0_%D0%BC%D0%BE%D1%80%D0%BF%D0%B5%D1%85%D0%BE%D0%B2'>—сылка на страницу</a></font>.</b></span>"
			if("L: “екущая  арта")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. —сылки на карты находятся <font color='#009900'><a href='http://cm.ss13.ru/index.php'>здесь</a></font>.</b></span>"
			if("A: Ќет –егенерации ѕлазмы")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ≈сли у вас не восстанавливается плазма, это значит, что вы либо находитесь вне травы ксеноморфов, либо используете переключаемые умения, расходующие плазму или препятствующие ее восстановлению, такие как Hide (—крытность) у Runner или Emit Pheromones (–аспространение ‘еромонов).</b></span>"
			if("A: ѕроглатывание «а  сеноморфа")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ѕроглатывание живых существ способствует их быстрой транспортировке к яйцам. ƒля того, чтобы проглотить жертву, схватите ее (CTRL+Click) и затем нажмите на себя. ƒля того, чтобы срыгнуть жертву, нажмите кнопку 'Regurgitate' на интерфейсе слева вверху. ƒержать людей в себе можно в течении минуты, после которой вы их срыгнете, а жертве расплавит случайную конечность.</b></span>"
			if("A: »спользование ћедэвака")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ћедэвак позволяет эвакуировать раненных бойцов с поля боя при условии, что над ними нет крыши или она стеклянная. ƒля проверки осмотрите тайл (SHIFT+Click). ѕоложив пациента в Medevac Stretcher таким же способом, как и на Roller Bed, нажмите правой кнопкой и нажмите Activate Medevac. „тобы забрать пациента с помощью установленного на шаттле системе медэвака, необходимо дождаться сигнала от медика, взлететь в FlyBy режиме, в консоли управления оружием шаттла выбрать систему медэвака и затем выбрать имя морпеха. «атем, необходимо пройти в салон непосредственно к самой системе медэвака и активировать ее кликом.</b></span>"
			if("J: ƒжоб Ѕаны")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ¬се джоб баны, в том числе на ксеноморфов, перманентны.</b></span>"
			if("E: »дет »вент")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ¬ данном раунде идет специальный ивент и некоторые вещи могут работать по-другому, однако обычные правила в силе, за исключением случаев, указанных администраторами.</b></span>"
			if("R: –ации")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. –ации были изменены: теперь ; позволит говорить в канал отряда, в то время как :z дает доступ в General (ќбщий) канал. ѕолевым медикам доступен медицинский канал через :m (:ь), полевым инженерам доступен Engineering (»нденерный) через :e (:у), а Squad Leader и Acting Squad Leader ( омандир ќтряда и »сполняющий –оль  омандира ќтряда) имеют доступ к командному каналу через :v (:м). (¬ скобках указаны кириллические буквы для каналов, они также работают). ќсмотр своего наушника показывает доступные вам каналы.</b></span>"
			if("S: ѕредохранитель")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <fond color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ƒля того, чтобы переключить предохранитель, используйте (ALT+Click) на оружие в вашей руке, либо Toggle Gun Safety во вкладке Weapons. ќсматривание оружия через (SHIFT+Click) покажет информацию о текущем состоянии предохранителя.</b></span>"
			if("D: ѕрисоединение ќтключено")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. јдминистрация отключила присоединение, так как раунд близится к концу. ¬ы можете зайти в Observe (Ќаблюдать), чтобы посмотреть конец раунда и дождаться начала нового.</b></span>"
			if("M: ћакросы")
				msgplayer = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> воспользовался авто-ответом <font color='#009900'>'[choice]'</font>. ƒля настройки макроса, нажмите правой кнопкой на самую верхнюю часть клиента игры, выберите Clien -> Macros. —оздание макроса для unique-action позволит нормально пользоваться стандартным дробовиком морпехов; макрос на resist поможет быстрей начать сопротивляться в некоторых ситуациях. ƒля более подробной информации посетите эту <font color='#009900'><a href='http://cm.ss13.ru/index.php?title=Macros'>ссылку</a></font>.</b></span>"
			if("IC Issue")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. This issue has been deemed an IC (In-Character) issue, and will not be handled by staff. In case it's relevant, you may wish to ask your <a href='http://wiki.ss13marines.ru/index.php?title=Rank'>Chain Of Command</a> about your issue if you believe <a href='http://wiki.ss13marines.ru/index.php?title=Marine_Law'>Marine Law</a> has been broken.</b></span>"
			if("Being Handled")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. The issue is already being dealt with.</b></span>"
			if("Fixed")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. The issue is already fixed.</b></span>"
			if("Thanks")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>! Have a DMCA day!</b></span>"
			if("Guilty")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. You broke Space Law.</b></span>"
			if("L: Xeno Quickstart Guide")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. Your answer can be found on the Xeno Quickstart Guide. <a href='http://cm-ss13.com/wiki/Xeno_Quickstart_Guide'>Check it out here.</a></b></span>"
			if("L: Marine quickstart guide")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. Your answer can be found on the Marine Quickstart Guide. <a href='http://cm-ss13.com/wiki/Marine_Quickstart_Guide'>Check it out here.</a></b></span>"
			if("L: Current Map")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. If you need a map for the current game, you can find it <a href='http://cm-ss13.com/wiki/Main_Page'>here.</a></b></span>"
			if("A: No plasma regen")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. If you have low/no plasma regen, it's most likely because you are off weeds or are currently using a passive ability, such as the Runner's 'Hide' or emitting a pheromone.</b></span>"
			if("A: Devour as Xeno")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. Devouring is useful to quickly transport incapacitated hosts from one place to another. In order to devour a host as a Xeno, grab the mob (CTRL+Click) and then click on yourself to begin devouring. The host can resist by breaking out of your belly, so make sure your target is incapacitated or only have them devoured for a short time. Also, the devoured host will eventually be digested (~5 minutes), which results in you killing a viable host to grow the hive. To release your target, click 'Regurgitate' on the HUD to throw them back up.</b></span>"
			if("J: Job bans")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. All job bans, including xeno bans, are permanent.</b></span>"
			if("E: Event in progress")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. There is currently a special event running and many things may be changed or different, however normal rules still apply unless you have been specifically instructed otherwise by a staff member.</b></span>"
			if("R: Radios")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. Radios have been changed, the prefix for all squad marines is now ; to access your squad radio. Squad Medics have access to the medical channel using :m, Engineers have :e and the (acting) Squad Leader has :v for command.  Examine your radio headset to get a listing of the channels you have access to.</b></span>"
			if("S: Safety")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. To toggle gun safety, use (Alt+Click) on the gun in your hands or Toggle Gun Safety in Weapons tab. Examining gun with (SHIFT+Click) will show you current state of gun safety.</b></span>"
			if("D: Joining disabled")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. A staff member has disabled joining for new players as the current round is coming to an end, you can observe while it ends and wait for a new round to start.</b></span>"
			if("M: Macros")
				msgplayer = "<span style='color:#0000FF'><b>NOTICE: <font color=red>[usr.key]</font> is autoresponding with <font color='#009900'>'[choice]'</font>. To set a macro right click the title bar, select Client->Macros. Binding unique-action to a key is useful for pumping shotguns etc; Binding load-from-attachment will activate any scopes etc; Binding resist and give to seperate keys is also handy. For more information on macros can be found <a href='http://cm-ss13.com/wiki/Macros'>here.</a></b></span>"
			else return
	...
	if(href_list["ccmark"]) // CentComm-mark. We want to let all Admins know that something is "Marked", but not let the player know because it's not very RP-friendly.
		...
		var/msg = "<span style='color:#0000FF'><b>«јћ≈“ ј: <font color=red>[usr.key]</font> отвечает игроку <font color=red>[ref_person.ckey]/([ref_person]) на HQ запрос.</font>.</b></span>"
