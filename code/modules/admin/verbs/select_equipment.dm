...
/client/proc/cmd_admin_dress(var/mob/living/carbon/human/M in mob_list)
	...
	var/list/dresspacks = list(
		...
		"USCM Senior Enlisted Advisor (SEA)",
...
/*		for(var/i = 1; i <= I.access.len; i++)
	...
	message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] changed the equipment of [key_name_admin(M)] to [dresscode].</span>", 1)
	return
...
/mob/proc/arm_equipment(var/mob/living/carbon/human/M, var/dresscode, var/squad)
	switch(dresscode)
		...
		if("USCM Vehicle Crewman")
			...
			W.assignment = "Vehicle Crewman"
			W.rank = "Vehicle Crewman"
			W.registered_name = M.real_name
			W.paygrade = "O1"
			M.equip_to_slot_or_del(W, WEAR_ID)
			if(M.mind)
				M.mind.role_comm_title = "VC"
				M.mind.assigned_role = "Vehicle Crewman"
				M.mind.set_cm_skills(/datum/skills/vehicle_crew)

		if("USCM Senior Enlisted Advisor")
			...
			W.assignment = "Senior Enlisted Advisor"
			W.rank = "Senior Enlisted Advisor"
			W.registered_name = M.real_name
			W.paygrade = "E9"
			M.equip_to_slot_or_del(W, WEAR_ID)
			if(M.mind)
				M.mind.role_comm_title = "SEA"
				M.mind.assigned_role = "Senior Enlisted Advisor"
				M.mind.set_cm_skills(/datum/skills/senior_enlisted_advisor)
		...
		if("USCM Civ Synthetic")
			if(!M.mind || !M.client)
				...
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] attempted to change the equipment of [key_name_admin(M)] to [dresscode], but failed due mob not having active player.</span>", 1)
				return
			...
			if(!name)
				...
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] attempted to change the equipment of [key_name_admin(M)] to [dresscode], but canceled.</span>", 1)
				return
			M.real_name = name
			M.mind.name = name
		...
		if("USCM Captain (CO)")
			...
			M.equip_to_slot_or_del(new /obj/item/clothing/gloves/marine/veteran/PMC(M), WEAR_HANDS)
			M.equip_to_slot_or_del(new /obj/item/storage/pouch/general/large(M), WEAR_R_STORE)
			M.equip_to_slot_or_del(new /obj/item/storage/pouch/general/large(M), WEAR_L_STORE)
