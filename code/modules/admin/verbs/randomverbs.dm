...
/client/proc/cmd_admin_subtle_message(mob/M as mob in mob_list)
	...
	message_admins("<span style='color:#0000FF'>\bold SubtleMessage: [key_name_admin(usr)] -> [key_name_admin(M)] : [msg]</span>", 1)
...
/client/proc/cmd_admin_world_narrate() // Allows administrators to fluff events a little easier -- TLE
	...
	message_admins("<span style='color:#0000FF'>\bold GlobalNarrate: [key_name_admin(usr)] : [msg]<BR></span>", 1)
...
/client/proc/cmd_admin_direct_narrate(var/mob/M)	// Targetted narrate -- TLE
	...
	message_admins("<span style='color:#0000FF'>\bold DirectNarrate: [key_name(usr)] to ([M.name]/[M.key]): [msg]<BR></span>", 1)
...
/client/proc/cmd_admin_godmode(mob/M as mob in mob_list)
	...
	to_chat(usr, "<span style='color:#0000FF'>Toggled [(M.status_flags & GODMODE) ? "ON" : "OFF"]</span>")
...
//	G.has_enabled_antagHUD = 2
	...
	G:show_message(text("<span style='color:#0000FF'><B>You may now respawn.  You should roleplay as if you learned nothing about the round during your time with the dead.</B></span>"), 1)
	...
	if(config.antag_hud_allowed)
		for(var/mob/dead/observer/g in get_ghosts())
			...
			if(g.antagHUD)
				...
				to_chat(g, "<span style='color:#FF0000'><B>The Administrator has disabled AntagHUD </B></span>")
		config.antag_hud_allowed = 0
		to_chat(src, "<span style='color:#FF0000'><B>AntagHUD usage has been disabled</B></span>")
		action = "disabled"
	else
		for(var/mob/dead/observer/g in get_ghosts())
			if(!g.client.holder)						// Add the verb back for all non-admin ghosts
				g.verbs += /mob/dead/observer/verb/toggle_antagHUD
			to_chat(g, "<span style='color:#0000FF'><B>The Administrator has enabled AntagHUD </B></span>"	)
		config.antag_hud_allowed = 1
		action = "enabled"
		to_chat(src, "<span style='color:#0000FF'><B>AntagHUD usage has been enabled</B></span>")
...
/client/proc/toggle_antagHUD_restrictions()
	...
	if(config.antag_hud_restricted)
		for(var/mob/dead/observer/g in get_ghosts())
			to_chat(g, "<span style='color:#0000FF'><B>The administrator has lifted restrictions on joining the round if you use AntagHUD</B></span>")
		action = "lifted restrictions"
		config.antag_hud_restricted = 0
		to_chat(src, "<span style='color:#0000FF'><B>AntagHUD restrictions have been lifted</B></span>")
	else
		for(var/mob/dead/observer/g in get_ghosts())
			to_chat(g, "<span style='color:#FF0000'><B>The administrator has placed restrictions on joining the round if you use AntagHUD</B></span>")
			to_chat(g, "<span style='color:#FF0000'><B>Your AntagHUD has been disabled, you may choose to re-enabled it but will be under restrictions </B></span>")
			g.antagHUD = 0
			g.has_enabled_antagHUD = 0
		action = "placed restrictions"
		config.antag_hud_restricted = 1
		to_chat(src, "<span style='color:#FF0000'><B>AntagHUD restrictions have been enabled</B></span>")
...
/client/proc/respawn_character()
	...
	if(G_found.mind && !G_found.mind.active)	//mind isn't currently in use by someone/something
		...
		if(findtext(G_found.real_name,"monkey"))
			if(alert("This character appears to have been a monkey. Would you like to respawn them as such?",,"Yes","No")=="Yes")
				...
				message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] has respawned [new_monkey.key] as a filthy xeno.</span>", 1)
	...
	*/
	...
	message_admins("<span style='color:#0000FF'>[admin] has respawned [player_key] as [new_character.real_name].</span>", 1)
...
/datum/admins/proc/force_predator_round()
	...
	message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] admin-forced a predator round.</span>", 1)
	return
	...
	for(var/mob/living/silicon/ai/M in mob_list)
		if (M.stat == 2)
			...
			for(var/mob/living/silicon/ai/O in mob_list)
				to_chat(O, "<span style='color:#FF0000'>" + input + "\red...LAWS UPDATED</span>")
				O.show_laws()
...
/client/proc/cmd_admin_rejuvenate(mob/living/M as mob in mob_list)
	...
	if(config.allow_admin_rev)
		...
		message_admins("<span style='color:#FF0000'>Admin [key_name_admin(usr)] healed / revived [key_name_admin(M)]!</span>", 1)
	else
...
/client/proc/cmd_admin_create_centcom_report()
	...
	switch(alert("Should this be announced to the general population?",,"Yes","No"))
		...
		if("No")
			command_announcement.Announce("<span style='color:#FF0000'>New update available at all communication consoles.</span>", customname, new_sound = 'sound/AI/commandreport.ogg')
...
/client/proc/cmd_admin_gib_self()
	...
	if(confirm == "Yes")
		...
		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] used gibself.</span>", 1)
...
/client/proc/cmd_manual_ban()
	...
	if("No")
		...
		to_chat(M, "\red<BIG><B>You have been banned by [usr.client.ckey].\nReason: [reason].</B></BIG>")
		to_chat(M, "<span style='color:#FF0000'>This is a permanent ban.</span>")
		to_chat(M, "\red To try to resolve this matter head to http:)
...
/client/proc/toggle_view_range()
	...
	//message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] changed their view range to [view].</span>", 1)	//why? removed by order of XSI
...
/client/proc/cmd_admin_attack_log(mob/M as mob in mob_list)
	...
	to_chat(usr, text("<span style='color:#FF0000'><b>Attack Log for []</b></span>", mob))
...
/client/proc/everyone_random()
	...
	if(notifyplayers == "Yes")
		to_chat(world, "<span style='color:#0000FF'><b>Admin [usr.key] has forced the players to have completely random identities!</span>")
