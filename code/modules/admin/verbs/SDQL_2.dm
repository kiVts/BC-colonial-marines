...
/proc/SDQL_testout(list/query_tree, indent = 0)
	...
	for(var/item in query_tree)
		if(istype(item, /list))
			to_chat(world, "[spaces](")
			SDQL_testout(item, indent + 1)
			to_chat(world, "[spaces])")

		else
			to_chat(world, "[spaces][item]")
		...
		if(!isnum(item) && query_tree[item])
			...
			if(istype(query_tree[item], /list))
				...
				to_chat(world, "[spaces]    [query_tree[item]]")
		...
		if(op != "")
			switch(op)
				...
				if("or", "||")
					...
					to_chat(usr, "<span style='color:#FF0000'>SDQL2: Unknown op [op]</span>")
					result = null
		else
			result = val
...
/proc/SDQL2_tokenize(query_text)
	...
	for(var/i = 1, i <= len, i++)
		...
		if(char in whitespace)
			...
			if(word != "")
				to_chat(usr, "<span style='color:#FF0000'>SDQL2: You have an error in your SDQL syntax, unexpected ' in query: \"<font color=gray>[query_text]</font>\" following \"<font color=gray>[word]</font>\". Please check your syntax, and try again.</span>")
				return null
			...
			if(i > len)
				to_chat(usr, "<span style='color:#FF0000'>SDQL2: You have an error in your SDQL syntax, unmatched ' in query: \"<font color=gray>[query_text]</font>\". Please check your syntax, and try again.</span>")
				return null
