...
var/list/admin_verbs_admin = list(
	...
	/client/proc/create_admin_fan_fact,
...
/client/proc/invisimin()
	...
	if(holder && mob)
		if(mob.invisibility == INVISIBILITY_OBSERVER)
			...
			to_chat(mob, "<span style='color:#FF0000'><b>Invisimin off. Invisibility reset.</b></span>")
			mob.alpha = max(mob.alpha + 100, 255)
			mob.add_to_all_mob_huds()
		else
			mob.invisibility = INVISIBILITY_OBSERVER
			to_chat(mob, "<span style='color:#0000FF'><b>Invisimin on. You are now as invisible as a ghost.</b></span>")
...
/client/proc/drop_bomb() // Some admin dickery that can probably be done better -- TLE
	...
	message_admins("<span style='color:#0000FF'>[ckey] used 'Drop Bomb' at [epicenter.loc].</span>")
...
/client/proc/give_spell(mob/T as mob in mob_list) // -- Urist
	...
	message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] gave [key_name(T)] the spell [S].</span>", 1)
...
/client/proc/give_disease(mob/T as mob in mob_list) // -- Giacom
	...
	message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] gave [key_name(T)] the disease [D].</span>", 1)
...
/client/proc/make_sound(var/obj/O in object_list) // -- TLE
	...
	if(O)
		...
		message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] made [O] at [O.x], [O.y], [O.z]. make a sound</span>", 1)
...
/client/proc/kill_air() // -- TLE
	...
	message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] used 'kill air'.</span>", 1)
*/
...
/client/proc/editappear(mob/living/carbon/human/M as mob in mob_list)
	...
	if(!istype(M, /mob/living/carbon/human))
		to_chat(usr, "<span style='color:#FF0000'>You can only do this to humans!</span>")
		return
...
/client/proc/man_up(mob/T as mob in mob_list)
	...
	message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] told [key_name(T)] to man up and deal with it.</span>", 1)
...
/client/proc/global_man_up()
	...
	message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] told everyone to man up and deal with it.</span>", 1)
*/
/client/proc/change_security_level()
	set name = "Set Security Level"
	set desc = "Sets the almayer security level"
	set category = "Fun"
...
/client/proc/delete_away_timer()
	set name = "Delete Xeno Away Timer in View"
	set desc = "Set the away_timer of all clientless Xenos in view to 0 to allow players to become them."
	set category = "Fun"
	if(!holder)	return

	if(alert("Are you sure you want to set the away_timer of all visible Xenos to 0? Make sure there aren't any visible AFK Xenos with players that might return!",, "Confirm", "Cancel") == "Cancel") return

	for(var/mob/living/carbon/Xenomorph/X in view())
		if(X.client) continue
		X.away_timer = 0

	log_admin("Admin [key_name_admin(usr)] set the away_timer of nearby clientless Xenos to 0.", 1)
	message_admins("<b>[key_name(src)]</b> set the away_timer of nearby clientless Xenos to 0.", 1)
