...
/datum/admins/proc/log_admin_rank_modification(var/adm_ckey, var/new_rank)
	...
	if(!usr.client.holder || !(usr.client.holder.rights & R_PERMISSIONS))
		to_chat(usr, "<span style='color:#FF0000'>You do not have permission to do this!</span>")
		return
	...
	if(new_admin)
		...
		log_query.Execute()
		to_chat(usr, "<span style='color:#0000FF'>New admin added.</span>")
	else
		if(!isnull(admin_id) && isnum(admin_id))
			...
			log_query.Execute()
			to_chat(usr, "<span style='color:#0000FF'>Admin rank changed.</span>")
...
/datum/admins/proc/log_admin_permission_modification(var/adm_ckey, var/new_permission)
	...
	if(admin_rights & new_permission) //This admin already has this permission, so we are removing it.
		...
		log_query.Execute()
		to_chat(usr, "<span style='color:#0000FF'>Permission removed.</span>")
		...
		log_query.Execute()
		to_chat(usr, "<span style='color:#0000FF'>Permission added.</span>")