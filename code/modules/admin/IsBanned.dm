...
world/IsBanned(key,address,computer_id)
	...
	if(!guests_allowed && IsGuestKey(key))
		...
		message_admins("<span style='color:#0000FF'>Failed Login: [key] - Guests not allowed</span>")
		return list("reason"="guest", "desc"="\nReason: Guests not allowed. Please sign in with a byond account.")

	//check if the IP address is a known TOR node
	if(config && config.ToRban && ToRban_isbanned(address))
		log_access("Failed Login: [src] - Banned: ToR")
		message_admins("<span style='color:#0000FF'>Failed Login: [src] - Banned: ToR</span>")
	...
	if(config.ban_legacy_system)
		...
		if(.)
			...
			message_admins("<span style='color:#0000FF'>Failed Login: [key] id:[computer_id] ip:[address] - Banned [.["reason"]]</span>")
			return .
