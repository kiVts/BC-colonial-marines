...
/client/verb/ooc(msg as text)
	...
	if(!(prefs.toggles_chat & CHAT_OOC))
		to_chat(src, "<span style='color:#FF0000'>You have OOC muted.</span>")
		return

	if(!holder)
		if(!ooc_allowed)
			to_chat(src, "<span style='color:#FF0000'>OOC is globally muted</span>")
			return
		if(!dooc_allowed && (mob.stat == DEAD))
			to_chat(usr, "<span style='color:#FF0000'>OOC for dead mobs has been turned off.</span>")
			return
		if(prefs.muted & MUTE_OOC)
			to_chat(src, "<span style='color:#FF0000'>You cannot use OOC (muted).</span>")
			return
...
/client/verb/looc(msg as text)
	...
	if(!(prefs.toggles_chat & CHAT_LOOC))
		to_chat(src, "<span style='color:#FF0000'>You have LOOC muted.</span>")
		return

	if(!holder)
		if(!looc_allowed)
			to_chat(src, "<span style='color:#FF0000'>LOOC is globally muted</span>")
			return
		if(!dlooc_allowed && (mob.stat == DEAD))
			to_chat(usr, "<span style='color:#FF0000'>LOOC for dead mobs has been turned off.</span>")
			return
		if(prefs.muted & MUTE_OOC)
			to_chat(src, "<span style='color:#FF0000'>You cannot use LOOC (muted).</span>")
			return
		if(handle_spam_prevention(msg,MUTE_OOC))
			return
		if(findtext(msg, "byond://"))
...
/client/verb/motd()
	...
	if( join_motd )
		...
		to_chat(src, "<span style='color:#FF0000'>The motd is not set in the server configuration.</span>")
	return
