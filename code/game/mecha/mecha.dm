	...
	if(istype(user,/mob/living/carbon/human))
		...
		if(H.species.can_shred(user))
			if(!prob(src.deflect_chance))
				...
				to_chat(user, "<span style='color:#FF0000'>You slash at the armored suit!</span>")
				visible_message("<span style='color:#FF0000'>[user] slashes at [name]'s armor!</span>")
			else
				src.log_append_to_last("Armor saved.")
				playsound(src.loc, 'sound/weapons/slash.ogg', 25, 1)
				to_chat(user, "\green Your claws had no effect!")
				src.occupant_message("<span style='color:#0000FF'>The [user]'s claws are stopped by the armor.</span>")
				visible_message("<span style='color:#0000FF'>[user] rebounds off [name]'s armor!</span>")
		else
...
/obj/mecha/attack_animal(mob/living/user as mob)
	...
	if(user.melee_damage_upper == 0)
		...
		if(!prob(src.deflect_chance))
			...
			src.check_for_internal_damage(list(MECHA_INT_CONTROL_LOST))
			visible_message("<span style='color:#FF0000'><B>[user]</B> [user.attacktext] [src]!</span>")
			log_combat(user, src, "attacked")
		else
			src.log_append_to_last("Armor saved.")
			playsound(src.loc, 'sound/weapons/slash.ogg', 25, 1)
			src.occupant_message("<span style='color:#0000FF'>The [user]'s attack is stopped by the armor.</span>")
			visible_message("<span style='color:#0000FF'>The [user] rebounds off [src.name]'s armor!</span>")
			log_combat(user, src, "attacked")
	return
...
/obj/mecha/proc/dynhitby(atom/movable/A)
	...
	if(prob(src.deflect_chance) || istype(A, /mob))
		src.occupant_message("<span style='color:#0000FF'>The [A] bounces off the armor.</span>")
...
/obj/mecha/proc/dynbulletdamage(var/obj/item/projectile/Proj)
	if(prob(src.deflect_chance))
		src.occupant_message("<span style='color:#0000FF'>The armor deflects incoming projectile.</span>")
...
/obj/mecha/proc/dynattackby(obj/item/W as obj, mob/user as mob)
	...
	if(prob(src.deflect_chance))
		to_chat(user, "<span style='color:#FF0000'>\The [W] bounces off [src.name].</span>")
		src.log_append_to_last("Armor saved.")
	...
	if(istype(W, /obj/item/card/id)||istype(W, /obj/item/device/pda))
		if(add_req_access || maint_access)
			if(internals_access_allowed(usr))
				...
				to_chat(user, "<span style='color:#FF0000'>Invalid ID: Access denied.</span>")
		else
			to_chat(user, "<span style='color:#FF0000'>Maintenance protocols disabled by operator.</span>")
		...
		if(src.health<initial(src.health))
			to_chat(user, "<span style='color:#0000FF'>You repair some damage to [src.name].</span>")
...
/obj/mecha/verb/connect_to_port()
	...
	if(possible_port)
		if(connect(possible_port))
			src.occupant_message("<span style='color:#0000FF'>[name] connects to the port.</span>")
			src.verbs += /obj/mecha/verb/disconnect_from_port
			src.verbs -= /obj/mecha/verb/connect_to_port
			return
		else
			src.occupant_message("<span style='color:#FF0000'>[name] failed to connect to the port.</span>")
			return
	else
...
/obj/mecha/verb/disconnect_from_port()
	...
	if(usr!=src.occupant)
		return
	if(disconnect())
		src.occupant_message("<span style='color:#0000FF'>[name] disconnects from the port.</span>")
		src.verbs -= /obj/mecha/verb/disconnect_from_port
		src.verbs += /obj/mecha/verb/connect_to_port
	else
		src.occupant_message("<span style='color:#FF0000'>[name] is not connected to the port at the moment.</span>")
...
/obj/mecha/verb/move_inside()
	...
	if(iscarbon(usr))
		...
		if(C.handcuffed)
			to_chat(usr, "<span style='color:#FF0000'>Kinda hard to climb in while handcuffed don't you think?</span>")
			return
	if (src.occupant)
		to_chat(usr, "<span style='color:#0000FF'><B>The [src.name] is already occupied!</B></span>")
		src.log_append_to_last("Permission denied.")
		return
/*
	if (usr.abiotic())
		to_chat(usr, "<span style='color:#0000FF'><B>Subject cannot have abiotic items on.</B></span>")
		return
*/
	var/passed
	if(src.dna)
		...
		passed = 1
	if(!passed)
		to_chat(usr, "<span style='color:#FF0000'>Access denied</span>")
		src.log_append_to_last("Permission denied.")
		return
//	to_chat(usr, "You start climbing into [src.name]")

	visible_message("<span style='color:#0000FF'>[usr] starts to climb into [src.name]</span>")
...
//	to_chat(user, "Installing MMI, please stand by.")
	...
	visible_message("<span style='color:#0000FF'>[usr] starts to insert an MMI into [src.name]</span>")
...
/obj/mecha/Topic(href, href_list)
	...
	if(href_list["remove_passenger"] && state >= 1)
		...
		if (!passengers)
			to_chat(user, "<span style='color:#FF0000'>There are no passengers to remove.</span>")
			return
		...
		user.visible_message("<span style='color:#FF0000'>[user] begins opening the hatch on \the [P]...", "\red You begin opening the hatch on \the [P]...</span>")
		if (!do_after(user, 40, FALSE, 5, BUSY_ICON_GENERIC))
			return

		user.visible_message("<span style='color:#FF0000'>[user] opens the hatch on \the [P] and removes [occupant]!", "\red You open the hatch on \the [P] and remove [occupant]!</span>")
		P.go_out()
