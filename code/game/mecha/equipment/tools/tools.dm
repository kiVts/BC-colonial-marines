	...
	action(atom/target)
		...
		if(istype(target,/obj))
			...
			if(chassis.occupant.a_intent == "hurt")
				...
				M.updatehealth()
				occupant_message("<span style='color:#FF0000'>You squeeze [target] with the [src.name]!</span>")
				chassis.visible_message("<span style='color:#FF0000'>[chassis] squeezes [target]!</span>")
	...
	action(atom/target) //copypasted from extinguisher. TODO: Rewrite from scratch.
		...
		if(do_after_cooldown(target))
			if( istype(target, /obj/structure/reagent_dispensers/watertank) && get_dist(chassis,target) <= 1)
				...
				occupant_message("<span style='color:#0000FF'>\The [src] is now refilled</span>")
				playsound(chassis, 'sound/effects/refill.ogg', 25, 1, 4)
				return

			if (src.reagents.total_volume < 1)
				occupant_message("<span style='color:#FF0000'>\The [src] is empty.</span>")
				return
...
/obj/item/mecha_parts/mecha_equipment/anticcw_armor_booster //what is that noise? A BAWWW from TK mutants.
	...
	proc/dynattackby(obj/item/W as obj, mob/user as mob)
		...
		if(prob(chassis.deflect_chance*deflect_coeff))
			to_chat(user, "<span style='color:#FF0000'>The [W] bounces off [chassis] armor.</span>")
...
/obj/item/mecha_parts/mecha_equipment/antiproj_armor_booster
	...
	proc/dynbulletdamage(var/obj/item/projectile/Proj)
		...
		if(prob(chassis.deflect_chance*deflect_coeff))
			chassis.occupant_message("<span style='color:#0000FF'>The armor deflects incoming projectile.</span>")
	...
	proc/dynhitby(atom/movable/A)
		...
		if(prob(chassis.deflect_chance*deflect_coeff) || istype(A, /mob/living) || istype(A, /obj/item/mecha_parts/mecha_tracking))
			chassis.occupant_message("<span style='color:#0000FF'>The [A] bounces off the armor.</span>")
...
/obj/item/mecha_parts/mecha_equipment/tool/safety_clamp
	...
	action(atom/target)
		...
		if(istype(target,/obj))
			...
			if(M.stat>1) return
			if(chassis.occupant.a_intent == "hurt")
				chassis.occupant_message("<span style='color:#FF0000'>You obliterate [target] with [src.name], leaving blood and guts everywhere.</span>")
				chassis.visible_message("<span style='color:#FF0000'>[chassis] destroys [target] in an unholy fury.</span>")
			if(chassis.occupant.a_intent == "disarm")
				chassis.occupant_message("<span style='color:#FF0000'>You tear [target]'s limbs off with [src.name].</span>")
				chassis.visible_message("<span style='color:#FF0000'>[chassis] rips [target]'s arms off.</span>")
			else
				step_away(M,chassis)
...
/obj/item/mecha_parts/mecha_equipment/tool/passenger/proc/move_inside(var/mob/user)
	if (chassis)
		chassis.visible_message("<span style='color:#0000FF'>[user] starts to climb into [chassis].</span>")
	...
	if(do_after(user, 40, FALSE, 5, BUSY_ICON_GENERIC))
		if(!src.occupant)
			...
			to_chat(user, "<span style='color:#FF0000'>[src.occupant] was faster. Try better next time, loser.</span>")
	else
...
/obj/mecha/proc/move_inside_passenger()
	...
	if (!isturf(usr.loc))
		to_chat(usr, "<span style='color:#FF0000'>You can't reach the passenger compartment from here.</span>")
		return
	...
	switch (feedback)
		if (OCCUPIED)
			to_chat(usr, "<span style='color:#FF0000'>The passenger compartment is already occupied!</span>")
		if (LOCKED)
			to_chat(usr, "<span style='color:#FF0000'>The passenger compartment hatch is locked!</span>")
		if (OCCUPIED|LOCKED)
			to_chat(usr, "<span style='color:#FF0000'>All of the passenger compartments are already occupied or locked!</span>")
		if (0)
			to_chat(usr, "<span style='color:#FF0000'>\The [src] doesn't have a passenger compartment.</span>")
