...
/obj/machinery/dna_scannernew/New()
	...
	component_parts += new /obj/item/circuitboard/machine/clonescanner(src)
...
/obj/machinery/dna_scannernew/proc/eject_occupant()
	...
	for(var/obj/O in src)
		if((!istype(O,/obj/item/reagent_container)) && (!istype(O,/obj/item/circuitboard/machine/clonescanner)) && (!istype(O,/obj/item/stock_parts)) && (!istype(O,/obj/item/stack/cable_coil)))
...
/obj/machinery/dna_scannernew/verb/move_inside()
	...
	if (!ishuman(usr) && !ismonkey(usr)) //Make sure they're a mob that has dna
		to_chat(usr, "<span style='color:#0000FF'>Try as you might, you can not climb up into the scanner.</span>")
		return
	if (src.occupant)
		to_chat(usr, "<span style='color:#0000FF'><B>The scanner is already occupied!</B></span>")
		return
	if (usr.abiotic())
...
/obj/machinery/dna_scannernew/attackby(var/obj/item/item as obj, var/mob/user as mob)
	if(istype(item, /obj/item/reagent_container/glass))
		if(beaker)
			to_chat(user, "<span style='color:#FF0000'>A beaker is already loaded into the machine.</span>")
			return
	...
	if (src.occupant)
		to_chat(user, "<span style='color:#0000FF'><B>The scanner is already occupied!</B></span>")
		return
	if (M.abiotic())
		to_chat(user, "<span style='color:#0000FF'><B>Subject cannot have abiotic items on.</B></span>")
		return
	put_in(M)
...
/obj/machinery/computer/scan_consolenew/Topic(href, href_list)
	...
	if(href_list["ejectBeaker"])
		if(connected.beaker)
			var/obj/item/reagent_container/glass/B = connected.beaker
			B.loc = connected.loc
