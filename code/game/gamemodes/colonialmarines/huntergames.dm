...
/datum/game_mode/huntergames/declare_completion()
	...
	if(finished == 1 && !isnull(winner) && istype(winner))
		...
		to_chat(world, "<span style='color:#FF0000'><FONT size = 4><B>We have a winner! >> [winner.real_name] ([winner.key]) << defeated all enemies!</B></FONT></span>")
		...
		to_chat(world, "<span style='color:#FF0000'><FONT size = 4><B>NOBODY WON!?</B></FONT></span>")
