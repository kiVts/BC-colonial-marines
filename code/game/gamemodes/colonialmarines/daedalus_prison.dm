...
/datum/game_mode/prison_rescue/can_start()
...
/datum/game_mode/prison_rescue/pre_setup()
	return 1
...
/datum/game_mode/prison_rescue/post_setup()
...
/datum/game_mode/prison_rescue/proc/transform_xeno(var/datum/mind/ghost)
	...
	to_chat(new_xeno, "<B>Your job is to spread the hive and protect the Queen. You can become the Queen yourself by evolving into a drone.</B>")
...
/datum/game_mode/prison_rescue/proc/transform_survivor(var/datum/mind/ghost)
...
//Give them proper jobs and stuff here later
	...
	spawn(4)
		...
		to_chat(H, "<span style='color:#0000FF'>You are a survivor of the attack on the Daedalus Prison facility. You worked or lived in the prison, and managed to avoid the alien attacks.. until now.</span>")
...
/datum/game_mode/prison_rescue/process()
...
/datum/game_mode/prison_rescue/proc/count_humans()
	var/human_count = 0
...
/datum/game_mode/prison_rescue/proc/count_xenos()
	var/xeno_count = 0
...
//////////////////////////
/datum/game_mode/prison_rescue/check_win()
...
///////////////////////////////
/datum/game_mode/prison_rescue/check_finished()
	if(finished != 0)
		return 1
...
/datum/game_mode/prison_rescue/declare_completion()
	if(finished == 1)
		...
		to_chat(world, "<span style='color:#FF0000'><FONT size = 4><B>Alien major victory!</B></FONT></span>")
		...
		to_chat(world, "<span style='color:#FF0000'><FONT size = 4><B>Marines major victory!</B></FONT></span>")
		...
		to_chat(world, "<span style='color:#FF0000'><FONT size = 3><B>Marine minor victory.</B></FONT></span>")
		...
		to_chat(world, "<span style='color:#FF0000'><FONT size = 3><B>Alien minor victory.</B></FONT></span>")
		...
		to_chat(world, "<span style='color:#FF0000'><FONT size = 3><B>Draw.</B></FONT></span>")
		to_chat(world, "<FONT size = 3><B>The [MAIN_SHIP_NAME] has blown by a nuclear fission device... there are no winners!</B></FONT>")
		world << 'sound/misc/sadtrombone.ogg'
		log_game("Draw (Nuke)\nXenos Remaining: [count_xenos()]. Humans remaining: [count_humans()]\nRound time: [duration2text()]")
	else
		to_chat(world, "<span style='color:#FF0000'>Whoops, something went wrong with declare_completion(), finished: [finished]. Blame the coders!</span>")
...
/datum/game_mode/proc/auto_declare_completion_prison_rescue()
	return
