/datum/game_mode/whiskey_outpost
	...
	var/spawn_next_wave = 600 //Spawn first batch at ~15 minutes //200
...
/datum/game_mode/whiskey_outpost/proc/spawn_player(var/mob/M)
	...
	switch(H.mind.assigned_role)
		...
		if("Executive Officer")
			...
			var/obj/item/card/id/gold/W = new(H)
			W.name = "[M.real_name]'s ID Card (Lieutenant Commander)"
			W.access = get_all_accesses()
			W.assignment = "Lieutenant Commander"
			W.paygrade = "O2"
			W.registered_name = M.real_name
			H.equip_to_slot_or_del(W, WEAR_ID)
			...
			if(H)
				to_chat(H, "________________________")
				to_chat(H, "<span class='boldnotice'>You are the Dust Raiders Lieutenant Commander!</span>")
				to_chat(H, "Coordinate your team and prepare defenses.")
				to_chat(H, "Stay alive!")
				to_chat(H, "Hold the outpost for one hour until the distress beacon can be broadcast to the remaining Dust Raiders!")
				to_chat(H, "The local population warned you about establishing a base in the jungles of [map_tag]...")
		...
		if("Maintenance Tech")
			...
			var/obj/item/card/id/W = new(H)
			W.name = "[M.real_name]'s ID Card (Outpost Bunker Crew)"
			W.access = get_all_accesses()
			W.assignment = "Outpost Bunker Crew"
			W.paygrade = "E6E"
			W.registered_name = M.real_name
			H.equip_to_slot_or_del(W, WEAR_ID)
			...
			if(H)
				to_chat(H, "________________________")
				to_chat(H, "<span class='boldnotice'>You are a Dust Raiders Bunker Crew!</span>")
		...
		if("Vehicle Crew")
			...
			var/obj/item/card/id/W = new(H)
			W.name = "[M.real_name]'s ID Card (Outpost Bunker Vehicle Crew)"
			W.access = get_all_accesses()
			W.assignment = "Outpost Bunker Vehicle Crew"
			W.paygrade = "01"
			W.registered_name = M.real_name
			H.equip_to_slot_or_del(W, WEAR_ID)
			...
			if(H)
				to_chat(H, "________________________")
				to_chat(H, "<span class='boldnotice'>You are a Dust Raiders Bunker Vehicle Crew!</span>")
				to_chat(H, "Tank up, prepare!")
				to_chat(H, "Stay alive!")
				to_chat(H, "Aid your comrades!")
				to_chat(H, "________________________")
			sleep(240) //So they can see it
		...
		if("Requisitions Officer")
			...
			var/obj/item/card/id/W = new(H)
			W.name = "[M.real_name]'s ID Card (Outpost Bunker Quarter Master)"
			W.access = get_all_accesses()
			W.assignment = "Outpost Bunker Quarter Master"
			W.paygrade = "E7E"
			W.registered_name = M.real_name
			H.equip_to_slot_or_del(W, WEAR_ID)
			...
			if(H)
				to_chat(H, "________________________")
				to_chat(H, "<span class='boldnotice'>You are a Dust Raiders Bunker Quarter Master!</span>")
				to_chat(H, "Gear up, prepare!")
				to_chat(H, "Stay alive!")
				to_chat(H, "Aid your comrades!")
				to_chat(H, "________________________")
			sleep(240) //So they can see it
		...
		if("Cargo Technician")
			...
			var/obj/item/card/id/W = new(H)
			W.name = "[M.real_name]'s ID Card (Outpost Bunker Crew Logistics)"
			W.access = get_all_accesses()
			W.assignment = "Outpost Bunker Crew Logistics"
			W.paygrade = "E6E"
			W.registered_name = M.real_name
			H.equip_to_slot_or_del(W, WEAR_ID)
			...
			if(H)
				to_chat(H, "________________________")
				to_chat(H, "<span class='boldnotice'>You are a Dust Raiders Bunker Crew Logistics!</span>")
				to_chat(H, "Gear up, prepare!")
				to_chat(H, "Stay alive!")
				to_chat(H, "Aid your comrades!")
				to_chat(H, "________________________")
...
/datum/game_mode/whiskey_outpost/process()
	...
	if(wave_ticks_passed >= spawn_next_wave)
		if(count_xenos() < 50)//Checks braindead too, so we don't overpopulate! Also make sure its less than twice us in the world, so we advance waves/get more xenos the more marines survive.
			...
			to_chat(world, "<span style='color:#0000FF'><b>Wave:</b> [xeno_wave][wave_times_delayed?"|\red Times delayed: [wave_times_delayed]":""]</span>")
			to_chat(world, "<span class='notice'>*___________________________________*</span>")
			to_chat(world, "<br><br>")
			to_chat(world, "<br><br>")
...
/datum/game_mode/whiskey_outpost/check_win()
	...
	else if(world.time > 79000 + lobby_time)
...
//			to_chat(world, "Loc: 4")
	...
	switch(xeno_wave)//Xeno spawn controller
		if(1)//Mostly weak runners
			...
			spawn_xeno_num = (humans_alive * 2) //Reset
			spawn_next_wave = 600
		...
		if(8)
			spawn_next_wave += 300 //Slow down now, strong castes introduced next wave
		...
		if(9)//Ravager and Praetorian Added, Tier II more common, Tier I less common
			spawn_next_wave -= 500 //Speed it up again. After the period of grace.
				...
				if(0 to 5)//Normal list, but makes it easier to pick stronger units
					switch(random_wave)
						if(0)//Add another Ravager
							spawnxeno += list(/mob/living/carbon/Xenomorph/Ravager/titan)
						if(1)//Add another Carrier
							spawnxeno += list(/mob/living/carbon/Xenomorph/Carrier/titan)
						if(2)//Add another Praetorian
							spawnxeno += list(/mob/living/carbon/Xenomorph/Praetorian/titan)
						if(3)//Add another Boiler
							spawnxeno += list(/mob/living/carbon/Xenomorph/Boiler/titan)
						if(4)//Add another Crusher
							spawnxeno += list(/mob/living/carbon/Xenomorph/Crusher/titan)
						if(5)//Add another Hunter and Spitter
							spawnxeno += list(/mob/living/carbon/Xenomorph/Lurker/titan,
										/mob/living/carbon/Xenomorph/Spitter/titan)
...
/datum/game_mode/whiskey_outpost/proc/CleanXenos()//moves dead xenos to space
	...
	if(xeno_count)
		to_chat(world, "<span class='notice'>*___________________________________*</span>")
		to_chat(world, "<span class='boldnotice'>***Whiskey Outpost Controller***</span>")
		to_chat(world, "<span style='color:#0000FF'>Moved [xeno_count] Xeno remains to trash.</span>")
		to_chat(world, "<span class='notice'>*___________________________________*</span>")
...
/obj/machinery/wo_recycler
	...
	attack_hand(mob/user)
		...
		if(istype(usr, /mob/living/silicon) || \
			...
			to_chat(usr, "<span style='color:#FF0000'>You don't have the dexterity to do this!</span>")
			return
		if(working)
			to_chat(user, "<span style='color:#FF0000'>Wait for it to recharge first.</span>")
			return

		var/remove_max = 10
		var/turf/T = src.loc
		if(T)
			to_chat(user, "<span style='color:#FF0000'>You turn on the recycler.</span>")
			var/removed = 0
			for(var/i, i < remove_max, i++)
				for(var/obj/O in T)
					if(istype(O,/obj/structure/closet/crate))
						...
						if(C.contents.len)
							to_chat(user, "<span style='color:#FF0000'>[O] must be emptied before it can be recycled</span>")
							continue
