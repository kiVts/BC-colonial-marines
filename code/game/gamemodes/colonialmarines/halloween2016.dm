...
/obj/effect/blocker/fog
	name = "dense fog"
	desc = "Густой туман. Лучший способ ждать пока он спадёт."
...
/obj/effect/blocker/fogtox
	name = "dense toxic fog"
	desc = "Очень густой опасный зелёный туман."
	icon = 'icons/effects/effects.dmi'
	icon_state = "smoke"
	color = "#db3413"
	anchored = 1
...
/obj/effect/blocker/fogtox/New()
	..()

/obj/effect/blocker/fogtox/Entered(mob/living/M)
	..()
	if(!istype(M.wear_mask, /obj/item/clothing/mask/gas))
		M.adjustOxyLoss(5) //Basic oxyloss from "can't breathe"
		M.adjustFireLoss(1*rand(10, 15)) //Inhalation damage
	...
	if(ishuman(M))
		...
		H.adjustFireLoss(1*rand(15, 20)) //Burn damage, randomizes between various parts //Amount corresponds to upgrade level, 1 to 2.5
	else
		...
		if("Corporate Liaison") //Lead the way, corporate drone!
			...
			spawn(40)
				if(H)
					...
					to_chat(H, "<span style='color:#FF0000'><b>You are the [H.mind.assigned_role]!<b></span>")
