...
/datum/game_mode/colonialmarines/announce()
	to_chat(world, "<span class='round_header'>В данный момент карта - [map_tag]!</span>")
	var/musical_track = pick('sound/AI/welcome.ogg')
...
/*
/obj/effect/landmark/lv624/block_hellhound
	name = "block_hellhound"
	icon_state = "spawn_event"*/
...
/datum/game_mode/colonialmarines/pre_setup()
	...
	for(var/turf/T in map_items)
		...
		switch(map_tag)
			...
			if(MAP_DESERT_DAM) new /obj/item/map/big_red_map(T)
			if(MAP_DECALM_SLAY) new /obj/item/map/decalm_slay_map(T)
			if(MAP_DEAD_WOOD) new /obj/item/map/dead_wood_map(T)
			if(MAP_SECRET_OBJECT) new /obj/item/map/secret_object_map(T)
	...
	if(monkey_amount)
		...
		switch(map_tag)
			...
			if(MAP_DESERT_DAM) monkey_types = list(/mob/living/carbon/monkey/tajara, /mob/living/carbon/monkey/skrell)
			if(MAP_DECALM_SLAY) monkey_types = list(/mob/living/carbon/monkey, /mob/living/carbon/monkey/tajara, /mob/living/carbon/monkey/unathi, /mob/living/carbon/monkey/skrell)
			if(MAP_DEAD_WOOD) monkey_types = list(/mob/living/carbon/monkey, /mob/living/carbon/monkey/skrell)
			if(MAP_SECRET_OBJECT) monkey_types = list(/mob/living/carbon/monkey)
	...
	spawn (50)
		switch(map_tag)
			...
			if(MAP_DESERT_DAM)
				command_announcement.Announce("We've lost contact with the Weyland-Yutani's research facility, [map_tag]. The [MAIN_SHIP_NAME] has been dispatched to assist.", "[MAIN_SHIP_NAME]")
			if(MAP_DECALM_SLAY)
				command_announcement.Announce("We've lost contact with the Weyland-Yutani's secret project, [map_tag]. The [MAIN_SHIP_NAME] has been dispatched to assist.", "[MAIN_SHIP_NAME]")
			if(MAP_SECRET_OBJECT)
				command_announcement.Announce("We've lost contact with the Weyland-Yutani's secret universal object, [map_tag]. The [MAIN_SHIP_NAME] has been dispatched to assist.", "[MAIN_SHIP_NAME]")
			if(MAP_DEAD_WOOD)
				command_announcement.Announce("We've lost contact with the Weyland-Yutani's toxic factory 'secret weapon research center', [map_tag]. The [MAIN_SHIP_NAME] has been dispatched to assist.", "[MAIN_SHIP_NAME]")
...
/datum/game_mode/colonialmarines/declare_completion()
	...
	to_chat(world, "<span class='round_body'>Thus ends the story of the brave men and women of the [MAIN_SHIP_NAME] , Defcon level ? , and their struggle on [map_tag].</span>")
	world << "<span class='round_body'>End of Round Grief (EORG) is an IMMEDIATE 3 hour ban with no warnings.</span>"
	var/musical_track
	switch(round_finished)
		if(MODE_INFESTATION_X_MAJOR) musical_track = pick('sound/theme/sad_loss1.ogg','sound/theme/sad_loss2.ogg','sound/theme/gyl_vetra_po_pustymo_corably.ogg')
		if(MODE_INFESTATION_M_MAJOR) musical_track = pick('sound/theme/winning_triumph1.ogg','sound/theme/winning_triumph2.ogg')
		if(MODE_INFESTATION_X_MINOR) musical_track = pick('sound/theme/neutral_melancholy1.ogg','sound/theme/neutral_melancholy2.ogg','sound/theme/water_horror.ogg')
		if(MODE_INFESTATION_M_MINOR) musical_track = pick('sound/theme/neutral_hopeful1.ogg','sound/theme/neutral_hopeful2.ogg','sound/theme/the_glory_of_victory_v2.ogg')
