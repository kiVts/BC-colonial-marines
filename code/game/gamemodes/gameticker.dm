...
/datum/controller/gameticker/proc/setup()
	...
	if(config.autooocmute)
		to_chat(world, "<span style='color:#FF0000'><B>The OOC channel has been globally disabled due to round start!</B></span>")
		...
		if(!EvacuationAuthority.dest_status != NUKE_EXPLOSION_IN_PROGRESS && game_finished && (mode_finished || post_game))
			...
			spawn(50)
				...
				if(config.autooocmute && !ooc_allowed)
					to_chat(world, "<span style='color:#FF0000'><B>The OOC channel has been globally enabled due to round end!</B></span>")
					ooc_allowed = 1
			...
			spawn(50)
				if(!round_end_announced) // Spam Prevention. Now it should announce only once.
					to_chat(world, "<span style='color:#FF0000'>The round has ended!</span>")
