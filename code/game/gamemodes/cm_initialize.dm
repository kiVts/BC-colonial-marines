...
/mob/verb/adjust_predator_round()
	...
	ticker.mode.pred_maximum_num = input(src,"What is the new maximum number of predators?","Input:",2) as num|null
...
/datum/game_mode/proc/check_xeno_late_join(mob/xeno_candidate)
	if(jobban_isbanned(xeno_candidate, "Alien")) // User is jobbanned
		to_chat(xeno_candidate, "<span class='warning'>¬ы забанены за ксеноморфа, вы не можете появится.</span>")
		return
	return 1

/datum/game_mode/proc/attempt_to_join_as_larva(mob/xeno_candidate)
	if(!ticker.mode.stored_larva)
		to_chat(xeno_candidate, "<span class='warning'>Ќет лярв.</span>")
		return FALSE
	...
	if(!available_queens.len)
		to_chat(xeno_candidate, "<span class='warning'>Ќет королевы на яйцекладе.</span>")
		return FALSE
	...
	if (!istype(mother) || !xeno_candidate || !xeno_candidate.client)
		return FALSE
	if(!ticker.mode.stored_larva)
		to_chat(xeno_candidate, "<span class='warning'>Ќету больше лярв.</span>")
		return FALSE
	if(!mother.ovipositor || mother.is_mob_incapacitated(TRUE))
		to_chat(xeno_candidate, "<span class='warning'> оролева не в состоянии принять вас.</span>")
		return FALSE
	if(!xeno_bypass_timer && !istype(xeno_candidate, /mob/new_player))
		...
		if(deathtime < 3000 && ( !xeno_candidate.client.holder || !(xeno_candidate.client.holder.rights & R_ADMIN)) )
			to_chat(xeno_candidate, "<span class='warning'>¬ы умерли [deathtimeminutes >= 1 ? "[deathtimeminutes] minute\s and " : ""][deathtimeseconds] second\s назад.</span>")
			to_chat(xeno_candidate, "<span class='warning'>¬ы должны подождать 5 минут после смерти для того чтобы вернуться в игру!</span>")
			return FALSE
	return mother
...
/datum/game_mode/proc/spawn_larva(mob/xeno_candidate, var/mob/living/carbon/Xenomorph/Queen/mother)
	...
	if(!ticker.mode.stored_larva || !mother || !istype(mother))
		to_chat(xeno_candidate, "<span class='warning'>„то-то пошло не так. ¬ данный момент нельзя возродится.</span>")
	...
	new_xeno.visible_message("<span class='xenodanger'>Ћярва внезапно вылезает!</span>",
	"<span class='xenodanger'>¬ы пробуждает от сна и вырываетесь. «а улей!</span>")
	new_xeno << sound('sound/effects/xeno_newlarva.ogg')
	new_xeno.key = xeno_candidate.key
	if(new_xeno.client)
		new_xeno.client.change_view(world.view)
	to_chat(new_xeno, "<span class='xenoannounce'>“ы-лярва ксеноморфа, пробудившаяся ото сна!</span>")
	new_xeno << sound('sound/effects/xeno_newlarva.ogg')
...
/datum/game_mode/proc/attempt_to_join_as_xeno(mob/xeno_candidate, instant_join = 0)
	...
	if(!available_xenos.len || (instant_join && !available_xenos_non_ssd.len))
		to_chat(xeno_candidate, "<span class='warning'>Ќет никаких доступных ксеноморфов. ¬ы можете попробовать получить грудолома, переключив свою кандидатуру Xenomorph в настройках -> Toggle SpecialRole Candidacy.</span>")
	...
	if(!instant_join)
		...
		if(!(new_xeno in living_mob_list) || new_xeno.stat == DEAD)
			to_chat(xeno_candidate, "<span class='warning'>¬ы не можете присоединиться, если ксеноморф мертв.</span>")
			return

		if(new_xeno.client)
			to_chat(xeno_candidate, "<span class='warning'>Ётот ксеноморф уже занят.</span>")
			return
...
/datum/game_mode/proc/transform_xeno(datum/mind/ghost_mind)
	...
	if(is_queen)
		...
		to_chat(new_xeno, "Talk in Hivemind using <strong>:a or ;</strong> (e.g. ':aMy life for the Queen!')")
	else
...
/datum/game_mode/proc/transform_survivor(var/datum/mind/ghost)
	...
	switch(map_tag)
		...
		if(MAP_LV_624,MAP_BIG_RED,MAP_DESERT_DAM,MAP_DECALM_SLAY,MAP_DEAD_WOOD)
		...
		if(MAP_ICE_COLONY,MAP_SECRET_OBJECT)
...
//Give them proper jobs and stuff here later
		...
		if("UPP Agent") //UPP Agent
			...
			switch(id_assignment)
				if("Assistant")
					...
					H.equip_to_slot_or_del(new /obj/item/ammo_magazine/pistol/holdout(H.back), WEAR_IN_BACK)
					H.equip_to_slot_or_del(new /obj/item/storage/firstaid/adv(H.back), WEAR_IN_BACK)
					H.equip_to_slot_or_del(new /obj/item/research_disk/upp(H.back), WEAR_IN_BACK)
				if("Civilian")
					H.equip_to_slot_or_del(new /obj/item/clothing/under/pj/red(H), WEAR_BODY)
					H.equip_to_slot_or_del(new /obj/item/clothing/shoes/black(H), WEAR_FEET)
					H.equip_to_slot_or_del(new /obj/item/storage/backpack/satchel/norm(H), WEAR_BACK)
	...
	spawn(4)
		...
		switch(map_tag)
			if(MAP_PRISON_STATION)
				to_chat(H, "<span style='color:#0000FF'>You are a survivor of the attack on Fiorina Orbital Penitentiary. You worked or lived on the prison station, and managed to avoid the alien attacks.. until now.</span>")
			if(MAP_ICE_COLONY)
				to_chat(H, "<span style='color:#0000FF'>You are a survivor of the attack on the ice habitat. You worked or lived on the colony, and managed to avoid the alien attacks.. until now.</span>")
			else
				to_chat(H, "<span style='color:#0000FF'>You are a survivor of the attack on the colony. You worked or lived in the archaeology colony, and managed to avoid the alien attacks...until now.</span>")
		to_chat(H, "<span style='color:#0000FF'>You are fully aware of the xenomorph threat and are able to use this knowledge as you see fit.</span>")
	return 1
...
/datum/game_mode/proc/initialize_post_marine_gear_list()
	...
	for(var/X in attachment_vendors)
		...
		A.products = list(
					...
					/obj/item/attachable/scope/mini = round(scale * 8),
					/obj/item/attachable/flashlight = round(scale * 14),
					/obj/item/attachable/reddot = round(scale * 14),
					/obj/item/attachable/scope/iftsscope = round(scale * 4),
					...
					/obj/item/attachable/fverticalgrip = round(scale * 4),
					...
					/obj/item/attachable/stock/l_stock = round(scale * 4),
					...
					/obj/item/attachable/stock/mou53 = round(scale * 3),
...
//						"Ammo",
						...
						/obj/item/ammobox/m39inc = round(scale * 1),
						/obj/item/ammo_magazine/smg/m39/incendiary = round(scale * 5),
						/obj/item/ammobox/m39ext = round(scale * 1),
						/obj/item/ammo_magazine/smg/m39/extended = round(scale * 10),
						/obj/item/ammobox/m39le = round(scale * 1),
						/obj/item/ammo_magazine/smg/m39/le = round(scale * 5),
						...
						/obj/item/ammobox/inc = round(scale * 1),
						/obj/item/ammo_magazine/rifle/incendiary = round(scale * 5),
						/obj/item/ammobox/l42 = round(scale * 3),
						/obj/item/ammo_magazine/carabin = round(scale * 5),
						/obj/item/ammobox/l42ext = round(scale * 1),
						/obj/item/ammo_magazine/carabin/extended = round(scale * 20),
						/obj/item/ammobox/l42incendiary = round(scale * 1),
						/obj/item/ammo_magazine/carabin/incendiary = round(scale * 5),
						/obj/item/ammobox/l42ap = round(scale * 1),
						/obj/item/ammo_magazine/carabin/ap = round(scale * 5),
						/obj/item/ammobox/smart = round(scale * 2),
						/obj/item/ammobox/vp78 = round(scale * 2),
						/obj/item/ammobox/s88mod4 = round(scale * 2),
						/obj/item/ammobox/m4a3 = round(scale * 2),
						/obj/item/ammobox/m4a3ap = round(scale * 2),
...
//						"Backpacks",
						...
						/obj/item/storage/backpack/marine/engineerpack = round(scale * 5),
						/obj/item/storage/backpack/marine/engineerpack/flamethrower2 = round(scale * 5),
...
//						"Pouches",
						/obj/item/storage/pouch/general/medium = round(scale * 4),
						/obj/item/storage/pouch/shotgun = round(scale * 4),
						/obj/item/storage/pouch/construction = round(scale * 4),
						/obj/item/storage/pouch/tools = round(scale * 4),
						/obj/item/storage/pouch/explosive = round(scale * 4),
						/obj/item/storage/pouch/pyro = round(scale * 4),
						/obj/item/storage/pouch/syringe = round(scale * 4),
						/obj/item/storage/pouch/medical = round(scale * 4),
						/obj/item/storage/pouch/medkit = round(scale * 4),
...
//						"Firearms",
						...
						/obj/item/weapon/gun/rifle/l42 = round(scale * 10),
						...
						/obj/item/weapon/gun/shotgun/mou53 = round(scale * 1),

//						"Kits",
						/obj/item/storage/box/Pyrotechnician_kit = round(scale * 4),
						/obj/item/storage/box/sniper_kit = round(scale * 4),
						/obj/item/storage/box/defense_kit = round(scale * 4),
						/obj/item/storage/box/veteran_kit = round(scale * 4),
						/obj/item/storage/box/trooper_kit = round(scale * 4),
						/obj/item/storage/box/heavy_kit = round(scale * 4),
						/obj/item/storage/box/field_kit = round(scale * 4),
						/obj/item/storage/box/cts_kit = round(scale * 4),
						/obj/item/storage/box/grenadier_kit = round(scale * 4),
						/obj/item/storage/box/mini_med_kit = round(scale * 4),
						/obj/item/storage/box/jtac_kit = round(scale * 4),
//						/obj/item/storage/box/fis_kit = round(scale * 4),
						/obj/item/storage/box/point_man_kit = round(scale * 4),

//						"Explosives",
						/obj/item/explosive/mine = round(scale * 10),
						/obj/item/storage/box/nade_box = round(scale * 2),
						/obj/item/storage/box/nade_box2 = round(scale * 2),
						/obj/item/storage/box/nade_box3 = round(scale * 2),
						/obj/item/explosive/grenade/frag/m15 = round(scale * 2),
						/obj/item/explosive/grenade/incendiary = round(scale * 4),
						/obj/item/explosive/grenade/phosphorus = round(scale * 4),
						/obj/item/explosive/grenade/smokebomb = round(scale * 5),
						/obj/item/explosive/grenade/cloakbomb = round(scale * 3),

//						"Miscellaneous",
						/obj/item/storage/large_holster/machete/full = round(scale * 25),
						/obj/item/device/flashlight/combat = round(scale * 15),
						/obj/item/storage/box/m94 = round(scale * 50),
...
//						"Firearms",
						...
						/obj/item/weapon/gun/rifle/l42 = round(scale * 5),

//						"Ammo",
						/obj/item/ammo_magazine/carabin = round(scale * 20),
						/obj/item/ammo_magazine/carabin/extended = round(scale * 2),
						/obj/item/ammo_magazine/carabin/incendiary = round(scale * 2),
						/obj/item/ammo_magazine/carabin/ap = round(scale * 2),
						...
						/obj/item/ammo_magazine/rifle/ap = round(scale * 2),
						/obj/item/ammo_magazine/shotgun = round(scale * 10),
						/obj/item/ammo_magazine/shotgun/buckshot = round(scale * 10),
