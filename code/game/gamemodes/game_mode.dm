		...
		// Antags
		   ...
		   special_role == "Head Revolutionary" && prob(30))
			...
			if(suplink)
				var/extra = 4
				suplink.uses += extra
				to_chat(man, "<span style='color:#FF0000'>We have received notice that enemy intelligence suspects you to be linked with us. We have thus invested significant resources to increase your uplink's capacity.</span>")
			else
				// Give them a warning!
				to_chat(man, "<span style='color:#FF0000'>They are on to you!</span>")
...
//		if(BE_PREF)			roletext = "Preffixs ([your_pref])"
//		if(BE_SUF)			roletext = "Suffics ([rand_suf])"
	...
	while(candidates.len < recommended_enemies) //Pick randomly just the number of people we need and add them to our list of candidates
		if(drafted.len > 0)
			...
			if(applicant)
				...
				to_chat(world, "<span style='color:#FF0000'>[applicant.key] was force-drafted as [roletext], because there aren't enough candidates.</span>")
...
proc/display_roundstart_logout_report()
	var/msg = "<span style='color:#0000FF'><b>Roundstart logout report\n\n</span>"
...
/proc/show_objectives(var/datum/mind/player)
	...
	var/obj_count = 1
	to_chat(player.current, "<span style='color:#0000FF'>Your current objectives:</span>")
