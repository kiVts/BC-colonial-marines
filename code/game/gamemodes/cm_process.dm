...
#define QUEEN_DEATH_COUNTDOWN 			 6000 //10 minutes. Can be changed into a variable if it needs to be manipulated later.
	...
	if(ENABLE_FANFACTS)
		...
		if(F)
			for(var/ckey in F.dir)
				to_chat(src, "<span class='round_body'>*_______________*Fun Fact*_______________*</span><br>")
				to_chat(src, "<center><span class='round_body'>[F]</span><br></center>")
				to_chat(src, "<span class='round_body'>*___________________________________*</span><br>")
...
/datum/game_mode/proc/declare_completion_announce_round_stats()
	set waitfor = 0
	sleep(140)
	var/list/dat = list({"<span class='round_body'>Статистика раунда:</span><br>
...
//	config_set(flag/allow_synthetic_gun_use, 1)
	...
	if(length(spawns) < 1)
		...
		to_chat(world, "<br><br><h1><span class='warning'>End of Round Deathmatch initialization failed, пожалуйста не гриферете.</span></h1><br><br>")
		return
...
/datum/game_mode/proc/disperse_fog()
	set waitfor = 0
	to_chat(world, "<span class='boldnotice'></span>")//Туман на севере колонии спадает.
