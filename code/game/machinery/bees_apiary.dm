...
/obj/machinery/apiary/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(istype(O, /obj/item/queen_bee))
		if(health > 0)
			to_chat(user, "<span style='color:#FF0000'>There is already a queen in there.</span>")
		else
			health = 10
			nutrilevel += 10
			if(user.temp_drop_inv_item(O))
				cdel(O)
				to_chat(user, "<span style='color:#0000FF'>You carefully insert the queen into [src], she gets busy making a hive.</span>")
				bees_in_hive = 0
		...
		if(user.temp_drop_inv_item(O))
			if(health > 0)
				to_chat(user, "<span style='color:#0000FF'>You insert [O] into [src]. A relaxed humming appears to pick up.</span>")
			else
				to_chat(user, "<span style='color:#0000FF'>You insert [O] into [src]. Now it just needs some bees.</span>")
			cdel(O)
	else if(istype(O, /obj/item/tool/minihoe))
		if(health > 0)
			to_chat(user, "<span style='color:#FF0000'><b>You begin to dislodge the apiary from the tray, the bees don't like that.</b></span>")
			angry_swarm(user)
		else
			to_chat(user, "<span style='color:#0000FF'>You begin to dislodge the dead apiary from the tray.</span>")
		if(do_after(user, 50, TRUE, 5, BUSY_ICON_GENERIC))
			new hydrotray_type(src.loc)
			new /obj/item/frame/apiary(src.loc)
			to_chat(user, "<span style='color:#FF0000'>You dislodge the apiary from the tray.</span>")
			cdel(src)
	else if(istype(O, /obj/item/tool/bee_net))
		var/obj/item/tool/bee_net/N = O
		if(N.caught_bees > 0)
			to_chat(user, "<span style='color:#0000FF'>You empty the bees into the apiary.</span>")
			bees_in_hive += N.caught_bees
			N.caught_bees = 0
		else
			to_chat(user, "<span style='color:#0000FF'>There are no more bees in the net.</span>")
	else if(istype(O, /obj/item/reagent_container/glass))
		var/obj/item/reagent_container/glass/G = O
		if(harvestable_honey > 0)
			if(health > 0)
				to_chat(user, "<span style='color:#FF0000'>You begin to harvest the honey. The bees don't seem to like it.</span>")
				angry_swarm(user)
			else
				to_chat(user, "<span style='color:#0000FF'>You begin to harvest the honey.</span>")
			if(do_after(user,50, TRUE, 5, BUSY_ICON_GENERIC))
				G.reagents.add_reagent("honey",harvestable_honey)
				harvestable_honey = 0
				to_chat(user, "<span style='color:#0000FF'>You successfully harvest the honey.</span>")
		else
			to_chat(user, "<span style='color:#0000FF'>There is no honey left to harvest.</span>")
	else
		angry_swarm(user)
		..()

/obj/machinery/apiary/CanPass(atom/movable/mover)
...
/obj/machinery/apiary/verb/harvest_honeycomb()
	...
	to_chat(usr, "<span style='color:#0000FF'>You harvest the honeycomb from the hive. There is a wild buzzing!</span>")
	angry_swarm(usr)
