	...
	if (istype(W, /obj/item/card/emag))
		...
		if(C.flags_token & tokensupport)
			if(user.drop_inv_item_to_loc(W, src))
				coin = W
				to_chat(user, "<span style='color:#0000FF'>You insert the [W] into the [src]</span>")
		else
...
/obj/machinery/vending/attack_hand(mob/user as mob)
	if(tipped_level == 2)
		tipped_level = 1
		user.visible_message("<span style='color:#0000FF'>[user] begins to heave the vending machine back into place!","\blue You start heaving the vending machine back into place..</span>")
		if(do_after(user,80, FALSE, 5, BUSY_ICON_FRIENDLY))
			user.visible_message("<span style='color:#0000FF'>[user] rights the [src]!","\blue You right the [src]!</span>")
			flip_back()
			return
		else
			tipped_level = 2
...
/obj/machinery/vending/Topic(href, href_list)
	...
	if(href_list["remove_coin"] && !istype(usr,/mob/living/silicon))
		...
		to_chat(usr, "<span style='color:#0000FF'>You remove the [coin] from the [src]</span>")
		coin = null
	...
	if(href_list["remove_ewallet"] && !istype(usr,/mob/living/silicon))
		...
		to_chat(usr, "<span style='color:#0000FF'>You remove the [ewallet] from the [src]</span>")
		ewallet = null
			...
			if(R.price == null)
				...
				if (ewallet)
					if (R.price <= ewallet.worth)
						...
						to_chat(usr, "<span style='color:#FF0000'>The ewallet doesn't have enough money to pay for that.</span>")
...
/obj/machinery/vending/proc/vend(datum/data/vending_product/R, mob/user)
	...
	if (R in coin_records)
		if(!coin)
			to_chat(user, "<span style='color:#0000FF'>You need to insert a coin to get this item.</span>")
			return
		if(coin.string_attached)
			if(prob(50))
				to_chat(user, "<span style='color:#0000FF'>You successfully pull the coin out before the [src] could swallow it.</span>")
			else
				to_chat(user, "<span style='color:#0000FF'>You weren't able to pull the coin out fast enough, the machine ate it, string and all.</span>")
				cdel(coin)
				coin = null
		else
			cdel(coin)
