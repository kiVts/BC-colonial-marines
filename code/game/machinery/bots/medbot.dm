	...
	if(open && !locked)
		...
		spawn(0)
			for(var/mob/O in hearers(src, null))
				O.show_message("<span style='color:#FF0000'><B>[src] buzzes oddly!</B></span>", 1)
...
/obj/machinery/bot/medbot/proc/medicate_patient(mob/living/carbon/C as mob)
	...
	if(!reagent_id) //If they don't need any of that they're probably cured!
		...
		visible_message("<span style='color:#FF0000'><B>[src] is trying to inject [src.patient]!</B></span>")
		spawn(30)
			if ((get_dist(src, src.patient) <= 1) && (src.on))
				...
				visible_message("<span style='color:#FF0000'><B>[src] injects [src.patient] with the syringe!</B></span>")
...
/obj/machinery/bot/medbot/explode()
	src.on = 0
	visible_message("<span style='color:#FF0000'><B>[src] blows apart!</B></span>", 1)
