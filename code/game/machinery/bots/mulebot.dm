...
/obj/machinery/bot/mulebot/attackby(var/obj/item/I, var/mob/user)
	if(istype(I,/obj/item/card/emag))
		locked = !locked
		to_chat(user, "<span style='color:#0000FF'>You [locked ? "lock" : "unlock"] the mulebot's controls!</span>")
		...
		if(locked)
			to_chat(user, "<span style='color:#0000FF'>The maintenance hatch cannot be opened or closed while the controls are locked.</span>")
			return

		open = !open
		if(open)
			src.visible_message("[user] opens the maintenance hatch of [src]", "<span style='color:#0000FF'>You open [src]'s maintenance hatch.</span>")
			on = 0
			icon_state="mulebot-hatch"
		else
			src.visible_message("[user] closes the maintenance hatch of [src]", "<span style='color:#0000FF'>You close [src]'s maintenance hatch.</span>")
			icon_state = "mulebot0"
		...
		if (src.health < maxhealth)
			...
			user.visible_message(
				"<span style='color:#FF0000'>[user] repairs [src]!</span>",
				"<span style='color:#0000FF'>You repair [src]!</span>"
			)
		else
			to_chat(user, "<span style='color:#0000FF'>[src] does not need a repair!</span>")
	else if(load && ismob(load))  // chance to knock off rider
		if(prob(1+I.force * 2))
			unload(0)
			user.visible_message("<span style='color:#FF0000'>[user] knocks [load] off [src] with \the [I]!", "\red You knock [load] off [src] with \the [I]!</span>")
		else
...
/obj/machinery/bot/mulebot/bullet_act()
	if(prob(50) && !isnull(load))
		unload(0)
	if(prob(25))
		src.visible_message("<span style='color:#FF0000'>Something shorts out inside [src]!</span>")
		...
		switch(href_list["op"])
			...
			if("power")
				if (src.on)
					...
					if (!turn_on())
						to_chat(usr, "<span style='color:#FF0000'>You can't switch on [src].</span>")
						return
				else
					return
			...
			if("cellremove")
				if(open && cell && !usr.get_active_hand())
					...
					usr.visible_message("<span style='color:#0000FF'>[usr] removes the power cell from [src].", "\blue You remove the power cell from [src].</span>")
					updateDialog()
				...
				if(open && !cell)
					...
					if(istype(C))
						if(usr.drop_held_item())
							...
							usr.visible_message("<span style='color:#0000FF'>[usr] inserts a power cell into [src].", "\blue You insert the power cell into [src].</span>")
							updateDialog()
			...
			if("wirecut")
				if(istype(usr.get_active_hand(), /obj/item/tool/wirecutters))
					...
					to_chat(usr, "<span style='color:#0000FF'>You need wirecutters!</span>")
			if("wiremend")
				if(istype(usr.get_active_hand(), /obj/item/tool/wirecutters))
					var/wirebit = text2num(href_list["wire"])
					wires |= wirebit
			...
			if("wirepulse")
				if(istype(usr.get_active_hand(), /obj/item/device/multitool))
					switch(href_list["wire"])
						if("1","2")
							to_chat(usr, "<span style='color:#0000FF'>\icon[src] The charge light flickers.</span>")
						if("4")
							to_chat(usr, "<span style='color:#0000FF'>\icon[src] The external warning lights flash briefly.</span>")
						if("8")
							to_chat(usr, "<span style='color:#0000FF'>\icon[src] The load platform clunks.</span>")
						if("16", "32")
							to_chat(usr, "<span style='color:#0000FF'>\icon[src] The drive motor whines briefly.</span>")
						else
							to_chat(usr, "<span style='color:#0000FF'>\icon[src] You hear a radio crackle.</span>")
				else
					to_chat(usr, "<span style='color:#0000FF'>You need a multitool!</span>")
	...
	if(!(wires & wire_mobavoid))		//usually just bumps, but if avoidance disabled knock over mobs
		...
		if(ismob(M))
			if(istype(M,/mob/living/silicon/robot))
				src.visible_message("<span style='color:#FF0000'>[src] bumps into [M]!</span>")
			else
				src.visible_message("<span style='color:#FF0000'>[src] knocks over [M]!</span>")
				M.stop_pulling()
				M.Stun(8)
				M.KnockDown(5)
				M.lying = 1
...
/obj/machinery/bot/mulebot/proc/RunOver(var/mob/living/carbon/human/H)
	src.visible_message("<span style='color:#FF0000'>[src] drives over [H]!</span>")
...
/obj/machinery/bot/mulebot/explode()
	src.visible_message("<span style='color:#FF0000'><B>[src] blows apart!</B></span>", 1)
