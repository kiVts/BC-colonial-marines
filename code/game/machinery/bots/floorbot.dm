...
/obj/machinery/bot/floorbot/process()
	...
	if(src.loc == src.target || src.loc == src.target.loc)
		if(istype(src.target, /obj/item/stack/tile/plasteel))
			...
			visible_message("<span style='color:#FF0000'>[src] makes an excited booping sound.</span>")
			spawn(50)
				src.amount ++
				src.anchored = 0
				src.repairing = 0
...
/obj/machinery/bot/floorbot/proc/repair(var/turf/target)
	...
	if(istype(target, /turf/open/space/))
		visible_message("<span style='color:#FF0000'>[src] begins to repair the hole</span>")
		...
		visible_message("<span style='color:#FF0000'>[src] begins to improve the floor.</span>")
		src.repairing = 1
		spawn(50)
...
/obj/machinery/bot/floorbot/proc/eattile(var/obj/item/stack/tile/plasteel/T)
	if(!istype(T, /obj/item/stack/tile/plasteel))
		return
	visible_message("<span style='color:#FF0000'>[src] begins to collect tiles.</span>")
	src.repairing = 1
	spawn(20)
		if(isnull(T))
			src.target = null
...
/obj/machinery/bot/floorbot/proc/maketile(var/obj/item/stack/sheet/metal/M)
	...
	if(M.get_amount() > 1)
		return
	visible_message("<span style='color:#FF0000'>[src] begins to create tiles.</span>")
	src.repairing = 1
	spawn(20)
		if(isnull(M))
			src.target = null
