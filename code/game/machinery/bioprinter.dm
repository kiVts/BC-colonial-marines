...
/obj/machinery/bioprinter/attack_hand(mob/user)
	...
	if(stored_matter >= products[choice][2] && stored_metal >= products[choice][3]) //Matter and metal
		...
		to_chat(user, "<span style='color:#0000FF'>\The [src] is now printing the selected organ. Please hold.</span>")
		working = 1
...
//Matter
	if(istype(W, /obj/item/reagent_container/food/snacks/meat))
		to_chat(user, "<span style='color:#0000FF'>\The [src] processes \the [W].</span>")
		stored_matter += 50
		user.drop_held_item()
		cdel(W)
		return
