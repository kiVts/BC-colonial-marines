...
/obj/machinery/gibber/attack_hand(mob/user as mob)
	if(stat & (NOPOWER|BROKEN))
		return
	if(operating)
		to_chat(user, "<span style='color:#FF0000'>It's locked and running</span>")
		return
	else
...
/obj/machinery/gibber/proc/startgibbing(mob/user as mob)
	if(src.operating)
		return
	if(!src.occupant)
		visible_message("<span style='color:#FF0000'>You hear a loud metallic grinding sound.</span>")
		return
	use_power(1000)
	visible_message("<span style='color:#FF0000'>You hear a loud squelchy grinding sound.</span>")
	src.operating = 1
	update_icon()
