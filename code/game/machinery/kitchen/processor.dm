		...
		monkey
			process(loc, what)
				...
				if (O.client) //grief-proof
					O.loc = loc
					O.visible_message("<span style='color:#0000FF'>Suddenly [O] jumps out from the processor!</span>", \
...
/obj/machinery/processor/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(src.processing)
		to_chat(user, "<span style='color:#FF0000'>The processor is in the process of processing.</span>")
		return 1
	if(src.contents.len > 0) //TODO: several items at once? several different items?
		to_chat(user, "<span style='color:#FF0000'>Something is already in the processing chamber.</span>")
		return 1
	var/obj/what = O
	...
	if (!P)
		to_chat(user, "<span style='color:#FF0000'>That probably won't blend.</span>")
		return 1
...
/obj/machinery/processor/attack_hand(var/mob/user as mob)
	...
	if(src.contents.len == 0)
		to_chat(user, "<span style='color:#FF0000'>The processor is empty.</span>")
		return 1
	for(var/O in src.contents)
		...
		if (!P)
			...
			continue
		src.processing = 1
		user.visible_message("<span style='color:#0000FF'>[user] turns on [src].</span>", \
			"You turn on [src].", \
		...
		P.process(src.loc, O)
		src.processing = 0
	src.visible_message("<span style='color:#0000FF'>\the [src] finished processing.</span>", \
