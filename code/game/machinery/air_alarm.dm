...
/*	if (istype(W, /obj/item/tool/wirecutters))
		...
		for(var/mob/O in viewers(user, null))
			O.show_message(text("<span style='color:#FF0000'>[] has []activated []!", user, (stat&BROKEN) ? "de" : "re</span>", src), 1)
		update_icon()
		return
*/
			...
			if(istype(W, /obj/item/card/id) || istype(W, /obj/item/device/pda))// trying to unlock the interface with an ID card
				if(stat & (NOPOWER|BROKEN))
					...
					if(allowed(usr) && !isWireCut(AALARM_WIRE_IDSCAN))
						locked = !locked
						to_chat(user, "<span style='color:#0000FF'>You [ locked ? "lock" : "unlock"] the Air Alarm interface.</span>")
						updateUsrDialog()
					else
