...
/obj/machinery/atmospherics/unary/cryo_cell/proc/put_mob(mob/living/carbon/M as mob)
	if (stat & (NOPOWER|BROKEN))
		to_chat(usr, "<span style='color:#FF0000'>The cryo cell is not functioning.</span>")
		return
	if (!istype(M))
		to_chat(usr, "<span style='color:#FF0000'><B>The cryo cell cannot handle such a lifeform!</B></span>")
		return
	if (occupant)
		to_chat(usr, "<span style='color:#FF0000'><B>The cryo cell is already occupied!</B></span>")
		return
	if (M.abiotic())
		to_chat(usr, "<span style='color:#FF0000'>Subject may not have abiotic items on.</span>")
		return
	if(!node)
		to_chat(usr, "<span style='color:#FF0000'>The cell is not correctly connected to its pipe network!</span>")
		return
	M.forceMove(src)
	if(M.health > -100 && (M.health < 0 || M.sleeping))
		to_chat(M, "<span style='color:#0000FF'><b>You feel a cold liquid surround you. Your skin starts to freeze up.</b></span>")
	occupant = M
	update_use_power(2)
//	M.metabslow = 1
	add_fingerprint(usr)
