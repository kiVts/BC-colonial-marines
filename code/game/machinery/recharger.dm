...
obj/machinery/recharger/attackby(obj/item/G as obj, mob/user as mob)
	...
	if(allowed)
		if(charging)
			to_chat(user, "<span style='color:#FF0000'>\A [charging] is already charging here.</span>")
			return
		// Checks to make sure he's not in space doing it, and that the area got proper power.
		var/area/a = get_area(src)
		if(!isarea(a) || (a.power_equip == 0 && !a.unlimited_power))
			to_chat(user, "<span style='color:#FF0000'>The [name] blinks red as you try to insert the item!</span>")
			return
		...
		if(charging)
			to_chat(user, "<span style='color:#FF0000'>Remove [charging] first!</span>")
			return
		anchored = !anchored
