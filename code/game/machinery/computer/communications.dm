 ...
 * CO COMPUTER
 */
/obj/machinery/computer/commandtable
	name = "Universal Command Table"
	desc = "Ёта консоль предназначена для отдачи приказов отряду и оповещений, также много других функций. ќчень полезнаявещь для CO."
	...
	anchored = 1
	req_access = list(ACCESS_MARINE_BRIDGE)
	var/w_class = 3.0		 //upped to three because books are, y'know, pretty big. (and you could hide them inside eachother recursively forever)
	circuit = "/obj/item/circuitboard/computer/communications"
	var/prints_intercept = 0
...
/obj/machinery/computer/commandtable/verb/verb_pickup()
	set src in oview(1)
	set category = "Object"
	set name = "Pick up"
	...
	if((!istype(usr, /mob/living/carbon)) || (istype(usr, /mob/living/brain)))//Is humanoid, and is not a brain
		to_chat(usr, "<span style='color:#FF0000'>You can't pick things up!</span>")
		return
	if( usr.stat || usr.is_mob_restrained() )//Is not asleep/dead and is not restrained
		to_chat(usr, "<span style='color:#FF0000'>You can't pick things up!</span>")
		return
	if(src.anchored) //Object isn't anchored
		to_chat(usr, "<span style='color:#FF0000'>You can't pick that up!</span>")
		return
	if(!usr.hand && usr.r_hand) //Right hand is not full
		to_chat(usr, "<span style='color:#FF0000'>Your right hand is full.</span>")
		return
	if(usr.hand && usr.l_hand) //Left hand is not full
		to_chat(usr, "<span style='color:#FF0000'>Your left hand is full.</span>")
		return
...
/obj/machinery/computer/commandtable/proc/mob_can_equip(M as mob, slot, disable_warning = 0)
	if(!slot)
		return FALSE
	if(!M)
		return FALSE
...
/obj/machinery/computer/commandtable/New()
	..()
	crew_announcement.newscast = 1
	start_processing()

/obj/machinery/computer/commandtable/process()
	if(..())
		if(state != STATE_STATUSDISPLAY)
			updateDialog()

/obj/machinery/computer/commandtable/Topic(href, href_list)
	if(..()) return FALSE
...
/obj/machinery/computer/commandtable/attack_ai(var/mob/user as mob)
	return attack_hand(user)

/obj/machinery/computer/commandtable/attack_paw(var/mob/user as mob)
	return attack_hand(user)

/obj/machinery/computer/commandtable/attack_hand(var/mob/user as mob)
	if(..()) return FALSE
...
/*
/obj/machinery/computer/commandtable/proc/interact_ai(var/mob/living/silicon/ai/user as mob)
	var/dat = ""
	switch(aistate)
		if(STATE_DEFAULT)
			if(emergency_shuttle.location() && !emergency_shuttle.online())
...
/obj/machinery/computer/commandtable/proc/post_status(command, data1, data2)
...
/obj/machinery/computer/commandtable/proc/get_loc_turf()
	var/atom/L = loc
