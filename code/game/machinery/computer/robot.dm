...
/obj/machinery/computer/robotics/attack_hand(var/mob/user as mob)
	if(..())
		return
	if (src.z > 6)
		to_chat(user, "<span style='color:#FF0000'><b>Unable to establish a connection</b>: \black You're too far away from the station!</span>")
		return
		...
		if (href_list["eject"])
			...
			if (istype(I))
				if(src.check_access(I))
					if (!status)
						message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] has initiated the global cyborg killswitch!</span>")
						log_game("<span style='color:#0000FF'>[key_name(usr)] has initiated the global cyborg killswitch!</span>")
						src.status = 1
						src.start_sequence()
						src.temp = null
					...
					if(choice == "Confirm")
						if(R && istype(R))
							if(R.mind && R.mind.special_role && R.emagged)
								...
								message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] detonated [R.name]!</span>")
								log_game("<span style='color:#0000FF'>[key_name_admin(usr)] detonated [R.name]!</span>")
								R.self_destruct()
			else
				to_chat(usr, "<span style='color:#FF0000'>Access Denied.</span>")
			...
			if(src.allowed(usr))
				...
				if(R && istype(R)) // Extra sancheck because of input var references
					...
					if(choice == "Confirm")
						if(R && istype(R))
							message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] [R.canmove ? "locked down" : "released"] [R.name]!</span>")
			...
			if(src.allowed(usr))
				...
				if(istype(R) && istype(usr, /mob/living/silicon) && usr.mind.special_role && (usr.mind.original == usr) && !R.emagged)
					...
					if(choice == "Confirm")
						if(R && istype(R))
//							message_admins("<span style='color:#0000FF'>[key_name_admin(usr)] emagged [R.name] using robotic console!</span>")
