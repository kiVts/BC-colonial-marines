		...
		if(istype(I,/obj/item/weapon/card/emag))
			if(network)
				...
				if(E.uses)
					...
					return
				authorised = 1
				to_chat(user, "<span style='color:#0000FF'>You authorised the circuit network!</span>")
				updateDialog()
			else
				to_chat(user, "<span style='color:#0000FF'>You must select a camera network circuit!</span>")
		else if(istype(I,/obj/item/weapon/screwdriver))
			secured = !secured
			user.visible_message("<span style='color:#0000FF'>The [src] can [secured ? "no longer" : "now"] be modified.</span>")
			updateBuildPath()
		return
			...
			if(istype(I,/obj/item/weapon/card/emag))
				if(network)
					...
					if(E.uses)
						...
						return
					authorised = 1
					to_chat(usr, "<span style='color:#0000FF'>You authorised the circuit network!</span>")
					updateDialog()
				else
					to_chat(usr, "<span style='color:#0000FF'>You must select a camera network circuit!</span>")
