	...
	switch(state)
		if(0)
			if(istype(P, /obj/item/tool/wrench))
				...
				if(do_after(user, 20, TRUE, 5, BUSY_ICON_BUILD))
					to_chat(user, "<span style='color:#0000FF'>You wrench the frame into place.</span>")
					anchored = 1
					state = 1
			if(istype(P, /obj/item/tool/weldingtool))
				...
				if(do_after(user, 20, TRUE, 5, BUSY_ICON_BUILD))
					...
					to_chat(user, "<span style='color:#0000FF'>You deconstruct the frame.</span>")
		...
		if(1)
			if(istype(P, /obj/item/tool/wrench))
				playsound(loc, 'sound/items/Ratchet.ogg', 25, 1)
				if(do_after(user, 20, TRUE, 5, BUSY_ICON_BUILD))
					to_chat(user, "<span style='color:#0000FF'>You unfasten the frame.</span>")
					anchored = 0
					state = 0
			if(istype(P, /obj/item/circuitboard/aicore) && !circuit)
				if(user.drop_held_item())
					playsound(loc, 'sound/items/Deconstruct.ogg', 25, 1)
					to_chat(user, "<span style='color:#0000FF'>You place the circuit board inside the frame.</span>")
					icon_state = "1"
					circuit = P
					P.forceMove(src)
			if(istype(P, /obj/item/tool/screwdriver) && circuit)
				playsound(loc, 'sound/items/Screwdriver.ogg', 25, 1)
				to_chat(user, "<span style='color:#0000FF'>You screw the circuit board into place.</span>")
				state = 2
				icon_state = "2"
			if(istype(P, /obj/item/tool/crowbar) && circuit)
				playsound(loc, 'sound/items/Crowbar.ogg', 25, 1)
				to_chat(user, "<span style='color:#0000FF'>You remove the circuit board.</span>")
				state = 1
				icon_state = "0"
				circuit.loc = loc
				circuit = null
		if(2)
			if(istype(P, /obj/item/tool/screwdriver) && circuit)
				playsound(loc, 'sound/items/Screwdriver.ogg', 25, 1)
				to_chat(user, "<span style='color:#0000FF'>You unfasten the circuit board.</span>")
				state = 1
				icon_state = "1"
		...
		if(3)
			if(istype(P, /obj/item/tool/wirecutters))
				if (brain)
					...
					to_chat(user, "<span style='color:#0000FF'>You remove the cables.</span>")
					state = 2
					icon_state = "2"
			...
			if(istype(P, /obj/item/device/mmi))
				if(!P:brainmob)
					to_chat(user, "<span style='color:#FF0000'>Sticking an empty [P] into the frame would sort of defeat the purpose.</span>")
					return
				if(P:brainmob.stat == 2)
					to_chat(user, "<span style='color:#FF0000'>Sticking a dead [P] into the frame would sort of defeat the purpose.</span>")
					return

				if(jobban_isbanned(P:brainmob, "AI"))
					to_chat(user, "<span style='color:#FF0000'>This [P] does not seem to fit.</span>")
					return
			...
			if(istype(P, /obj/item/tool/crowbar) && brain)
				playsound(loc, 'sound/items/Crowbar.ogg', 25, 1)
				to_chat(user, "<span style='color:#0000FF'>You remove the brain.</span>")
				brain.loc = loc
				brain = null
				icon_state = "3"
		...
		if(4)
			if(istype(P, /obj/item/tool/crowbar))
				playsound(loc, 'sound/items/Crowbar.ogg', 25, 1)
				to_chat(user, "<span style='color:#0000FF'>You remove the glass panel.</span>")
				state = 3
				if (brain)
					icon_state = "3b"
				else
			...
			if(istype(P, /obj/item/tool/screwdriver))
				playsound(loc, 'sound/items/Screwdriver.ogg', 25, 1)
				to_chat(user, "<span style='color:#0000FF'>You connect the monitor.</span>")
				...
				switch(interaction)
					if("AICARD")
						...
						if(C.contents.len)//If there is an AI on card.
							to_chat(U, "<span style='color:#FF0000'><b>Transfer failed</b>: \black Existing AI found on this terminal. Remove existing AI to install a new one.</span>")
						else
							...
							to_chat(U, "<span style='color:#0000FF'><b>Transfer successful</b>: \black [T.name] ([rand(1000,9999)].exe) removed from host terminal and stored within local memory.</span>")
				...
				switch(interaction)
					if("AICARD")
						...
						if(A)//If AI exists on the card. Else nothing since both are empty.
							...
							to_chat(U, "<span style='color:#0000FF'><b>Transfer successful</b>: \black [A.name] ([rand(1000,9999)].exe) installed and executed succesfully. Local copy has been removed.</span>")
							cdel(T)
					...
					if("AICARD")
						...
						if(!T.contents.len)
							if (!C.contents.len)
								...
								to_chat(U, "<span style='color:#0000FF'><b>Transfer successful</b>: \black [A.name] ([rand(1000,9999)].exe) installed and executed successfully. Local copy has been removed.</span>")
						else
							if(!C.contents.len && T.occupant && !T.active)
								...
								to_chat(U, "<span style='color:#0000FF'><b>Transfer successful</b>: \black [T.occupant.name] ([rand(1000,9999)].exe) removed from host terminal and stored within local memory.</span>")
								T.occupant.loc = C
								T.occupant.cancel_camera()
								T.occupant = null
							else if (C.contents.len)
								to_chat(U, "<span style='color:#FF0000'><b>ERROR</b>: \black Artificial intelligence detected on terminal.</span>")
							else if (T.active)
								to_chat(U, "<span style='color:#FF0000'><b>ERROR</b>: \black Reconstruction in progress.</span>")
							else if (!T.occupant)
								to_chat(U, "<span style='color:#FF0000'><b>ERROR</b>: \black Unable to locate artificial intelligence.</span>")
	else
		to_chat(U, "<span style='color:#FF0000'><b>ERROR</b>: \black AI flush is in progress, cannot execute transfer protocol.</span>")
	return
