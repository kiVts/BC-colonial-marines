...
/obj/machinery/computer/prison_shuttle
	...
	attackby(I as obj, user as mob)
		if(istype(I, /obj/item/tool/screwdriver))
			...
			if(do_after(user, 20, TRUE, 5, BUSY_ICON_BUILD))
				...
				if (src.stat & BROKEN)
					to_chat(user, "<span style='color:#0000FF'>The broken glass falls out.</span>")
					new /obj/item/shard( src.loc )
					A.state = 3
					A.icon_state = "3"
				else
					to_chat(user, "<span style='color:#0000FF'>You disconnect the monitor.</span>")
					A.state = 4
					A.icon_state = "4"
			...
			hacked = 1
			to_chat(user, "<span style='color:#0000FF'>You disable the lock.</span>")
		else
			return src.attack_hand(user)
	...
	attack_hand(var/mob/user as mob)
		if(!src.allowed(user) && (!hacked))
			...
			return
		if(prison_break)
			to_chat(user, "<span style='color:#FF0000'>Unable to locate shuttle.</span>")
			return
		if(..())
			return
	...
	Topic(href, href_list)
		...
		if (href_list["sendtodock"])
			if (!prison_can_move())
				to_chat(usr, "<span style='color:#FF0000'>The prison shuttle is unable to leave.</span>")
				return
			if(!prison_shuttle_at_station|| prison_shuttle_moving_to_station || prison_shuttle_moving_to_prison) return
			post_signal("prison")
			to_chat(usr, "<span style='color:#0000FF'>The prison shuttle has been called and will arrive in [(PRISON_MOVETIME/10)] seconds.</span>")
