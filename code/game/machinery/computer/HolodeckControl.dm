...
/obj/machinery/computer/HolodeckControl/attackby(var/obj/item/D as obj, var/mob/user as mob)
	if(istype(D, /obj/item/card/emag) && !emagged)
		...
		emagged = 1
		to_chat(user, "<span style='color:#0000FF'>You vastly increase projector power and override the safety and security protocols.</span>")
		...
		if(prob(50))
			...
			for(var/obj/machinery/scoreboard/X in machines)
				if(X.id == id)
					X.score(side)
					// no break, to update multiple scoreboards
			visible_message("<span style='color:#0000FF'>Swish! \the [I] lands in \the [src].</span>", 3)
		else
			visible_message("<span style='color:#FF0000'>\the [I] bounces off of \the [src]'s rim!</span>", 3)
		return 0
	else
		return ..()
