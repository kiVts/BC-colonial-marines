...
/obj/machinery/porta_turret/Topic(href, href_list)
	...
	if ((href_list["power"]) && (src.allowed(usr)))
		if(anchored) // you can't turn a turret on/off if it's not anchored/secured
			...
			to_chat(usr, "<span style='color:#FF0000'>It has to be secured first!</span>")
...
/obj/machinery/porta_turret/attackby(obj/item/W as obj, mob/user as mob)
	...
	if ((istype(W, /obj/item/weapon/card/emag)) && (!src.emagged))
		...
		to_chat(user, "<span style='color:#FF0000'>You short out [src]'s threat assessment circuits.</span>")
		spawn(0)
			for(var/mob/O in hearers(src, null))
				O.show_message("<span style='color:#FF0000'>[src] hums oddly...</span>", 1)
		emagged = 1
	...
	switch(build_step)
		if(0) // first step
			if(istype(W, /obj/item/weapon/wrench) && !anchored)
				...
				to_chat(user, "<span style='color:#0000FF'>You secure the external bolts.</span>")
				anchored = 1
				build_step = 1
				return
		...
		if(2)
			if(istype(W, /obj/item/weapon/wrench))
				playsound(src.loc, 'sound/items/Ratchet.ogg', 100, 1)
				to_chat(user, "<span style='color:#0000FF'>You bolt the metal armor into place.</span>")
				build_step = 3
				return
				...
				if (WT.get_fuel() < 5) // uses up 5 fuel.
					to_chat(user, "<span style='color:#FF0000'>You need more fuel to complete this task.</span>")
					return
		...
		if(3)
			if(istype(W, /obj/item/weapon/gun/energy)) // the gun installation part
				...
				to_chat(user, "<span style='color:#0000FF'>You add \the [W] to the turret.</span>")
				build_step = 4
		...
		if(4)
			if(isprox(W))
				build_step = 5
				to_chat(user, "<span style='color:#0000FF'>You add the prox sensor to the turret.</span>")
				del(W)
				return
		...
		if(5)
			if(istype(W, /obj/item/weapon/screwdriver))
				...
				build_step = 6
				to_chat(user, "<span style='color:#0000FF'>You close the internal access hatch.</span>")
				return
		...
		if(7)
			if(istype(W, /obj/item/weapon/weldingtool))
				...
				if(do_after(user, 30))
					if(!src || !WT.remove_fuel(5, user)) return
					build_step = 8
					to_chat(user, "<span style='color:#0000FF'>You weld the turret's armor down.</span>")
...
/obj/machinery/porta_turret_cover/attackby(obj/item/W as obj, mob/user as mob)
	...
	if ((istype(W, /obj/item/weapon/card/emag)) && (!Parent_Turret.emagged))
		to_chat(user, "<span style='color:#FF0000'>You short out [Parent_Turret]'s threat assessment circuits.</span>")
		spawn(0)
			for(var/mob/O in hearers(Parent_Turret, null))
				O.show_message("<span style='color:#FF0000'>[Parent_Turret] hums oddly...</span>", 1)
