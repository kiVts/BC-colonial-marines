			...
			if(cell)
				...
				if(istype(C))
					if(user.drop_inv_item_to_loc(C, src))
						...
						user.visible_message("<span style='color:#0000FF'>[user] inserts a power cell into [src].", "\blue You insert the power cell into [src].</span>")
		else
			to_chat(user, "The hatch must be open to insert a power cell.")
			return
	else if(istype(I, /obj/item/tool/screwdriver))
		open = !open
		user.visible_message("<span style='color:#0000FF'>[user] [open ? "opens" : "closes"] the hatch on the [src].", "\blue You [open ? "open" : "close"] the hatch on the [src].</span>")
		update_icon()
...
/obj/machinery/space_heater/interact(mob/user as mob)
	...
	if(open)
		...
		if(on)
			...
			stop_processing()
		user.visible_message("<span style='color:#0000FF'>[user] switches [on ? "on" : "off"] the [src].","\blue You switch [on ? "on" : "off"] the [src].</span>")
		update_icon()
	return
		...
		switch(href_list["op"])
			...
			if("cellremove")
				if(open && cell && !usr.get_active_hand())
					usr.visible_message("<span style='color:#0000FF'>[usr] removes \the [cell] from \the [src].", "\blue You remove \the [cell] from \the [src].</span>")
					cell.updateicon()
				...
				if(open && !cell)
					...
					if(istype(C))
						if(usr.drop_held_item())
							...
							usr.visible_message("<span style='color:#0000FF'>[usr] inserts \the [C] into \the [src].", "\blue You insert \the [C] into \the [src].</span>")
