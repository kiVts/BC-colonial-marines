...
/obj/machinery/turret/attack_hand(var/mob/living/carbon/human/user)
	...
	if(user.species.can_shred(user) && !(stat & BROKEN))
		...
		visible_message("<span style='color:#FF0000'><B>[user] has slashed at [src]!</B></span>")
		src.take_damage(15)
	return
...
/obj/machinery/turretid/attackby(obj/item/weapon/W, mob/user)
	...
	if (istype(W, /obj/item/weapon/card/emag) && !emagged)
		to_chat(user, "<span style='color:#FF0000'>You short out the turret controls' access analysis module.</span>")
		emagged = 1
		locked = 0
		if(user.machine==src)
...
/obj/machinery/turret/attack_animal(mob/living/M as mob)
	...
	if(!(stat & BROKEN))
		visible_message("<span style='color:#FF0000'><B>[M] [M.attacktext] [src]!</B></span>")
