...
*/
	if (ishuman(user))
		...
		if(H.getBrainLoss() >= 60)
			visible_message("<span style='color:#FF0000'>[H] stares cluelessly at [src] and drools.</span>")
			return 1
		else if(prob(H.getBrainLoss()))
			to_chat(user, "<span style='color:#FF0000'>You momentarily forget how to use [src].</span>")
			return 1
