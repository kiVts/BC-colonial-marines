...
/obj/machinery/nuclearbomb/attack_hand(mob/user as mob)
	if (src.extended)
		...
		if(removal_stage < 5)
			src.anchored = 1
			visible_message("<span style='color:#FF0000'>With a steely snap, bolts slide out of [src] and anchor it to the flooring!</span>")
		else
			visible_message("<span style='color:#FF0000'>\The [src] makes a highly unpleasant crunching noise. It looks like the anchoring bolts have been cut.</span>")
		if(!src.lighthack)
			flick("nuclearbombc", src)
...
/obj/machinery/nuclearbomb/verb/make_deployable()
	...
	if (src.deployable)
		to_chat(usr, "<span style='color:#FF0000'>You close several panels to make [src] undeployable.</span>")
		src.deployable = 0
	else
		to_chat(usr, "<span style='color:#FF0000'>You adjust some panels to make [src] deployable.</span>")
		src.deployable = 1
	return
				...
				if (!istype(usr.get_active_hand(), /obj/item/device/multitool))
					...
					if(src.wires[temp_wire])
						...
						if(src.safety_wire == temp_wire)
							...
							if(src.safety == 1)
								visible_message("<span style='color:#0000FF'>The [src] quiets down.</span>")
								if(!src.lighthack)
									if (src.icon_state == "nuclearbomb2")
										src.icon_state = "nuclearbomb1"
							else
								visible_message("<span style='color:#0000FF'>The [src] emits a quiet whirling noise!</span>")
			...
			if (src.yes_code)
				...
				if (href_list["timer"])
					if (src.timing == -1.0)
						return
					if (src.safety)
						to_chat(usr, "<span style='color:#FF0000'>The safety is still on.</span>")
						return
				...
				if (href_list["anchor"])
					...
					if(src.anchored)
						...
						visible_message("<span style='color:#FF0000'>The anchoring bolts slide back into the depths of [src].</span>")
