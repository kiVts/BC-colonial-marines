...
/obj/machinery/door/window/hitby(AM as mob|obj)
	...
	..()
	visible_message("<span style='color:#FF0000'><B>The glass door was hit by [AM].</B></span>", 1)
	var/tforce = 0
	if(ismob(AM))
		tforce = 40
	else
...
/obj/machinery/door/window/attack_hand(mob/user)
	if(istype(user,/mob/living/carbon/human))
		...
		if(H.species.can_shred(H))
			playsound(src.loc, 'sound/effects/Glasshit.ogg', 25, 1)
			visible_message("<span style='color:#FF0000'><B>[user] smashes against the [src.name].</B></span>", 1)
			take_damage(25)
			return
...
/obj/machinery/door/window/attackby(obj/item/I, mob/user)
	...
	if (src.operating == -1 && istype(I, /obj/item/tool/crowbar))
		...
		if (do_after(user,40, TRUE, 5, BUSY_ICON_BUILD))
			to_chat(user, "<span style='color:#0000FF'>You removed the windoor electronics!</span>")
	...
	if(!(I.flags_item & NOBLUDGEON) && I.force && density) //trying to smash windoor with item
		var/aforce = I.force
		playsound(src.loc, 'sound/effects/Glasshit.ogg', 25, 1)
		visible_message("<span style='color:#FF0000'><B>[src] was hit by [I].</B></span>")
