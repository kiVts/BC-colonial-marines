...
/obj/machinery/camera/attackby(W as obj, mob/living/user as mob)
	...
	// OTHER
		...
		if (!src.can_use())
			to_chat(user, "<span style='color:#0000FF'>Camera non-functional</span>")
			return
		if (src.bugged)
			to_chat(user, "<span style='color:#0000FF'>Camera bug removed.</span>")
			src.bugged = FALSE
		else
			to_chat(user, "<span style='color:#0000FF'>Camera bugged.</span>")
			src.bugged = TRUE
	else
		..()
	return
