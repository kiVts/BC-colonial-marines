...
/obj/item/pipe/attackby(obj/item/W, mob/user)
	...
	for(var/obj/machinery/atmospherics/M in src.loc)
		if((M.initialize_directions & pipe_dir) && M.check_connect_types_construction(M,src))	// matches at least one direction on either type of pipe & same connection type
			to_chat(user, "<span style='color:#FF0000'>There is already a pipe of the same type at this location.</span>")
			return 1
	// no conflicts found

	var/pipefailtext = "<span style='color:#FF0000'>There's nothing to connect this pipe section to!" //(with how the pipe code works, at least one end needs to be connected to something, otherwise the game deletes the segment)</span>"
...
//					to_chat(world, "[V.node2.name] is connected to valve, forcing it to update its nodes.")
	...
	user.visible_message( \
		...
		"<span style='color:#0000FF'>You have fastened the [src].</span>", \
		"You hear ratchet.")
...
/obj/item/pipe_meter/attackby(var/obj/item/W as obj, var/mob/user as mob)
	...
	if(!locate(/obj/machinery/atmospherics/pipe, src.loc))
		to_chat(user, "<span style='color:#FF0000'>You need to fasten it to a pipe</span>")
		return 1
	new/obj/machinery/meter( src.loc )
	playsound(src.loc, 'sound/items/Ratchet.ogg', 25, 1)
	to_chat(user, "<span style='color:#0000FF'>You have fastened the meter to the pipe</span>")
	cdel(src)
