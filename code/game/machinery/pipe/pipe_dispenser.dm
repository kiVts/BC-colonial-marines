...
/obj/machinery/pipedispenser/attackby(var/obj/item/W as obj, var/mob/user as mob)
	...
	if (istype(W, /obj/item/pipe) || istype(W, /obj/item/pipe_meter))
		to_chat(usr, "<span style='color:#0000FF'>You put [W] back to [src].</span>")
		user.drop_held_item()
		cdel(W)
		return
		...
		if (unwrenched==0)
			...
			to_chat(user, "<span style='color:#0000FF'>You begin to unfasten \the [src] from the floor...</span>")
			if (do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
				user.visible_message( \
					"[user] unfastens \the [src].", \
					"<span style='color:#0000FF'>You have unfastened \the [src]. Now it can be pulled somewhere else.</span>", \
					"You hear ratchet.")
				src.anchored = 0
				src.stat |= MAINT
				src.unwrenched = 1
			...
			playsound(src.loc, 'sound/items/Ratchet.ogg', 25, 1)
			to_chat(user, "<span style='color:#0000FF'>You begin to fasten \the [src] to the floor...</span>")
			if (do_after(user, 20, TRUE, 5, BUSY_ICON_BUILD))
				user.visible_message( \
					"[user] fastens \the [src].", \
					"<span style='color:#0000FF'>You have fastened \the [src]. Now it can dispense pipes.</span>", \
					"You hear ratchet.")
				src.anchored = 1
				src.stat &= ~MAINT
				src.unwrenched = 0
