...
/obj/machinery/iv_drip/examine(mob/user)
	...
	if(beaker)
		if(beaker.reagents && beaker.reagents.reagent_list.len)
			to_chat(user, "<span style='color:#0000FF'>Attached is \a [beaker] with [beaker.reagents.total_volume] units of liquid.</span>")
		else
			to_chat(user, "<span style='color:#0000FF'>Attached is an empty [beaker].</span>")
	else
		to_chat(user, "<span style='color:#0000FF'>No chemicals are attached.</span>")

	to_chat(user, "<span style='color:#0000FF'>[attached ? attached : "No one"] is attached.</span>")
