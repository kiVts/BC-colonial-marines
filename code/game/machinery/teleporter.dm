		...
		if(istype(L, /obj/effect/landmark/) && istype(L.loc, /turf))
			...
			if(C.data == "Clown Land")
				...
				for(var/mob/O in hearers(src, null))
					O.show_message("<span style='color:#FF0000'>Incoming bluespace portal detected, unable to lock in.</span>", 2)
				...
				for(var/mob/O in hearers(src, null))
					O.show_message("<span style='color:#0000FF'>Locked In</span>", 2)
				src.locked = L
				one_time_use = 1
...
/obj/machinery/teleport/hub/proc/teleport(atom/movable/M as mob|obj)
	...
	if (!com.locked)
		for(var/mob/O in hearers(src, null))
			O.show_message("<span style='color:#FF0000'>Failure: Cannot authenticate locked on coordinates. Please reinstate coordinate matrix.</span>")
		return
	if (istype(M, /atom/movable))
		...
		for(var/mob/B in hearers(src, null))
			B.show_message("<span style='color:#0000FF'>Test fire completed.</span>")
	return
/*
/proc/do_teleport(atom/movable/M as mob|obj, atom/destination, precision)
	...
	if (istype(M, /obj/item/disk/nuclear)) // Don't let nuke disks get teleported --NeoFite
		for(var/mob/O in viewers(M, null))
			O.show_message(text("<span style='color:#FF0000'><B>The [] bounces off of the portal!</B></span>", M.name), 1)
		return
	if (istype(M, /mob/living))
		var/mob/living/MM = M
		if(MM.check_contents_for(/obj/item/disk/nuclear))
			to_chat(MM, "<span style='color:#FF0000'>Something you are carrying seems to be unable to pass through the portal. Better drop it if you want to go through.</span>")
			return
	var/disky = 0
	...
	if (disky)
		for(var/mob/P in viewers(M, null))
			P.show_message(text("<span style='color:#FF0000'><B>The [] bounces off of the portal!</B></span>", M.name), 1)
		return
...
//Bags of Holding cause bluespace teleportation to go funky. --NeoFite
	if (istype(M, /mob/living))
		...
		if(MM.check_contents_for(/obj/item/storage/backpack/holding))
			to_chat(MM, "<span style='color:#FF0000'>The Bluespace interface on your Bag of Holding interferes with the teleport!</span>")
...
/obj/machinery/teleport/station/proc/engage()
	...
	if (com)
		...
		use_power(5000)
		for(var/mob/O in hearers(src, null))
			O.show_message("<span style='color:#0000FF'>Teleporter engaged!</span>", 2)
	src.add_fingerprint(usr)
	src.engaged = 1
	return
...
/obj/machinery/teleport/station/proc/disengage()
	...
	if (com)
		...
		com.accurate = 0
		for(var/mob/O in hearers(src, null))
			O.show_message("<span style='color:#0000FF'>Teleporter disengaged!</span>", 2)
	src.add_fingerprint(usr)
	src.engaged = 0
	return
...
/obj/machinery/teleport/station/verb/testfire()
	...
	if (com && !active)
		active = 1
		for(var/mob/O in hearers(src, null))
			O.show_message("<span style='color:#0000FF'>Test firing!</span>", 2)
		com.teleport()
		use_power(5000)
