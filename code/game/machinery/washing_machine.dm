...
/obj/machinery/washing_machine/attackby(obj/item/W as obj, mob/user as mob)
	/*if(istype(W,/obj/item/tool/screwdriver))
		panel = !panel
		to_chat(user, "<span style='color:#0000FF'>you [panel ? "open" : "close"] the [src]'s maintenance panel</span>")*/
...
//			return
		...
		if(contents.len < 5)
			if ( state in list(1, 3) )
				...
				to_chat(user, "<span style='color:#0000FF'>You can't put the item in right now.</span>")
		else
			to_chat(user, "<span style='color:#0000FF'>The washing machine is full.</span>")
	else
		..()
	update_icon()
...
/obj/machinery/washing_machine/attack_hand(mob/user as mob)
	switch(state)
		...
		if(4)
			...
			state = 1
		if(5)
			to_chat(user, "<span style='color:#FF0000'>The [src] is busy.</span>")
		if(6)
			state = 7
		if(7)
			if(gibs_ready)
