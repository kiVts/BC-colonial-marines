...
/obj/machinery/sparker/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/tool/screwdriver))
		...
		if (src.disable)
			user.visible_message("<span style='color:#FF0000'>[user] has disabled the [src]!", "\red You disable the connection to the [src].</span>")
			icon_state = "[base_state]-d"
		if (!src.disable)
			user.visible_message("<span style='color:#FF0000'>[user] has reconnected the [src]!", "\red You fix the connection to the [src].</span>")
			if(src.powered())
				icon_state = "[base_state]"
			else
				icon_state = "[base_state]-p"
