							...
							if(H.mind.cm_skills)
								...
								switch(H.mind.cm_skills.spec_weapons)
									...
									if(SKILL_SPEC_TRAINED)
										set_name = pick("Sniper Set", "Pyro Set", "Scout Set", "Heavy Grenadier Set", "Demolitionist Set", "Stormtrooper Set") // If spec go into cryo, get !!RANDOME!! AVAILABLE SET
...
/obj/machinery/cryopod/attackby(obj/item/W, mob/living/user)
	...
	if(istype(W, /obj/item/grab))
		...
		if(willing)
			...
			message_admins("<span style='color:#0000FF'>[key_name_admin(M)] has entered a stasis pod.</span>")
