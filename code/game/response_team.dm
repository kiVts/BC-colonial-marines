...
/client/proc/response_team()
	...
	if(!holder)
		to_chat(usr, "<span style='color:#FF0000'>Only administrators may use this command.</span>")
		return
	if(!ticker)
		to_chat(usr, "<span style='color:#FF0000'>The game hasn't started yet!</span>")
		return
	if(ticker.current_state == 1)
		to_chat(usr, "<span style='color:#FF0000'>The round hasn't started yet!</span>")
		return
	if(send_emergency_team)
		to_chat(usr, "<span style='color:#FF0000'>Central Command has already dispatched an emergency response team!</span>")
		return
	...
	if(get_security_level() != "red") // Allow admins to reconsider if the alert level isn't Red
		switch(alert("The station is not in red alert. Do you still want to dispatch a response team?",,"Yes","No"))
			if("No")
				return
	if(send_emergency_team)
		to_chat(usr, "<span style='color:#FF0000'>Looks like somebody beat you to it!</span>")
		return
...
client/verb/JoinResponseTeam()
	...
	/*	if(admin_emergency_team)
		...
		for (var/obj/effect/landmark/L in landmarks_list) if (L.name == "Commando")
			...
			to_chat(new_commando, "<span style='color:#0000FF'>You are [!leader_selected?"a member":"the <B>LEADER</B>"] of an Emergency Response Team, a type of military division, under CentComm's service. There is a code red alert on [station_name()], you are tasked to go and fix the problem.</span>")
...
/client/proc/create_response_team(obj/spawn_location, leader_selected = 0, commando_name)
	...
	//to_chat(usr, "<span style='color:#FF0000'>ERT has been temporarily disabled. Talk to a coder.</span>")
	//return
