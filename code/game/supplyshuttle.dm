...
/obj/machinery/computer/supplycomp/attackby(I as obj, user as mob)
	if(istype(I,/obj/item/card/emag) && !hacked)
		to_chat(user, "<span style='color:#0000FF'>Special supplies unlocked.</span>")
		hacked = 1
		return
	else
		..()
...
/obj/machinery/computer/supplycomp/Topic(href, href_list)
	...
	if (href_list["cancel_send"])
		...
		if(supply_controller.shoppinglist.len > 20)
			to_chat(usr, "<span style='color:#FF0000'>Current retrieval load has reached maximum capacity.</span>")
			return
