	...
	switch(state)
		if("01")
			if(istype(W, /obj/item/tool/weldingtool) && !anchored )
				...
				if (WT.remove_fuel(0,user))
					...
					if(do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
						...
						to_chat(user, "<span style='color:#0000FF'>You dissasembled the windoor assembly!</span>")
						new /obj/item/stack/sheet/glass/reinforced(get_turf(src), 5)
						if(secure)
							new /obj/item/stack/rods(get_turf(src), 4)
						cdel(src)
				else
					to_chat(user, "<span style='color:#0000FF'>You need more welding fuel to dissassemble the windoor assembly.</span>")
					return
			...
			if(istype(W, /obj/item/tool/wrench) && !anchored)
				...
				if(do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
					if(!src) return
					to_chat(user, "<span style='color:#0000FF'>You've secured the windoor assembly!</span>")
					src.anchored = 1
					if(src.secure)
			...
			//Unwrenching an unsecure assembly un-anchors it. Step 4 undone
				...
				if(do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
					if(!src) return
					to_chat(user, "<span style='color:#0000FF'>You've unsecured the windoor assembly!</span>")
					src.anchored = 0
					if(src.secure)
		...
		if("02")
			...
			if(istype(W, /obj/item/tool/wirecutters) && !src.electronics)
				...
				if(do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
					...
					to_chat(user, "<span style='color:#0000FF'>You cut the windoor wires.!</span>")
			...
			//Adding airlock electronics for access. Step 6 complete.
				...
				if(do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
					...
					user.drop_held_item()
					W.loc = src
					to_chat(user, "<span style='color:#0000FF'>You've installed the airlock electronics!</span>")
			...
			//Screwdriver to remove airlock electronics. Step 6 undone.
				...
				if(do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
					...
					to_chat(user, "<span style='color:#0000FF'>You've removed the airlock electronics!</span>")
					if(src.secure)
						src.name = "Secure Wired Windoor Assembly"
					else
						src.name = "Wired Windoor Assembly"
			...
			//Crowbar to complete the assembly, Step 7 complete.
				...
				if(!src.electronics)
					to_chat(usr, "<span style='color:#FF0000'>The assembly is missing electronics.</span>")
					return
				...
				if(do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
					...
					to_chat(user, "<span style='color:#0000FF'>You finish the windoor!</span>")
