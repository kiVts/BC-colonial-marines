...
/obj/structure/device/piano/attackby(obj/item/O as obj, mob/user as mob)
	if (istype(O, /obj/item/tool/wrench))
		if (anchored)
			...
			to_chat(user, "<span style='color:#0000FF'>You begin to loosen \the [src]'s casters...</span>")
			if (do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
				user.visible_message( \
					"[user] loosens \the [src]'s casters.", \
					"<span style='color:#0000FF'>You have loosened \the [src]. Now it can be pulled somewhere else.</span>", \
					"You hear ratchet.")
				src.anchored = 0
		else
			playsound(src.loc, 'sound/items/Ratchet.ogg', 25, 1)
			to_chat(user, "<span style='color:#0000FF'>You begin to tighten \the [src] to the floor...</span>")
			if (do_after(user, 20, TRUE, 5, BUSY_ICON_BUILD))
				user.visible_message( \
					"[user] tightens \the [src]'s casters.", \
					"<span style='color:#0000FF'>You have tightened \the [src]'s casters. Now it can be played again.</span>", \
					"You hear ratchet.")
				src.anchored = 1
	else
		..()
