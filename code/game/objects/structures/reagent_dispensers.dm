...
/obj/structure/reagent_dispensers/examine(mob/user)
	...
	to_chat(user, "<span style='color:#0000FF'>It contains:</span>")
	if(reagents && reagents.reagent_list.len)
		for(var/datum/reagent/R in reagents.reagent_list)
			to_chat(user, "<span style='color:#0000FF'>[R.volume] units of [R.name]</span>")
	else
		to_chat(user, "<span style='color:#0000FF'>Nothing.</span>")
...
/obj/structure/reagent_dispensers/fueltank/examine(mob/user)
	...
	if(user != loc) return
	if(modded)
		to_chat(user, "<span style='color:#FF0000'>Fuel faucet is wrenched open, leaking the fuel!</span>")
	if(rig)
...
/obj/structure/reagent_dispensers/fueltank/attack_hand()
	if (rig)
		...
		if(do_after(usr, 20, TRUE, 5, BUSY_ICON_BUILD))
			usr.visible_message("<span style='color:#0000FF'>[usr] detaches [rig] from \the [src].", "\blue  You detach [rig] from \the [src]</span>")
...
/obj/structure/reagent_dispensers/fueltank/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(W,/obj/item/device/assembly_holder))
		if (rig)
			to_chat(user, "<span style='color:#FF0000'>There is another device in the way.</span>")
			return ..()
		user.visible_message("[user] begins rigging [W] to \the [src].", "You begin rigging [W] to \the [src]")
		if(do_after(user, 20, TRUE, 5, BUSY_ICON_HOSTILE))
			user.visible_message("<span style='color:#0000FF'>[user] rigs [W] to \the [src].", "\blue  You rig [W] to \the [src]</span>")
