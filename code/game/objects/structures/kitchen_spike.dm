	...
	attackby(obj/item/grab/G, mob/user)
		...
		if(istype(G.grabbed_thing, /mob/living/carbon/monkey))
			...
			if(!occupied)
				...
				meat = 5
				meattype = 1
				visible_message("<span style='color:#FF0000'>[user] has forced [M] onto the spike, killing them instantly!</span>")
				M.death()
				cdel(M)
				...
				to_chat(user, "<span style='color:#FF0000'>The spike already has something on it, finish collecting its meat first!</span>")
		else
			to_chat(user, "<span style='color:#FF0000'>They are too big for the spike, try something smaller!</span>")
			return
