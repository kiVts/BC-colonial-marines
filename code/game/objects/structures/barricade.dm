// Snow, wood, sandbags, metal, plasteel, folding
...
/obj/structure/barricade/Bumped(atom/A)
	...
	if(istype(A, /mob/living/carbon/Xenomorph/Queen))
		...
		var/mob/living/carbon/Xenomorph/Queen/C = A
...
/obj/structure/barricade/razor/New()
	..()
	spawn(0)
		update_icon()

/obj/structure/barricade/razor/hitby(atom/movable/AM)
	if(AM.throwing && is_wired)
		if(iscarbon(AM))
			var/mob/living/carbon/C = AM
			C.visible_message("<span class='danger'>The barbed wire slices into [C]!</span>",
...
/obj/structure/barricade/razor/attackby(obj/item/W, mob/user)
...
/obj/structure/barricade/razor/attack_alien(mob/living/carbon/Xenomorph/M)
	M.animation_attack_on(src)
	health -= rand(M.melee_damage_lower, M.melee_damage_upper)
	if(barricade_hitsound)
		playsound(src, barricade_hitsound, 25, 1)
...
/obj/structure/barricade/razor/bullet_act(var/obj/item/projectile/P)
	bullet_ping(P)
...
/obj/structure/barricade/razor/proc/destroyed(deconstruct = FALSE)
	if(deconstruct)
...
/*
 /obj/structure/razorwire/New()
	..()
	for(var/obj/structure/barricade/razor/T in loc)
		if(T != src)
			cdel(T)

 /obj/structure/barricade/razor/Destroy()
	. = ..()
...
/obj/structure/barricade/razor/attackby(obj/item/W, mob/user)
	...
	if(istype(W, /obj/item/tool/shovel) && user.a_intent != "hurt")
		...
		if(ET.folded)
			return
		if(user.action_busy)
			user  << "<span style='color:#FF0000'>You are already shoveling!</span>"
			return
		...
		if(!do_after(user, ET.shovelspeed, TRUE, 5, BUSY_ICON_BUILD))
			return
		if(!ET.folded)
			user.visible_message("<span style='color:#0000FF'>\The [user] removes \the [src].</span>")
			destroy(TRUE)
		return
	else
		. = ..()
...
/obj/structure/barricade/turnets
	name = "hand rail"
	desc = "That's a handrail. A railing, for your hands. Woooow. It appears to be in good shape. The handrail is safely secured to the ground."
	icon_state = "turnets_0"
	health = 200
	maxhealth = 200
	crusher_resistant = TRUE
	barricade_resistance = 10
...
/obj/structure/barricade/turnets/ex_act(severity)
	switch(severity)
		if(1)
			health -= rand(400, 600)
		if(2)
...
/obj/structure/barricade/turnets/bullet_act(obj/item/projectile/P)
	bullet_ping(P)
...
/obj/structure/barricade/folding
	...
	desc = "A plasteel-carbon composite barricade. Resistant to most acids while being simple to repair. There are two pushplates that allow this barricade to fold into a neat package. Use a blowtorch to repair. Can be worn on the backpack slot once folded."
...
/obj/structure/barricade/folding/attackby(obj/item/W, mob/user)
...
/obj/structure/barricade/folding/bullet_act(obj/item/projectile/P)
	bullet_ping(P)
	health -= round(P.damage/10)
...
/obj/item/folding
	...
	desc = "A plasteel-carbon composite barricade. Resistant to most acids while being simple to repair. There are two pushplates that allow this barricade to fold into a neat package. Use a blowtorch to repair. Can be worn on the backpack slot once folded."
	icon = 'icons/marine/barricades.dmi'
	icon_state = "mb_folding"
	w_class = 2 //Fits in a backpack
	drag_delay = 4 //Pulling something on wheels is easy
	var/rollertype = /obj/structure/barricade/folding

/obj/item/folding/attack_self(mob/user)
	deploy_folding(user, user.loc)

/obj/item/folding/afterattack(obj/target, mob/user , proximity)
	if(!proximity)
		return
	if(isturf(target))
		var/turf/T = target
		if(!T.density)
			deploy_folding(user, target)

/obj/item/folding/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W, /obj/item/folding_holder) && rollertype == /obj/structure/barricade/folding)
		var/obj/item/folding_holder/RH = W
		if(!RH.held)
...
/obj/item/folding/proc/deploy_folding(mob/user, atom/location)
	var/obj/structure/barricade/folding/R = new rollertype(location)
	R.add_fingerprint(user)
	user.temp_drop_inv_item(src)
	cdel(src)

/obj/item/folding_holder
	name = "MB-6 Folding Barricade"
	desc = "A plasteel-carbon composite barricade. Resistant to most acids while being simple to repair. There are two pushplates that allow this barricade to fold into a neat package. Use a blowtorch to repair. Can be worn on the backpack slot once folded."
	icon = 'icons/marine/barricades.dmi'
	icon_state = "mb_folding"
	var/obj/item/folding/held

/obj/item/folding_holder/New()
	..()
	held = new /obj/item/folding(src)

/obj/item/folding_holder/attack_self(mob/user as mob)

	if(!held)
		to_chat(user, "<span class='warning'>The rack is empty.</span>")
		return

	var/obj/structure/barricade/folding/R = new(user.loc)
