	...
	if(health > 0)
		if(istype(W, /obj/item/tool/wrench))
			if(!anchored)
				...
				to_chat(user, "<span style='color:#0000FF'>Now securing the girder</span>")
				if(do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
					to_chat(user, "<span style='color:#0000FF'>You secured the girder!</span>")
				...
				if(do_after(user,15, TRUE, 5, BUSY_ICON_BUILD))
					dismantlectr++
					health -= 15
					to_chat(user, "<span style='color:#0000FF'>You unfasten a bolt from the girder!</span>")
				return


		else if(istype(W, /obj/item/tool/pickaxe/plasmacutter))
			to_chat(user, "<span style='color:#0000FF'>Now slicing apart the girder</span>")
			if(do_after(user,30, TRUE, 5, BUSY_ICON_HOSTILE))
				if(!src) return
				to_chat(user, "<span style='color:#0000FF'>You slice apart the girder!</span>")
				health = 0
				update_state()
		else if(istype(W, /obj/item/tool/pickaxe/diamonddrill))
			to_chat(user, "<span style='color:#0000FF'>You drill through the girder!</span>")
			dismantle()

		else if(istype(W, /obj/item/tool/screwdriver) && state == 2 && istype(src,/obj/structure/girder/reinforced))
			playsound(src.loc, 'sound/items/Screwdriver.ogg', 25, 1)
			to_chat(user, "<span style='color:#0000FF'>Now unsecuring support struts</span>")
			if(do_after(user,40, TRUE, 5, BUSY_ICON_BUILD))
				if(!src) return
				to_chat(user, "<span style='color:#0000FF'>You unsecured the support struts!</span>")
				state = 1

		else if(istype(W, /obj/item/tool/wirecutters) && istype(src,/obj/structure/girder/reinforced) && state == 1)
			playsound(src.loc, 'sound/items/Wirecutter.ogg', 25, 1)
			to_chat(user, "<span style='color:#0000FF'>Now removing support struts</span>")
			if(do_after(user,40, TRUE, 5, BUSY_ICON_BUILD))
				if(!src) return
				to_chat(user, "<span style='color:#0000FF'>You removed the support struts!</span>")
				new/obj/structure/girder( src.loc )
				cdel(src)

		else if(istype(W, /obj/item/tool/crowbar) && state == 0 && anchored )
			playsound(src.loc, 'sound/items/Crowbar.ogg', 25, 1)
			to_chat(user, "<span style='color:#0000FF'>Now dislodging the girder...</span>")
			if(do_after(user, 40, TRUE, 5, BUSY_ICON_BUILD))
				if(!src) return
				to_chat(user, "<span style='color:#0000FF'>You dislodged the girder!</span>")
			...
			if (WT.remove_fuel(0,user))
				...
				if(do_after(user,30, TRUE, 5, BUSY_ICON_BUILD))
					...
					if (buildctr >= 5)
						...
						return
					buildctr++
					to_chat(user, "<span style='color:#0000FF'>You weld the metal to the girder!</span>")
			return
			...
			if(do_after(user,15, TRUE, 5, BUSY_ICON_BUILD))
				...
				health -= 15
				dismantlectr++
				to_chat(user, "<span style='color:#0000FF'>You cut away from structural piping!</span>")
			return
			...
			if (P.pipe_type in list(0, 1, 5))	//simple pipes, simple bends, and simple manifolds.
				user.drop_held_item()
				P.loc = src.loc
				to_chat(user, "<span style='color:#0000FF'>You fit the pipe into the [src]!</span>")
		else
	else
		if (repair_state == 0)
		...
		if (repair_state == 1)
			if(istype(W, /obj/item/tool/weldingtool))
				if(do_after(user,30, TRUE, 5, BUSY_ICON_BUILD))
					...
					to_chat(user, "<span style='color:#0000FF'>You weld the girder together!</span>")
					repair()
				return
		..()
