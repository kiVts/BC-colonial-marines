...
/obj/structure/morgue_tray/MouseDrop_T(atom/movable/O, mob/user)
	...
	if (user != O)
		for(var/mob/B in viewers(user, 3))
			B.show_message("<span style='color:#FF0000'>[user] stuffs [O] into [src]!</span>", 1)
...
/obj/structure/morgue/crematorium/proc/cremate(mob/user)
	...
	if(contents.len <= 1) //1 because the tray is inside.
		visible_message("<span style='color:#FF0000'>You hear a hollow crackle.</span>")
	else
		visible_message("<span style='color:#FF0000'>You hear a roar as the crematorium activates.</span>")
