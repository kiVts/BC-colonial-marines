...
/obj/structure/table/Crossed(atom/movable/O)
	..()										//	 || istype(O,/mob/living/carbon/Xenomorph/Queen
...
/obj/structure/rack/Crossed(atom/movable/O)
	..()											//	 || istype(O,/mob/living/carbon/Xenomorph/Queen
	if(istype(O,/mob/living/carbon/Xenomorph/Ravager) || istype(O,/mob/living/carbon/Xenomorph/Queen) || istype(O,/mob/living/carbon/Xenomorph/Crusher))
		var/mob/living/carbon/Xenomorph/M = O
		if(!M.stat) //No dead xenos jumpin on the bed~
			visible_message("<span class='danger'>[O] plows straight through [src]!</span>")
			destroy()
