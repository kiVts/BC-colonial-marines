	...
	if (M.stat < 2 && M.health < 50 && prob(90))
		if(ishuman(M))
			...
			if(istype(P) && P.flags_inventory & BLOCKSHARPOBJ && prob(80))
				to_chat(M, "<span style='color:#FF0000'>The helmet protects you from being hit hard in the head!</span>")
				return
		var/time = rand(2, 6)
		if (prob(75))
			M.KnockOut(time)
		...
		for(var/mob/O in viewers(M, null))
			O.show_message(text("<span style='color:#FF0000'><B>[] has been knocked unconscious!</B>", M), 1, "\red You hear someone fall.</span>", 2)
	else
		to_chat(M, text("<span style='color:#FF0000'>[] tried to knock you unconcious!</span>",user))
		M.eye_blurry += 3
