...
/obj/item/storage/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(isrobot(user))
		to_chat(user, "<span style='color:#0000FF'>You're a robot. No.</span>")
	...
	if(istype(W, /obj/item/tool/kitchen/tray))
		...
		if(T.calc_carry() > 0)
			if(prob(85))
				to_chat(user, "<span style='color:#FF0000'>The tray won't fit in [src].</span>")
				return
			else
				W.loc = user.loc
				if ((user.client && user.s_active != src))
					user.client.screen -= W
				W.dropped(user)
				to_chat(user, "<span style='color:#FF0000'>God damnit!</span>")
