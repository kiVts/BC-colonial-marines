...
/obj/item/storage/backpack/holding
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if(crit_fail)
			to_chat(user, "<span style='color:#FF0000'>The Bluespace generator isn't working.</span>")
			return
		if(istype(W, /obj/item/storage/backpack/holding) && !W.crit_fail)
			to_chat(user, "<span style='color:#FF0000'>The Bluespace interfaces of the two devices conflict and malfunction.</span>")
			cdel(W)
			return
		..()
	...
	proc/failcheck(mob/user as mob)
		...
		if (prob(src.reliability))
			to_chat(user, "<span style='color:#FF0000'>The Bluespace portal resists your attempt to add another item.</span>")
		else
			to_chat(user, "<span style='color:#FF0000'>The Bluespace generator malfunctions!</span>")
...
/obj/item/storage/backpack/marine/engineerpack/afterattack(obj/O as obj, mob/user as mob, proximity)
	...
	if (istype(O, /obj/structure/reagent_dispensers/fueltank) && src.reagents.total_volume < max_fuel)
		...
		to_chat(user, "<span style='color:#0000FF'>You crack the cap off the top of the pack and fill it back up again from the tank.</span>")
		playsound(src.loc, 'sound/effects/refill.ogg', 25, 1, 3)
		return
	else if (istype(O, /obj/structure/reagent_dispensers/fueltank) && src.reagents.total_volume == max_fuel)
		to_chat(user, "<span style='color:#0000FF'>The pack is already full!</span>")
		return
	..()
...
/// Pyrotechnician backpack fuel tank
/obj/item/storage/backpack/marine/engineerpack/flamethrower2
	name = "USCM Pyrotechnician G4-1 Fuel Tank"
	...
	icon_state = "flametower_tank2"
	max_fuel = 350

/obj/item/storage/backpack/marine/engineerpack/flamethrower2/attackby(obj/item/W, mob/living/user)
...
/obj/item/storage/backpack/marine/engineerpack/flamethrower
	...
	desc = "A specialized back harness that carries the Broiler-T flexible refueling system. Designed by and for USCM Pyrotechnicians. It contains all 3 of the fuel types that can be used with the M240-T Incinerator."
