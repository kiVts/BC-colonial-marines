...
/obj/item/legcuffs/beartrap
	...
	suicide_act(mob/user)
		viewers(user) << "<span style='color:#FF0000'><b>[user] is putting the [src.name] on \his head! It looks like \he's trying to commit suicide.</b></span>"
		return (BRUTELOSS)
				...
				if(ishuman(AM))
					if(isturf(src.loc))
						...
						if(H.m_intent == MOVE_INTENT_RUN)
							...
							to_chat(H, "<span style='color:#FF0000'><B>You step on \the [src]!</B></span>")
							feedback_add_details("handcuffs","B") //Yes, I know they're legcuffs. Don't change this, no need for an extra variable. The "B" is used to tell them apart.
							for(var/mob/O in viewers(H, null))
								if(O == H)
									continue
								O.show_message("<span style='color:#FF0000'><B>[H] steps on \the [src].</B></span>", 1)
