...
/obj/item/explosive/grenade/chem_grenade/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W,/obj/item/device/assembly_holder) && (!stage || stage==1) && path != 2)
		...
		if(istype(det.a_left,det.a_right.type) || (!isigniter(det.a_left) && !isigniter(det.a_right)))
			to_chat(user, "<span style='color:#FF0000'>Assembly must contain one igniter.</span>")
			return
		if(!det.secured)
			to_chat(user, "<span style='color:#FF0000'>Assembly must be secured with screwdriver.</span>")
			return
		path = 1
		to_chat(user, "<span style='color:#0000FF'>You add [W] to the metal casing.</span>")
		...
		if(stage == 1)
			path = 1
			if(beakers.len)
				to_chat(user, "<span style='color:#0000FF'>You lock the assembly.</span>")
				name = "grenade"
			else
//					to_chat(user, "<span style='color:#FF0000'>You need to add at least one beaker before locking the assembly.</span>")
				to_chat(user, "<span style='color:#0000FF'>You lock the empty assembly.</span>")
				name = "fake grenade"
			...
			if(active && prob(95))
				to_chat(user, "<span style='color:#FF0000'>You trigger the assembly!</span>")
				prime()
				return
			else
				to_chat(user, "<span style='color:#0000FF'>You unlock the assembly.</span>")
				playsound(src.loc, 'sound/items/Screwdriver.ogg', 25, 0, 6)
				name = "unsecured grenade with [beakers.len] containers[detonator?" and detonator":""]"
		...
		path = 1
		if(beakers.len == 2)
			to_chat(user, "<span style='color:#FF0000'>The grenade can not hold more containers.</span>")
			return
		else
			if(W.reagents.total_volume)
				if(user.drop_held_item())
					to_chat(user, "<span style='color:#0000FF'>You add \the [W] to the assembly.</span>")
					W.forceMove(src)
					beakers += W
					stage = 1
					name = "unsecured grenade with [beakers.len] containers[detonator?" and detonator":""]"
			else
				to_chat(user, "<span style='color:#FF0000'>\the [W] is empty.</span>")
