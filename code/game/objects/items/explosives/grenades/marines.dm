...
/obj/item/explosive/grenade/incchem
	name = "M40 HICDP incendiary grenade"
	desc = "The M40 HICDP is a small, but deceptively strong incendiary grenade, using chem fuel. It is set to detonate in 4 seconds."
	icon_state = "grenade_chemf"
	det_time = 40
	item_state = "grenade_chemf"
	...
	prime()
		spawn(0)
			flame_radius2(2, get_turf(src))
			playsound(src.loc, 'sound/weapons/gun_flamethrower2.ogg', 35, 1, 4)
			cdel(src)
		return

proc/flame_radius2(radius = 1, turf/T) //~Art updated fire.
	if(!T || !isturf(T))
		return
	if(radius < 0)
		radius = 0
	...
	for(var/obj/flamer_fire/F in range(radius,T)) // No stacking flames!
		cdel(F)
	new /obj/flamer_fire(T, 5 + rand(0,11), 15, null, radius, heat = 40, burn = 50, f_color = "blue")
...
/obj/item/explosive/grenade/phosphorus
	...
	New()
		det_time = rand(80,160)//Adds some risk to using this thing.
		..()
...
/obj/item/explosive/grenade/phosphorus/flamer_fire_act()
	var/turf/T = loc
	cdel(src)
	explosion(T, -2, -2, 4)
