...
/obj/item/reagent_container/borghypo/attack(mob/living/M as mob, mob/user as mob)
	...
	to_chat(user, "<span style='color:#0000FF'>You inject [M] with the injector.</span>")
	to_chat(M, "<span style='color:#0000FF'>[user] injects you with the injector.</span>")
	...
	if(M.reagents)
		...
		to_chat(user, "<span style='color:#0000FF'>[t] units of \red [reagent_ids[mode]] \blue injected for a total of \red [round(M.reagents.get_reagent_amount(reagent_ids[mode]))]\blue. [reagent_volumes[reagent_ids[mode]]] units remaining.</span>")
...
/obj/item/reagent_container/borghypo/attack_self(mob/user as mob)
	...
	to_chat(user, "<span style='color:#0000FF'>Synthesizer is now producing '[R.name]'.</span>")
