...
/obj/item/reagent_container/syringe/afterattack(obj/target, mob/user, proximity)
	...
	if(mode == SYRINGE_BROKEN)
		to_chat(user, "<span style='color:#FF0000'>This syringe is broken!</span>")
		return
	...
	switch(mode)
		if(SYRINGE_DRAW)
			...
			if(reagents.total_volume >= reagents.maximum_volume)
				to_chat(user, "<span style='color:#FF0000'>The syringe is full.</span>")
				return
			...
			if(ismob(target))//Blood!
				if(iscarbon(target))//maybe just add a blood reagent to all mobs. Then you can suck them dry...With hundreds of syringes. Jolly good idea.
					...
					if(T.get_blood_id() && reagents.has_reagent(T.get_blood_id()))
						to_chat(user, "<span style='color:#FF0000'>There is already a blood sample in this syringe</span>")
						return
					if(!T.dna)
						to_chat(user, "You are unable to locate any blood.")
						return
					if(NOCLONE in T.mutations) //target done been et, no more blood in him
						to_chat(user, "<span style='color:#FF0000'>You are unable to locate any blood.</span>")
						return
				...
				if(!target.reagents.total_volume)
					to_chat(user, "<span style='color:#FF0000'>[target] is empty.</span>")
					return

				if(!target.is_open_container() && !istype(target,/obj/structure/reagent_dispensers))
					to_chat(user, "<span style='color:#FF0000'>You cannot directly remove reagents from this object.</span>")
					return

				var/trans = target.reagents.trans_to(src, amount_per_transfer_from_this) // transfer from, transfer to - who cares?

				to_chat(user, "<span style='color:#0000FF'>You fill the syringe with [trans] units of the solution.</span>")
			if (reagents.total_volume >= reagents.maximum_volume)
				mode=!mode
				update_icon()
		...
		if(SYRINGE_INJECT)
			if(!reagents.total_volume)
				to_chat(user, "<span style='color:#FF0000'>The syringe is empty.</span>")
				return
			...
			if(!target.is_open_container() && !ismob(target) && !istype(target, /obj/item/reagent_container/food) && !istype(target, /obj/item/clothing/mask/cigarette) && !istype(target, /obj/item/storage/fancy/cigarettes))
				to_chat(user, "<span style='color:#FF0000'>You cannot directly fill this object.</span>")
				return
			if(target.reagents.total_volume >= target.reagents.maximum_volume)
				to_chat(user, "<span style='color:#FF0000'>[target] is full.</span>")
				return
			...
			if(ismob(target))
				if(target != user)
					...
					if(injection_time != 60)
						user.visible_message("<span style='color:#FF0000'><B>[user] is trying to inject [target]!</B></span>")
					else
						user.visible_message("<span style='color:#FF0000'><B>[user] begins hunting for an injection port on [target]'s suit!</B></span>")

					if(!do_mob(user, target, injection_time, BUSY_ICON_FRIENDLY, BUSY_ICON_MEDICAL)) return

					user.visible_message("<span style='color:#FF0000'>[user] injects [target] with the syringe!</span>")
			...
			to_chat(user, "<span style='color:#0000FF'>You inject [trans] units of the solution. The syringe now contains [src.reagents.total_volume] units.</span>")
	...
	if(istype(target, /mob/living/carbon/human))
		...
		if (target != user && target.getarmor(target_zone, "melee") > 5 && prob(50))
			for(var/mob/O in viewers(world.view, user))
				O.show_message(text("<span style='color:#FF0000'><B>[user] tries to stab [target] in \the [hit_area] with [src.name], but the attack is deflected by armor!</B></span>"), 1)
			user.temp_drop_inv_item(src)
			cdel(src)
			return

		for(var/mob/O in viewers(world.view, user))
			O.show_message(text("<span style='color:#FF0000'><B>[user] stabs [target] in \the [hit_area] with [src.name]!</B></span>"), 1)
		...
		for(var/mob/O in viewers(world.view, user))
			O.show_message(text("<span style='color:#FF0000'><B>[user] stabs [target] with [src.name]!</B></span>"), 1)
		...
		switch(mode)
			...
			if(SYRINGE_INJECT)
				...
				if(ismob(target) && target != user)
					user.visible_message("<span style='color:#FF0000'><B>[user] is trying to inject [target] with a giant syringe!</B></span>")
					if(!do_mob(user, target, 300, BUSY_ICON_FRIENDLY, BUSY_ICON_MEDICAL)) return
					user.visible_message("<span style='color:#FF0000'>[user] injects [target] with a giant syringe!</span>")
					src.reagents.reaction(target, INGEST)
				if(ismob(target) && target == user)
					src.reagents.reaction(target, INGEST)
				spawn(5)
