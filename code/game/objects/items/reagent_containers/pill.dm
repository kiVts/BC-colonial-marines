		...
		if(M == user)
			...
			if(istype(M, /mob/living/carbon/human))
				...
				if(H.species.flags & IS_SYNTHETIC)
					to_chat(H, "<span style='color:#FF0000'>You can't eat pills.</span>")
					return

			to_chat(M, "<span style='color:#0000FF'>You swallow [src].</span>")
			...
			for(var/mob/O in viewers(world.view, user))
				O.show_message("<span style='color:#FF0000'>[user] forces [M] to swallow [src].</span>", 1)
	...
	afterattack(obj/target, mob/user, proximity)
		...
		if(target.is_open_container() != 0 && target.reagents)
			if(!target.reagents.total_volume)
				to_chat(user, "<span style='color:#FF0000'>[target] is empty. Cant dissolve pill.</span>")
				return
			to_chat(user, "<span style='color:#0000FF'>You dissolve the pill in [target]</span>")
			...
			for(var/mob/O in viewers(2, user))
				O.show_message("<span style='color:#FF0000'>[user] puts something in \the [target].</span>", 1)
