...
/obj/item/reagent_container/hypospray/attack(mob/M, mob/living/user)
	...
	if (reagents.total_volume)
		...
		to_chat(user, "<span style='color:#0000FF'>You inject [M] with [src].</span>")
		to_chat(M, "<span style='color:#FF0000'>You feel a tiny prick!</span>")
		...
		if(M.reagents)
			...
			to_chat(user, "<span style='color:#0000FF'>[trans] units injected. [reagents.total_volume] units remaining in [src].</span>")
