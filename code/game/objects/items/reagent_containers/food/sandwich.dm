...
/obj/item/reagent_container/food/snacks/csandwich/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(src.contents.len > sandwich_limit)
		to_chat(user, "<span style='color:#FF0000'>If you put anything else on \the [src] it's going to collapse.</span>")
		return
	else if(istype(W,/obj/item/shard))
		to_chat(user, "<span style='color:#0000FF'>You hide [W] in \the [src].</span>")
		user.drop_inv_item_to_loc(W, src)
		update()
		return
	else if(istype(W,/obj/item/reagent_container/food/snacks))
		to_chat(user, "<span style='color:#0000FF'>You layer [W] over \the [src].</span>")
...
/obj/item/reagent_container/food/snacks/csandwich/examine(mob/user)
	...
	to_chat(user, "<span style='color:#0000FF'>You think you can see [O.name] in there.</span>")
...
/obj/item/reagent_container/food/snacks/csandwich/attack(mob/M as mob, mob/user as mob, def_zone)
	...
	if(H && shard && M == user) //This needs a check for feeding the food to other people, but that could be abusable.
		to_chat(H, "<span style='color:#FF0000'>You lacerate your mouth on a [shard.name] in the sandwich!</span>")
