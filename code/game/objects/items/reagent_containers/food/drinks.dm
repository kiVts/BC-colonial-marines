...
/obj/item/reagent_container/food/drinks
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		...
		if(!R.total_volume || !R)
			to_chat(user, "<span style='color:#FF0000'>The [src.name] is empty!</span>")
			return 0
		...
		if(M == user)
			...
			to_chat(M, "<span style='color:#0000FF'>You swallow a gulp from \the [src].</span>")
	...
	afterattack(obj/target, mob/user, proximity)
		...
		if(istype(target, /obj/structure/reagent_dispensers)) //A dispenser. Transfer FROM it TO us.
			...
			if(reagents.total_volume >= reagents.maximum_volume)
				to_chat(user, "<span style='color:#FF0000'>[src] is full.</span>")
				return

			var/trans = target.reagents.trans_to(src, target:amount_per_transfer_from_this)
			to_chat(user, "<span style='color:#0000FF'>You fill [src] with [trans] units of the contents of [target].</span>")

		else if(target.is_open_container()) //Something like a glass. Player probably wants to transfer TO it.
			if(!reagents.total_volume)
				to_chat(user, "<span style='color:#FF0000'>[src] is empty.</span>")
				return
			...
			to_chat(user, "<span style='color:#0000FF'>You transfer [trans] units of the solution to [target].</span>")
	...
	examine(mob/user)
		...
		if(!reagents || reagents.total_volume==0)
			to_chat(user, "<span style='color:#0000FF'>\The [src] is empty!</span>")
		else if (reagents.total_volume<=src.volume/4)
			to_chat(user, "<span style='color:#0000FF'>\The [src] is almost empty!</span>")
		else if (reagents.total_volume<=src.volume*0.66)
			to_chat(user, "<span style='color:#0000FF'>\The [src] is half full!</span>")
		else if (reagents.total_volume<=src.volume*0.90)
			to_chat(user, "<span style='color:#0000FF'>\The [src] is almost full!</span>")
		else
			to_chat(user, "<span style='color:#0000FF'>\The [src] is full!</span>")
