...
/obj/item/reagent_container/food/snacks/attack(mob/M, mob/user, def_zone)
	if(!reagents.total_volume)						//Shouldn't be needed but it checks to see if it has anything left in it.
		to_chat(user, "<span style='color:#FF0000'>None of [src] left, oh no!</span>")
		M.drop_inv_item_on_ground(src)	//so icons update :[
		cdel(src)
		return 0

	if(package)
		to_chat(M, "<span style='color:#FF0000'>How do you expect to eat this with the package still on?</span>")
		return 0
		...
		if(M == user)								//If you're eating it yourself
			if(istype(M,/mob/living/carbon/human))
				...
				if(H.species.flags & IS_SYNTHETIC)
					to_chat(H, "<span style='color:#FF0000'>You have a monitor for a head, where do you think you're going to put that?</span>")
					return
			if (fullness <= 50)
				to_chat(M, "<span style='color:#FF0000'>You hungrily chew out a piece of [src] and gobble it!</span>")
			if (fullness > 50 && fullness <= 150)
				to_chat(M, "<span style='color:#0000FF'>You hungrily begin to eat [src].</span>")
			if (fullness > 150 && fullness <= 350)
				to_chat(M, "<span style='color:#0000FF'>You take a bite of [src].</span>")
			if (fullness > 350 && fullness <= 550)
				to_chat(M, "<span style='color:#0000FF'>You unwillingly chew a bit of [src].</span>")
			if (fullness > (550 * (1 + M.overeatduration / 2000)))	// The more you eat - the more you can eat
				to_chat(M, "<span style='color:#FF0000'>You cannot force any more of [src] to go down your throat.</span>")
				return 0
		else
			if(istype(M,/mob/living/carbon/human))
				var/mob/living/carbon/human/H = M
				if(H.species.flags & IS_SYNTHETIC)
					to_chat(H, "<span style='color:#FF0000'>They have a monitor for a head, where do you think you're going to put that?</span>")
					return


			if (fullness <= (550 * (1 + M.overeatduration / 1000)))
				for(var/mob/O in viewers(world.view, user))
					O.show_message("<span style='color:#FF0000'>[user] attempts to feed [M] [src].</span>", 1)
			else
				for(var/mob/O in viewers(world.view, user))
					O.show_message("<span style='color:#FF0000'>[user] cannot force anymore of [src] down [M]'s throat.</span>", 1)
					return 0
			...
			for(var/mob/O in viewers(world.view, user))
				O.show_message("<span style='color:#FF0000'>[user] feeds [M] [src].</span>", 1)
...
/obj/item/reagent_container/food/snacks/examine(mob/user)
	...
	if (bitecount==0)
		...
		to_chat(user, "<span style='color:#0000FF'>\The [src] was bitten by someone!</span>")
	else if (bitecount<=3)
		to_chat(user, "<span style='color:#0000FF'>\The [src] was bitten [bitecount] times!</span>")
	else
		to_chat(user, "<span style='color:#0000FF'>\The [src] was bitten multiple times!</span>")
...
/obj/item/reagent_container/food/snacks/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W,/obj/item/tool/kitchen/utensil))
		...
		if (U.reagents.total_volume > 0)
			to_chat(user, "<span style='color:#FF0000'>You already have something on your [U].</span>")
			return

		user.visible_message( \
			"[user] scoops up some [src] with \the [U]!", \
			"<span style='color:#0000FF'>You scoop up some [src] with \the [U]!</span>" \
		)
	...
	if(W.sharp == IS_SHARP_ITEM_ACCURATE)
		...
		if(user.drop_inv_item_to_loc(W, src))
			to_chat(user, "<span style='color:#FF0000'>You slip [W] inside [src].</span>")
	...
	if ( \
		...
		to_chat(user, "<span style='color:#FF0000'>You cannot slice [src] here! You need a table or at least a tray to do it.</span>")
		return 1
	var/slices_lost = 0
	if (!inaccurate)
		user.visible_message( \
			"<span style='color:#0000FF'>[user] slices \the [src]!</span>", \
			"<span style='color:#0000FF'>You slice \the [src]!</span>" \
		)
	else
		user.visible_message( \
			"<span style='color:#0000FF'>[user] crudely slices \the [src] with [W]!</span>", \
			"<span style='color:#0000FF'>You crudely slice \the [src] with your [W]!</span>" \
		)
...
/obj/item/reagent_container/food/snacks/attack_animal(var/mob/M)
	if(isanimal(M))
		...
		if(ismouse(M))
			...
			to_chat(N, text("<span style='color:#0000FF'>You nibble away at [src].</span>"))
			if(prob(50))
...
/obj/item/reagent_container/food/snacks/egg/throw_impact(atom/hit_atom)
	...
	src.visible_message("<span style='color:#FF0000'>[src.name] has been squashed.","\red You hear a smack.</span>")
	cdel(src)
...
/obj/item/reagent_container/food/snacks/egg/attackby(obj/item/W as obj, mob/user as mob)
	if(istype( W, /obj/item/toy/crayon ))
		...
		if(!(clr in list("blue","green","mime","orange","purple","rainbow","red","yellow")))
			to_chat(usr, "<span style='color:#0000FF'>The egg refuses to take on this color!</span>")
			return

		to_chat(usr, "<span style='color:#0000FF'>You color \the [src] [clr]</span>")
		icon_state = "egg-[clr]"
		egg_color = clr
	else
		..()
...
/obj/item/reagent_container/food/snacks/pie/throw_impact(atom/hit_atom)
	...
	src.visible_message("<span style='color:#FF0000'>[src.name] splats.","\red You hear a splat.</span>")
	cdel(src)
...
/obj/item/reagent_container/food/snacks/popcorn
	...
	On_Consume()
		if(prob(unpopped))	//lol ...what's the point?
			to_chat(usr, "<span style='color:#FF0000'>You bite down on an un-popped kernel!</span>")
...
/obj/item/reagent_container/food/snacks/monkeycube
	...
	proc/Expand()
		for(var/mob/M in viewers(src,7))
			to_chat(M, "<span style='color:#FF0000'>\The [src] expands!</span>")
...
/obj/item/pizzabox/attack_hand( mob/user as mob )
	...
	if( open && pizza )
		...
		to_chat(user, "<span style='color:#FF0000'>You take the [src.pizza] out of the [src].</span>")
		src.pizza = null
		update_icon()
		return
	...
	if( boxes.len > 0 )
		...
		to_chat(user, "<span style='color:#FF0000'>You remove the topmost [src] from your hand.</span>")
		box.update_icon()
		update_icon()
		return
	..()
...
/obj/item/pizzabox/attackby( obj/item/I as obj, mob/user as mob )
	if( istype(I, /obj/item/pizzabox/) )
		...
		if( !box.open && !src.open )
			...
			if( (boxes.len+1) + boxestoadd.len <= 5 )
				...
				to_chat(user, "<span style='color:#FF0000'>You put the [box] ontop of the [src]!</span>")
			else
				to_chat(user, "<span style='color:#FF0000'>The stack is too high!</span>")
		else
			to_chat(user, "<span style='color:#FF0000'>Close the [box] first!</span>")
	...
	if( istype(I, /obj/item/reagent_container/food/snacks/sliceable/pizza/) ) // Long ass fucking object name
		...
		if(open)
			...
			to_chat(user, "<span style='color:#FF0000'>You put the [I] in the [src]!</span>")
		else
			to_chat(user, "<span style='color:#FF0000'>You try to push the [I] through the lid but it doesn't work!</span>")
		return
