...
/obj/item/reagent_container/food/condiment
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		...
		if(M == user)
			to_chat(M, "<span style='color:#0000FF'>You swallow some of contents of the [src].</span>")
	...
	afterattack(obj/target, mob/user , flag)
		...
		//Something like a glass or a food item. Player probably wants to transfer TO it.
			...
			if(target.reagents.total_volume >= target.reagents.maximum_volume)
				to_chat(user, "<span style='color:#FF0000'>you can't add anymore to [target].</span>")
				return
			var/trans = src.reagents.trans_to(target, amount_per_transfer_from_this)
			to_chat(user, "<span style='color:#0000FF'>You transfer [trans] units of the condiment to [target].</span>")
