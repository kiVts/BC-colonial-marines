...
/obj/item/implant
	...
	proc/meltdown()	//breaks it down, making implant unrecongizible
		to_chat(imp_in, "<span style='color:#FF0000'>You feel something melting inside [part ? "your [part.display_name]" : "you"]!</span>")
		if (part)
			...
			if(ishuman(imp_in))
				if (elevel == "Localized Limb")
					if(part) //For some reason, small_boom() didn't work. So have this bit of working copypaste.
						imp_in.visible_message("<span style='color:#FF0000'>Something beeps inside [imp_in][part ? "'s [part.display_name]" : ""]!</span>")
...
<b>Integrity:</b> Implant can only be used three times before the nanobots are depleted."}
	...
	trigger(emote, mob/source as mob)
		...
		if (emote == "pale")
			src.uses--
			to_chat(source, "<span style='color:#0000FF'>You feel a sudden surge of energy!</span>")
			source.SetStunned(0)
