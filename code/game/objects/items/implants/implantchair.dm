...
/obj/machinery/implantchair
	...
	put_mob(mob/living/carbon/M)
		if(!iscarbon(M))
			to_chat(usr, "<span style='color:#FF0000'><B>The [src.name] cannot hold this!</B></span>")
			return
		if(src.occupant)
			to_chat(usr, "<span style='color:#FF0000'><B>The [src.name] is already occupied!</B></span>")
			return
		M.forceMove(src)
		src.occupant = M
		src.add_fingerprint(usr)
		...
		for(var/obj/item/implant/loyalty/imp in implant_list)
			...
			if(istype(imp, /obj/item/implant/loyalty))
				for (var/mob/O in viewers(M, null))
					O.show_message("<span style='color:#FF0000'>[M] has been implanted by the [src.name].</span>", 1)
