...
/obj/item/shard/suicide_act(mob/user)
	user.visible_message("<span style='color:#FF0000'><b>[user] is slitting \his [prob(50) ? "wrists" :"throat"] with [src]! It looks like \he's trying to commit suicide.</b></span>")
	return (BRUTELOSS)
...
/obj/item/shard/Crossed(AM as mob|obj)
	if(ismob(AM))
		...
		if(!M.buckled)
			to_chat(M, "<span style='color:#FF0000'><B>You step on \the [src]!</B></span>")
			if(ishuman(M))
