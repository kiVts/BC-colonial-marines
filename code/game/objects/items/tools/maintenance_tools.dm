...
/obj/item/tool/screwdriver
	...
	suicide_act(mob/user)
		viewers(user) << pick("<span style='color:#FF0000'><b>[user] is stabbing the [src.name] into \his temple! It looks like \he's trying to commit suicide.</b></span>", \
							"<span style='color:#FF0000'><b>[user] is stabbing the [src.name] into \his heart! It looks like \he's trying to commit suicide.</b></span>")
		return(BRUTELOSS)
...
/obj/item/tool/weldingtool/attack(mob/M, mob/user)
	...
	if(hasorgans(M))
		...
		if(H.species.flags & IS_SYNTHETIC)
			if(M == user)
				to_chat(user, "<span style='color:#FF0000'>You can't repair damage to your own body - it's against OH&S.</span>")
				return
...
/obj/item/tool/weldpack/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W, /obj/item/tool/weldingtool))
		...
		if(T.welding & prob(50))
			...
			to_chat(user, "<span style='color:#FF0000'>That was stupid of you.</span>")
			...
			if(T.welding)
				to_chat(user, "<span style='color:#FF0000'>That was close!</span>")
			src.reagents.trans_to(W, T.max_fuel)
			to_chat(user, "<span style='color:#0000FF'>Welder refilled!</span>")
			playsound(src.loc, 'sound/effects/refill.ogg', 25, 1, 3)
			return
	to_chat(user, "<span style='color:#0000FF'>The tank scoffs at your insolence.  It only provides services to welders.</span>")
	return
