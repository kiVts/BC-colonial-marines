	...
	if (reagents.total_volume > 0)
		...
		if(M == user)
			for(var/mob/O in viewers(M, null))
				O.show_message(text("<span style='color:#0000FF'>[] eats some [] from \the [].</span>", user, loaded, src), 1)
				M.reagents.add_reagent("nutriment", 1)
		else
			for(var/mob/O in viewers(M, null))
				O.show_message(text("<span style='color:#0000FF'>[] feeds [] some [] from \the []</span>", user, M, loaded, src), 1)
				M.reagents.add_reagent("nutriment", 1)
...
/obj/item/tool/kitchen/utensil/knife
	...
	suicide_act(mob/user)
		viewers(user) << pick("<span style='color:#FF0000'><b>[user] is slitting \his wrists with the [src.name]! It looks like \he's trying to commit suicide.</b></span>", \
							"<span style='color:#FF0000'><b>[user] is slitting \his throat with the [src.name]! It looks like \he's trying to commit suicide.</b></span>", \
							"<span style='color:#FF0000'><b>[user] is slitting \his stomach open with the [src.name]! It looks like \he's trying to commit seppuku.</b></span>")
		return (BRUTELOSS)

/obj/item/tool/kitchen/utensil/knife/attack(target as mob, mob/living/user as mob)
	if ((CLUMSY in user.mutations) && prob(50))
		to_chat(user, "<span style='color:#FF0000'>You accidentally cut yourself with the [src].</span>")
...
/obj/item/tool/kitchen/utensil/knife/attack(target as mob, mob/living/user as mob)
	if ((CLUMSY in user.mutations) && prob(50))
		to_chat(user, "<span style='color:#FF0000'>You somehow managed to cut yourself with the [src].</span>")
		user.take_limb_damage(20)
		return
	playsound(loc, 'sound/weapons/bladeslice.ogg', 25, 1, 5)
	return ..()
...
/obj/item/tool/kitchen/rollingpin/attack(mob/living/M as mob, mob/living/user as mob)
	if ((CLUMSY in user.mutations) && prob(50))
		to_chat(user, "<span style='color:#FF0000'>The [src] slips out of your hand and hits your head.</span>")
		...
		if(ishuman(M))
			...
			if (H.stat < 2 && H.health < 50 && prob(90))
				...
				if (istype(head_protection) && head_protection.flags_inventory & BLOCKSHARPOBJ  && prob(80))
					to_chat(H, "<span style='color:#FF0000'>The helmet protects you from being hit hard in the head!</span>")
					return
				var/time = rand(2, 6)
				if (prob(75))
					H.KnockOut(time)
				...
				user.visible_message("<span style='color:#FF0000'><B>[H] has been knocked unconscious!</B>", "\red <B>You knock [H] unconscious!</B></span>")
				return
			else
				H.visible_message("<span style='color:#FF0000'>[user] tried to knock [H] unconscious!", "\red [user] tried to knock you unconscious!</span>")
				H.eye_blurry += 3
	return ..()
...
/obj/item/tool/kitchen/tray/attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	...
	if((CLUMSY in user.mutations) && prob(50))              //What if he's a clown?
		to_chat(M, "<span style='color:#FF0000'>You accidentally slam yourself with the [src]!</span>")
		M.KnockDown(1)
	...
	if(!(user.zone_selected == ("eyes" || "head"))) //////////////hitting anything else other than the eyes
		...
		if(prob(50))
			playsound(M, 'sound/items/trayhit1.ogg', 25, 1)
			for(var/mob/O in viewers(M, null))
				O.show_message(text("<span style='color:#FF0000'><B>[] slams [] with the tray!</B></span>", user, M), 1)
			return
		else
	...
	if(istype(M, /mob/living/carbon/human) && ((H.head && (H.head.flags_inventory & COVEREYES) ) || (H.wear_mask && (H.wear_mask.flags_inventory & COVEREYES) ) || (H.glasses && (H.glasses.flags_inventory & COVEREYES) )))
		to_chat(M, "<span style='color:#FF0000'>You get slammed in the face with the tray, against your mask!</span>")
		if(prob(33))
			src.add_mob_blood(H)
			if (H.wear_mask)
		...
		to_chat(M, "<span style='color:#FF0000'>You get slammed in the face with the tray!</span>")
		if(prob(33))
			src.add_mob_blood(M)
			var/turf/location = H.loc
			if (istype(location, /turf))
		...
		if(prob(50))
			playsound(M, 'sound/items/trayhit1.ogg', 25, 1)
			for(var/mob/O in viewers(M, null))
				O.show_message(text("<span style='color:#FF0000'><B>[] slams [] in the face with the tray!</B></span>", user, M), 1)
		else
