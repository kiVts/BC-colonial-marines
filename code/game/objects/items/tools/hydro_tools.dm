...
/obj/item/tool/bee_net/attack_self(mob/user as mob)
	...
	for(var/mob/living/simple_animal/bee/B in T)
		if(B.feral < 0)
			...
			cdel(B)
			user.visible_message("<span style='color:#0000FF'>[user] nets some bees.","\blue You net up some of the becalmed bees.</span>")
		else
			user.visible_message("<span style='color:#FF0000'>[user] swings at some bees, they don't seem to like it.","\red You swing at some bees, they don't seem to like it.</span>")
			B.feral = 5
			B.target_mob = user
