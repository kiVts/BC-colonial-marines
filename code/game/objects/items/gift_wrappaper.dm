...
/obj/item/gift/attack_self(mob/user as mob)
	...
	if(gift)
		...
		to_chat(user, "<span style='color:#0000FF'>The gift was empty!</span>")
	cdel(src)
	return
...
/obj/effect/spresent/relaymove(mob/user)
	if (user.stat)
		return
	to_chat(user, "<span style='color:#0000FF'>You cant move.</span>")

/obj/effect/spresent/attackby(obj/item/W as obj, mob/user as mob)
	..()

	if (!istype(W, /obj/item/tool/wirecutters))
		to_chat(user, "<span style='color:#0000FF'>I need wirecutters for that.</span>")
		return

	to_chat(user, "<span style='color:#0000FF'>You cut open the present.</span>")
...
/obj/item/wrapping_paper/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (!( locate(/obj/structure/table, src.loc) ))
		to_chat(user, "<span style='color:#0000FF'>You MUST put the paper on a table!</span>")
	if (W.w_class < 4)
		if ((istype(user.l_hand, /obj/item/tool/wirecutters) || istype(user.r_hand, /obj/item/tool/wirecutters)))
			var/a_used = 2 ** (src.w_class - 1)
			if (src.amount < a_used)
				to_chat(user, "<span style='color:#0000FF'>You need more paper!</span>")
				return
			else
			...
			to_chat(user, "<span style='color:#0000FF'>You need scissors!</span>")
	else
		to_chat(user, "<span style='color:#0000FF'>The object is FAR too large!</span>")
	return
