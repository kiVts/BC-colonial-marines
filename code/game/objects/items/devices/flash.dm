...
/obj/item/device/flash/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 88921)
	else
...
/obj/item/device/flash/proc/clown_check(var/mob/user)
	if(user && (CLUMSY in user.mutations) && prob(50))
		if(user.drop_held_item())
			to_chat(user, "<span style='color:#FF0000'>\The [src] slips out of your hand.</span>")
		return 0
	return 1
...
/obj/item/device/flash/synthetic/attack(mob/living/M as mob, mob/user as mob)
	...
	if(!broken)
		broken = 1
		to_chat(user, "<span style='color:#FF0000'>The bulb has burnt out!</span>")
		icon_state = "flashburnt"
