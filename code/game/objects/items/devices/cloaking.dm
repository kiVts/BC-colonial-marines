...
/obj/item/device/cloaking_device/attack_self(mob/user as mob)
	...
	if (src.active)
		to_chat(user, "<span style='color:#0000FF'>The cloaking device is now active.</span>")
		src.icon_state = "shield1"
	else
		to_chat(user, "<span style='color:#0000FF'>The cloaking device is now inactive.</span>")
