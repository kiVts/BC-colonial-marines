...
/obj/item/device/gripper/verb/drop_item()
	...
	to_chat(src.loc, "<span style='color:#FF0000'>You drop \the [wrapped].</span>")
...
/obj/item/device/gripper/afterattack(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, proximity, params)
	...
	if(wrapped) //Already have an item.
		...
		if(grab)
			...
			to_chat(user, "<span style='color:#FF0000'>Your gripper cannot hold \the [target].</span>")
		...
		if(A.opened)
			if(A.cell)
				...
				user.visible_message("<span style='color:#FF0000'>[user] removes the power cell from [A]!", "You remove the power cell.</span>")
...
/obj/item/device/matter_decompiler/afterattack(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, proximity, params)
	...
	for(var/mob/M in T)
		if(istype(M,/mob/living/simple_animal/lizard) || istype(M,/mob/living/simple_animal/mouse))
			src.loc.visible_message("<span style='color:#FF0000'>[src.loc] sucks [M] into its decompiler. There's a horrible crunching noise.","\red It's a bit of a struggle, but you manage to suck [M] into your decompiler. It makes a series of visceral crunching noises.</span>")
			...
			to_chat(D, "<span style='color:#FF0000'>You begin decompiling the other drone.</span>")

			if(!do_after(D, 50, FALSE, 5, BUSY_ICON_GENERIC))
				to_chat(D, "<span style='color:#FF0000'>You need to remain still while decompiling such a large object.</span>")
				return

			if(!M || !D) return

			to_chat(D, "<span style='color:#FF0000'>You carefully and thoroughly decompile your downed fellow, storing as much of its resources as you can within yourself.</span>")
	...
	if(grabbed_something)
		to_chat(user, "<span style='color:#0000FF'>You deploy your decompiler and clear out the contents of \the [T].</span>")
	else
		to_chat(user, "<span style='color:#FF0000'>Nothing on \the [T] is useful to you.</span>")
	return