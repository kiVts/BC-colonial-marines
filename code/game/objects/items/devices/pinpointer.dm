/obj/item/device/pinpointer
	...
	attack_self()
		if(!active)
			active = 1
			workdisk()
			to_chat(usr, "<span style='color:#0000FF'>You activate the pinpointer</span>")
		else
			active = 0
			icon_state = "pinoff"
			to_chat(usr, "<span style='color:#0000FF'>You deactivate the pinpointer</span>")
...
/obj/item/device/pinpointer/nukeop
	var/mode = 0	//Mode 0 locates disk, mode 1 locates the shuttle
	var/obj/machinery/computer/shuttle_control/multi/syndicate/home = null

/obj/item/device/pinpointer/nukeop/attack_self(mob/user as mob)
	if(!active)
		active = 1
		if(!mode)
			workdisk()
...
/obj/item/device/pinpointer/nukeop/proc/worklocation()
	if(!active)	return
	if(!mode)
		workdisk()
		return
