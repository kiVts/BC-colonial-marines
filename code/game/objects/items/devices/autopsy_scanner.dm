...
/obj/item/device/autopsy_scanner/verb/print_data()
	...
	for(var/mob/O in viewers(usr))
		O.show_message("<span style='color:#FF0000'>\the [src] rattles and prints out a sheet of paper.</span>", 1)
...
/obj/item/device/autopsy_scanner/attack(mob/living/carbon/human/M as mob, mob/living/carbon/user as mob)
	...
	if(target_name != M.name)
		...
		to_chat(user, "<span style='color:#FF0000'>A new patient has been registered.. Purging data for previous patient.</span>")
	...
	for(var/mob/O in viewers(M))
		O.show_message("<span style='color:#FF0000'>[user.name] scans the wounds on [M.name]'s [S.display_name] with \the [src.name]</span>", 1)
