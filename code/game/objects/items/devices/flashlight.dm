...
/obj/item/device/flashlight/flare/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
...
/obj/item/device/flashlight/s_flare
	name = "signal flare"
	desc = "A green USCM issued flare. There are instructions on the side, it reads 'pull cord, make light' and call dropship."
	var/serial_number = 0
	w_class = 2
	brightness_on = 5 //As bright as a flashlight, but more disposable. Doesn't burn forever though
	icon_state = "s_flare"
...
/obj/item/device/flashlight/s_flare/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 897826)
	else
		serial_number = given_serial
...
/obj/item/device/flashlight/s_flare/New()
	fuel = rand(800, 1000) // Sorry for changing this so much but I keep under-estimating how long X number of ticks last in seconds.
	..()

/obj/item/device/flashlight/s_flare/Dispose()
	processing_objects -= src
	..()

/obj/item/device/flashlight/s_flare/process()
	fuel = max(fuel - 1, 0)
	if(!fuel || !on)
		turn_off()
		if(!fuel)
...
/obj/item/device/flashlight/s_flare/proc/turn_off()
	on = 0
	heat_source = 0
	force = initial(force)
	damtype = initial(damtype)
...
/obj/item/device/flashlight/s_flare/attack_alien(var/mob/living/carbon/Xenomorph/M)
	return

/obj/item/device/flashlight/s_flare/attack_self(mob/user)
...
/obj/item/device/flashlight/s_flare/on
