...
//SYNDICATE FUNCTIONS===================================
		...
		if("Detonate")//Detonate PDA... maybe
			...
			if(istype(cartridge, /obj/item/cartridge/syndicate))
				if(!(useMS && useTC))
					U.show_message("<span style='color:#FF0000'>An error flashes on your [src]: Connection unavailable</span>", 1)
					return
				if(useTC != 2) // Does our recepient have a broadcaster on their level?
					U.show_message("<span style='color:#FF0000'>An error flashes on your [src]: Recipient unavailable</span>", 1)
					return
				var/obj/item/device/pda/P = locate(href_list["target"])
				if(!isnull(P))
					if (!P.toff && cartridge.charges > 0)
						...
						if(prob(difficulty))
							U.show_message("<span style='color:#FF0000'>An error flashes on your [src].</span>", 1)
						else if (prob(difficulty * 7))
							U.show_message("<span style='color:#FF0000'>Energy feeds back into your [src]!</span>", 1)
							ui.close()
							detonate_act(src)
							...
							U.show_message("<span style='color:#0000FF'>Success!</span>", 1)
...
/obj/item/device/pda/proc/create_message(var/mob/living/U = usr, var/obj/item/device/pda/P, var/tap = 1)
	...
	if(useMS && useTC) // only send the message if it's stable
		...
		usr.log_talk(t, LOG_PDA, tag="PDA: [name] to [P.name]")
		P.overlays.Cut()
		...
		switch(scanmode)
			if(1)
				...
				for (var/mob/O in viewers(C, null))
					O.show_message("<span style='color:#FF0000'>[user] has analyzed [C]'s vitals!</span>", 1)

				user.show_message("<span style='color:#0000FF'>Analyzing Results for [C]:</span>")
				user.show_message("<span style='color:#0000FF'>\t Overall Status: [C.stat > 1 ? "dead" : "[C.health - C.halloss]% healthy"]</span>", 1)
				user.show_message("<span style='color:#0000FF'>\t Damage Specifics: [C.getOxyLoss() > 50 ? "\red" : "\blue"][C.getOxyLoss()]-[C.getToxLoss() > 50 ? "\red" : "\blue"][C.getToxLoss()]-[C.getFireLoss() > 50 ? "\red" : "\blue"][C.getFireLoss()]-[C.getBruteLoss() > 50 ? "\red" : "\blue"][C.getBruteLoss()]</span>", 1)
				user.show_message("<span style='color:#0000FF'>\t Key: Suffocation/Toxin/Burns/Brute</span>", 1)
				user.show_message("<span style='color:#0000FF'>\t Body Temperature: [C.bodytemperature-T0C]&deg;C ([C.bodytemperature*1.8-459.67]&deg;F)</span>", 1)
				if(C.tod && (C.stat == DEAD || (C.status_flags & FAKEDEATH)))
					user.show_message("<span style='color:#0000FF'>\t Time of Death: [C.tod]</span>")
				if(istype(C, /mob/living/carbon/human))
					var/mob/living/carbon/human/H = C
					var/list/damaged = H.get_damaged_limbs(1,1)
					user.show_message("<span style='color:#0000FF'>Localized Damage, Brute/Burn:</span>",1)
					if(length(damaged)>0)
						for(var/datum/limb/org in damaged)
							user.show_message(text("<span style='color:#0000FF'>\t []: []\blue-[]",capitalize(org.display_name),(org.brute_dam > 0)?"<span style='color:#FF0000'>[org.brute_dam]":0,(org.burn_dam > 0)?"\red [org.burn_dam]</span></span>":0),1)
					else
						user.show_message("<span style='color:#0000FF'>\t Limbs are OK.</span>",1)

				for(var/datum/disease/D in C.viruses)
					if(!D.hidden[SCANNER])
						user.show_message(text("<span style='color:#FF0000'><b>Warning: [D.form] Detected</b>\nName: [D.name].\nType: [D.spread].\nStage: [D.stage]/[D.max_stages].\nPossible Cure: [D.cure]</span>"))

			if(2)
				if (!istype(C:dna, /datum/dna))
					to_chat(user, "<span style='color:#0000FF'>No fingerprints found on [C]</span>")
				else if(!istype(C, /mob/living/carbon/monkey))
					if(!isnull(C:gloves))
						to_chat(user, "<span style='color:#0000FF'>No fingerprints found on [C]</span>")
				else
					to_chat(user, text("<span style='color:#0000FF'>[C]'s Fingerprints: [md5(C:dna.uni_identity)]</span>"))
				if (!C.blood_DNA || !C.blood_DNA.len)
					to_chat(user, "<span style='color:#0000FF'>No blood found on [C]</span>")
					if(C.blood_DNA)
						cdel(C.blood_DNA)
						C.blood_DNA = null
				else
					to_chat(user, "<span style='color:#0000FF'>Blood found on [C]. Analysing...</span>")
					spawn(15)
						for(var/blood in C.blood_DNA)
							to_chat(user, "<span style='color:#0000FF'>Blood type: [C.blood_DNA[blood]]\nDNA: [blood]</span>")

			if(4)
				for (var/mob/O in viewers(C, null))
					O.show_message("<span style='color:#FF0000'>[user] has analyzed [C]'s radiation levels!</span>", 1)

				user.show_message("<span style='color:#0000FF'>Analyzing Results for [C]:</span>")
				if(C.radiation)
					user.show_message("\green Radiation Level: \black [C.radiation]")
				else
					user.show_message("<span style='color:#0000FF'>No radiation detected.</span>")
...
/obj/item/device/pda/afterattack(atom/A as mob|obj|turf|area, mob/user as mob, proximity)
		...
		if(3)
			...
			if(!isnull(A.reagents))
				if(A.reagents.reagent_list.len > 0)
					...
					to_chat(user, "<span style='color:#0000FF'>[reagents_length] chemical agent[reagents_length > 1 ? "s" : ""] found.</span>")
					for (var/re in A.reagents.reagent_list)
						to_chat(user, "<span style='color:#0000FF'>\t [re]</span>")
				else
					to_chat(user, "<span style='color:#0000FF'>No active chemical agents found in [A].</span>")
			else
				to_chat(user, "<span style='color:#0000FF'>No significant chemical agents found in [A].</span>")
		...
		if(5)
			...
			if(istype(A, /obj/item/tank) || istype(A, /obj/machinery/portable_atmospherics) || istype(A, /obj/machinery/atmospherics/pipe/tank))
				...
				for (var/mob/O in viewers(user, null))
					to_chat(O, "<span style='color:#FF0000'>[user] has used [src] on \icon[icon] [A]</span>")
				var/pressure = A.return_pressure()
				var/temperature = A.return_temperature()
				var/gas = A.return_gas()

				to_chat(user, "<span style='color:#0000FF'>Results of analysis of \icon[icon]</span>")
				if (pressure>0)
					to_chat(user, "<span style='color:#0000FF'>Pressure: [round(pressure,0.1)] kPa</span>")
					to_chat(user, "<span style='color:#0000FF'>Gas Type: [gas]</span>")
					to_chat(user, "<span style='color:#0000FF'>Temperature: [round(temperature-T0C)]&deg;C</span>")
				else
					to_chat(user, "<span style='color:#0000FF'>Tank is empty!</span>")
	...
	if (!scanmode && istype(A, /obj/item/paper) && owner)
		...
		// Inform the user
		to_chat(user, "<span style='color:#0000FF'>Paper scanned and OCRed to notekeeper.</span>")
