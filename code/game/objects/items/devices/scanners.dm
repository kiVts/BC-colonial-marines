...
/obj/item/device/healthanalyzer/attack(mob/living/M, mob/living/user)
	...
	if(!istype(M, /mob/living/carbon) || (ishuman(M) && (M:species.flags & IS_SYNTHETIC)))
		...
		user.show_message("<span style='color:#0000FF'>Body Temperature: [M.bodytemperature-T0C]&deg;C ([M.bodytemperature*1.8-459.67]&deg;F)</span>", 1)
		user.show_message("<span style='color:#FF0000'><b>Warning: Blood Level ERROR: --% --cl.\blue Type: ERROR</span>")
		user.show_message("<span style='color:#0000FF'>Subject's pulse: <font color='red'>-- bpm.</font></span>")
		return
...
/obj/item/device/analyzer/attack_self(mob/user as mob)
	...
	user.show_message("<span style='color:#0000FF'><B>Results:</B></span>", 1)
	if(abs(env_pressure - ONE_ATMOSPHERE) < 10)
		user.show_message("<span style='color:#0000FF'>Pressure: [round(env_pressure,0.1)] kPa</span>", 1)
	else
		user.show_message("<span style='color:#FF0000'>Pressure: [round(env_pressure,0.1)] kPa</span>", 1)
	if(env_pressure > 0)
		user.show_message("<span style='color:#0000FF'>Gas Type: [env_gas]</span>", 1)
		user.show_message("<span style='color:#0000FF'>Temperature: [round(env_temp-T0C)]&deg;C</span>", 1)
...
/obj/item/device/mass_spectrometer/attack_self(mob/user as mob)
	if (user.stat)
		return
	if (crit_fail)
		to_chat(user, "<span style='color:#FF0000'>This device has critically failed and is no longer functional!</span>")
		return
	if (!(istype(user, /mob/living/carbon/human) || ticker) && ticker.mode.name != "monkey")
	...
	if(reagents.total_volume)
		...
		for(var/datum/reagent/R in reagents.reagent_list)
			if(R.id != "blood")
				...
				to_chat(user, "<span style='color:#FF0000'>The sample was contaminated! Please insert another sample</span>")
				return
			else
		...
		if(O.reagents.reagent_list.len > 0)
			...
			for (var/datum/reagent/R in O.reagents.reagent_list)
				if(prob(reliability))
					...
					recent_fail = 1
		if(dat)
			to_chat(user, "<span style='color:#0000FF'>Chemicals found: [dat]</span>")
		else
			to_chat(user, "<span style='color:#0000FF'>No active chemical agents found in [O].</span>")
	else
		to_chat(user, "<span style='color:#0000FF'>No significant chemical agents found in [O].</span>")
