...
/obj/item/device/megaphone/attack_self(mob/living/user)
	if (user.client)
		if(user.client.prefs.muted & MUTE_IC)
			...
			return
	if(!ishuman(user))
		to_chat(user, "<span style='color:#FF0000'>You don't know how to use this!</span>")
		return
	if(user.silent)
		return
	...
	if(spamcheck)
		to_chat(user, "<span style='color:#FF0000'>\The [src] needs to recharge!</span>")
		return
	...
	if ((src.loc == user && usr.stat == 0))
		if(emagged)
			if(insults)
				...
				to_chat(user, "<span style='color:#FF0000'>*BZZZZzzzzzt*</span>")
		else
...
/obj/item/device/megaphone/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/card/emag) && !emagged)
		to_chat(user, "<span style='color:#FF0000'>You overload \the [src]'s voice synthesizer.</span>")
		emagged = 1
