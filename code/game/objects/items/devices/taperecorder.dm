...
/obj/item/device/taperecorder/verb/record()
	...
	if(usr.stat)
		return
	if(emagged == 1)
		to_chat(usr, "<span style='color:#FF0000'>[src] makes a scratchy noise.</span>")
		return
...
/obj/item/device/taperecorder/attack_self(mob/user)
	if(recording == 0 && playing == 0)
		...
		icon_state = "taperecorderrecording"
		if(timerecorded < 3600 && playing == 0)
			to_chat(usr, "<span style='color:#0000FF'>Recording started.</span>")
			recording = 1
			timestamp+= timerecorded
			storedinfo += "\[[time2text(timerecorded*10,"mm:ss")]\] Recording started."
			for(timerecorded, timerecorded<3600)
			...
			to_chat(usr, "<span style='color:#FF0000'>Either [src]'s memory is full, or it is currently playing back its memory.</span>")
	else
		if(usr.stat)
		...
		if(recording == 1)
			...
			timestamp+= timerecorded
			storedinfo += "\[[time2text(timerecorded*10,"mm:ss")]\] Recording stopped."
			to_chat(usr, "<span style='color:#0000FF'>Recording stopped.</span>")
			icon_state = "taperecorderidle"
			return
		else if(playing == 1)
			playing = 0
			...
			to_chat(usr, "<span style='color:#FF0000'>Stop what?</span>")
			return
