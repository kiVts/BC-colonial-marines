...
/obj/item/dnainjector/attack(mob/M as mob, mob/user as mob)
	...
	//				message_admins("[key_name_admin(user)] injected [key_name_admin(M)] with the [name]")
				...
				user.visible_message("<span style='color:#FF0000'><B>[user] is trying to inject [M] with [src]!</B></span>")
				if(do_mob(user, M, HUMAN_STRIP_DELAY, BUSY_ICON_GENERIC, BUSY_ICON_GENERIC))
					if(src == user.get_active_hand())
						add_fingerprint(user)
						inject(M, user)
						user.visible_message("<span style='color:#FF0000'>[user] injects [M] with the DNA Injector!</span>")


		else

			for(var/mob/O in viewers(M, null))
				O.show_message(text("<span style='color:#FF0000'>[] has been injected with [] by [].</span>", M, src, user), 1)
				//Foreach goto(192)
			if (!(istype(M, /mob/living/carbon/human) || istype(M, /mob/living/carbon/monkey)))
				to_chat(user, "<span style='color:#FF0000'>Apparently it didn't work.</span>")
				return
			...
			*/
			if(user)//If the user still exists. Their mob may not.
				if(M)//Runtime fix: If the mob doesn't exist, mob.name doesnt work. - Nodrak
					user.show_message(text("<span style='color:#FF0000'>You inject [M.name]</span>"))
				else
					user.show_message(text("<span style='color:#FF0000'>You finish the injection.</span>"))
