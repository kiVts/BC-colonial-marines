...
/obj/item/stack/rods/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/stack/barbed_wire)) //making razorwire obstacles
		...
		use(4)
		B.use(1)
		var/obj/structure/barricade/razor/M = new/obj/item/stack/razorwire(user.loc, 1)
	...
	if (istype(W, /obj/item/tool/weldingtool))
		...
		if(amount < 4)
			to_chat(user, "<span style='color:#FF0000'>You need at least four rods to do this.</span>")
			return
		...
		if(WT.remove_fuel(0,user))
			...
			for (var/mob/M in viewers(src))
				M.show_message("<span style='color:#FF0000'>[src] is shaped into metal by [user.name] with the weldingtool.", 3, "\red You hear welding.</span>", 2)
...
/obj/item/stack/rods/attack_self(mob/user as mob)
	...
	if (locate(/obj/structure/grille, usr.loc))
		...
		if(amount < 4)
			to_chat(user, "<span style='color:#0000FF'>You need at least four rods to do this.</span>")
			return
		to_chat(usr, "<span style='color:#0000FF'>Assembling grille...</span>")
		in_use = 1
		if (!do_after(usr, 20, TRUE, 5, BUSY_ICON_BUILD))
			in_use = 0
			return
		var/obj/structure/grille/F = new /obj/structure/grille/ ( usr.loc )
		to_chat(usr, "<span style='color:#0000FF'>You assemble a grille</span>")
		in_use = 0
