...
/obj/item/stack/razorwire/attack_self(mob/user) //use barbed wire to deploy it
	...
	if(do_after(user, delay_assembly, TRUE, 5, BUSY_ICON_BUILD))
		if(!src) //Make sure the stack still exists
			return
		var/obj/structure/barricade/razor/M = new /obj/structure/barricade/razor(target)
		M.dir = user.dir
