	...
	switch(input(title, "What would you like to construct?") as null|anything in construction_options)
		if("One Direction")
			...
			for (var/obj/structure/window/win in user.loc)
				i++
				if(i >= 4)
					to_chat(user, "<span style='color:#FF0000'>There are too many windows in this location.</span>")
					return 1
				directions-=win.dir
				if(!(win.dir in cardinal))
					to_chat(user, "<span style='color:#FF0000'>Can't let you do that.</span>")
					return 1
		...
		if("Full Window")
			...
			if(src.loc != user)	return 1
			if(src.amount < 4)
				to_chat(user, "<span style='color:#FF0000'>You need more glass to do that.</span>")
				return 1
			if(locate(/obj/structure/window) in user.loc)
				to_chat(user, "<span style='color:#FF0000'>There is a window in the way.</span>")
				return 1
		...
		if("Windoor")
			...
			if(isturf(user.loc) && locate(/obj/structure/windoor_assembly/, user.loc))
				to_chat(user, "<span style='color:#FF0000'>There is already a windoor assembly in that location.</span>")
				return 1

			if(isturf(user.loc) && locate(/obj/machinery/door/window/, user.loc))
				to_chat(user, "<span style='color:#FF0000'>There is already a windoor in that location.</span>")
				return 1
