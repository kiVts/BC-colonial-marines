...
/obj/item/stack/sheet/animalhide/attackby(obj/item/W, mob/user)
	if(W.sharp)
		...
		user.visible_message("<span style='color:#0000FF'>\the [usr] starts cutting hair off \the [src]", "\blue You start cutting the hair off \the [src]", "You hear the sound of a knife rubbing against flesh</span>")
		if(do_after(user,50, TRUE, 5, BUSY_ICON_HOSTILE))
			to_chat(user, "<span style='color:#0000FF'>You cut the hair from this [src.singular_name]</span>")
