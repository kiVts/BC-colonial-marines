...
/obj/item/handcuffs/attack(mob/living/carbon/C, mob/user)
	...
	if ((CLUMSY in usr.mutations) && prob(50))
		to_chat(user, "<span style='color:#FF0000'>Uh ... how do those things work?!</span>")
...
/obj/item/handcuffs/proc/place_handcuffs(var/mob/living/carbon/target, var/mob/user)
	...
	if (ishuman(target))
		...
		if (!H.has_limb_for_slot(WEAR_HANDCUFFS))
			to_chat(user, "<span style='color:#FF0000'>\The [H] needs at least two wrists before you can cuff them together!</span>")
			return
...
/obj/item/handcuffs/cyborg/attack(mob/living/carbon/C as mob, mob/user as mob)
	if(!C.handcuffed)
		...
		playsound(src.loc, cuff_sound, 25, 1, 4)
		user.visible_message("<span style='color:#FF0000'><B>[user] is trying to put handcuffs on [C]!</B></span>")
...
/obj/item/restraints/attack(mob/living/carbon/C as mob, mob/user as mob)
	if(!istype(C, /mob/living/carbon/Xenomorph))
		to_chat(user, "<span style='color:#FF0000'>The cuffs do not fit!</span>")
		return
	if(!C.handcuffed)
		var/turf/p_loc = user.loc
		var/turf/p_loc_m = C.loc
		...
		for(var/mob/O in viewers(user, null))
			O.show_message("<span style='color:#FF0000'><B>[user] is trying to put restraints on [C]!</B></span>", 1)
		spawn(30)
			if(!C)	return
			if(p_loc == user.loc && p_loc_m == C.loc)
				C.handcuffed = new /obj/item/restraints(C)
				C.handcuff_update()
				C.visible_message("<span style='color:#FF0000'>[C] has been successfully restrained by [user]!</span>")
				cdel(src)
	return
