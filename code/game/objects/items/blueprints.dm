...
/obj/item/blueprints/proc/create_area()
	...
	if(!istype(res,/list))
		switch(res)
			if(ROOM_ERR_SPACE)
				to_chat(usr, "<span style='color:#FF0000'>The new area must be completely airtight!</span>")
				return
			if(ROOM_ERR_TOOLARGE)
				to_chat(usr, "<span style='color:#FF0000'>The new area too large!</span>")
				return
			else
				to_chat(usr, "<span style='color:#FF0000'>Error! Please notify administration!</span>")
				return
	...
	if(!str || !length(str)) //cancel
		return
	if(length(str) > 50)
		to_chat(usr, "<span style='color:#FF0000'>Name too long.</span>")
		return
	var/area/A = new
	A.name = str
...
/obj/item/blueprints/proc/edit_area()
	...
	if(!str || !length(str) || str==prevname) //cancel
		return
	if(length(str) > 50)
		to_chat(usr, "<span style='color:#FF0000'>Text too long.</span>")
		return
	set_area_machinery_title(A,str,prevname)
	for(var/area/RA in A.related)
		RA.name = str
	to_chat(usr, "<span style='color:#0000FF'>You set the area '[prevname]' title to '[str]'.</span>")
	interact()
	return
...
/obj/item/blueprints/proc/set_area_machinery_title(var/area/A,var/title,var/oldtitle)
	if (!oldtitle) // or oldreplacetext goes to infinite loop
		return
	for(var/area/RA in A.related)
