/obj/item/weapon/claymore
	...
	suicide_act(mob/user)
		viewers(user) << "<span style='color:#FF0000'><b>[user] is falling on the [src.name]! It looks like \he's trying to commit suicide.</b></span>"
		return(BRUTELOSS)
...
/obj/item/weapon/katana
	...
	suicide_act(mob/user)
		viewers(user) << "<span style='color:#FF0000'><b>[user] is slitting \his stomach open with the [src.name]! It looks like \he's trying to commit seppuku.</b></span>"
		return(BRUTELOSS)

/obj/item/weapon/katana/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
...
/obj/item/weapon/katana/replica/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 79225)
	else
		serial_number = given_serial
...
/obj/item/weapon/combat_knife/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 879225)
	else
		serial_number = given_serial
	desc += " Серийный номер [serial_number]"
	..(loc)
...
/obj/item/weapon/combat_knife/upp/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 879225)
	else
		serial_number = given_serial
...
/obj/item/weapon/throwing_knife/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
		serial_number = rand(1, 879225)
	else
		serial_number = given_serial
