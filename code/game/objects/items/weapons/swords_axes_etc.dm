...
/obj/item/weapon/classic_baton/attack(mob/M as mob, mob/living/user as mob)
	if ((CLUMSY in user.mutations) && prob(50))
		to_chat(user, "<span style='color:#FF0000'>You club yourself over the head.</span>")
...
*/
	...
	for(var/mob/O in viewers(M))
		if (O.client)	O.show_message("<span style='color:#FF0000'><B>[M] has been beaten with \the [src] by [user]!</B>", 1, "\red You hear someone fall</span>", 2)
...
/obj/item/weapon/telebaton/attack_self(mob/user as mob)
	on = !on
	if(on)
		user.visible_message("<span style='color:#FF0000'>With a flick of their wrist, [user] extends their telescopic baton.</span>",\
		"<span style='color:#FF0000'>You extend the baton.</span>",\
		...
		user.visible_message("<span style='color:#0000FF'>[user] collapses their telescopic baton.</span>",\
		"<span style='color:#0000FF'>You collapse the baton.</span>",\
		"You hear a click.")
		icon_state = "telebaton_0"
		item_state = "telebaton_0"
		w_class = 2
...
/obj/item/weapon/shield/energy/attack_self(mob/living/user as mob)
	if ((CLUMSY in user.mutations) && prob(50))
		to_chat(user, "<span style='color:#FF0000'>You beat yourself in the head with [src].</span>")
