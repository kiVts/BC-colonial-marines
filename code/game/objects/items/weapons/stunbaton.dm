...
/obj/item/roller/medevac/New(turf/loc, var/given_serial = 0)
	if(given_serial == 0)
...
/obj/item/weapon/baton/proc/check_user_auth(mob/user)
	...
	if(istype(H))
		...
		if(!istype(I) || !check_access(I))
			H.visible_message("<span style='color:#0000FF'>[src] beeeps as [H] picks it up", "<span class='danger'>WARNING: Unauthorized user detected. Denying access...</span></span>")
			H.KnockDown(20)
...
/obj/item/weapon/baton/attack(mob/M, mob/user)
	...
	if(user.a_intent == "hurt")
		...
		if (ishuman(L))
			...
			if(!target_zone)
				L.visible_message("<span style='color:#FF0000'><B>[user] misses [L] with \the [src]!</span>")
				return 0
