		...
		if("c-4detonator_1")
			if(!pr_open)
				...
				switch(alert(user, "What would you like to do?", "Lighter", "Press the button.", "Close the lighter."))
					if("Press the button.")
						to_chat(user, "<span style='color:#FF0000'>You press the button.</span>")
						flick("c-4detonator_click", src)
						if(src.bomb)
							src.bomb.detonate()
							log_admin("[user.real_name]([user.ckey]) has triggered [src.bomb] with [src].")
							message_admins("<span style='color:#FF0000'>[user.real_name]([user.ckey]) has triggered [src.bomb] with [src].</span>")
