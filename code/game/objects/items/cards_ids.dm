	...
	if(istype(O, /obj/item/card/id))
		...
		if(istype(user, /mob/living) && user.mind)
			if(user.mind.special_role)
				to_chat(usr, "<span style='color:#0000FF'>The card's microscanners activate as you pass it over the ID, copying its access.</span>")
...
/obj/item/card/id/syndicate/attack_self(mob/user as mob)
	if(!src.registered_name)
		...
		to_chat(user, "<span style='color:#0000FF'>You successfully forge the ID card.</span>")
