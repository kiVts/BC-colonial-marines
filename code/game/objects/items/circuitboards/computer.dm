...
/obj/item/circuitboard/computer/security/attackby(obj/item/I as obj, mob/user as mob)
	if(istype(I,/obj/item/card/emag))
		if(emagged)
			...
			return
		to_chat(user, "<span style='color:#0000FF'>You override the circuit lock and open controls.</span>")
		emagged = 1
		locked = 0
	else if(istype(I,/obj/item/card/id))
		if(emagged)
			to_chat(user, "<span style='color:#FF0000'>Circuit lock does not respond.</span>")
			return
		if(check_access(I))
			locked = !locked
			to_chat(user, "<span style='color:#0000FF'>You [locked ? "" : "un"]lock the circuit controls.</span>")
		else
			to_chat(user, "<span style='color:#FF0000'>Access denied.</span>")
	else if(istype(I,/obj/item/device/multitool))
		if(locked)
			to_chat(user, "<span style='color:#FF0000'>Circuit controls are locked.</span>")
			return
...
/obj/item/circuitboard/computer/rdconsole/attackby(obj/item/I as obj, mob/user as mob)
	if(istype(I,/obj/item/tool/screwdriver))
		user.visible_message("<span style='color:#0000FF'>\the [user] adjusts the jumper on the [src]'s access protocol pins.", "\blue You adjust the jumper on the access protocol pins.</span>")
		if(src.build_path == /obj/machinery/computer/rdconsole/core)
			src.name = "Circuit Board (RD Console - Robotics)"
			src.build_path = /obj/machinery/computer/rdconsole/robotics
			to_chat(user, "<span style='color:#0000FF'>Access protocols set to robotics.</span>")
		else
			src.name = "Circuit Board (RD Console)"
			src.build_path = /obj/machinery/computer/rdconsole/core
			to_chat(user, "<span style='color:#0000FF'>Access protocols set to default.</span>")
