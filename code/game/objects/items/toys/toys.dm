...
/obj/item/toy/balloon/afterattack(atom/A as mob|obj, mob/user as mob, proximity)
	...
	if (istype(A, /obj/structure/reagent_dispensers/watertank) && get_dist(src,A) <= 1)
		...
		to_chat(user, "<span style='color:#0000FF'>You fill the balloon with the contents of [A].</span>")
		...
		if(O.reagents)
			if(O.reagents.total_volume < 1)
				...
				if(O.reagents.has_reagent("pacid", 1))
					...
					src.desc = "A translucent balloon with some form of liquid sloshing around in it."
					to_chat(user, "<span style='color:#0000FF'>You fill the balloon with the contents of [O].</span>")
...
/obj/item/toy/balloon/throw_impact(atom/hit_atom)
	if(src.reagents.total_volume >= 1)
		src.visible_message("<span style='color:#FF0000'>The [src] bursts!","You hear a pop and a splash.</span>")
...
/obj/item/toy/crayon
	...
	suicide_act(mob/user)
		viewers(user) << "<span style='color:#FF0000'><b>[user] is jamming the [src.name] up \his nose and into \his brain. It looks like \he's trying to commit suicide.</b></span>"
...
/obj/item/toy/snappop
	...
	throw_impact(atom/hit_atom)
		...
		src.visible_message("<span style='color:#FF0000'>The [src.name] explodes!","\red You hear a snap!</span>")
...
/obj/item/toy/snappop/Crossed(H as mob|obj)
	if((ishuman(H))) //i guess carp and shit shouldn't set them off
		...
		if(M.m_intent == MOVE_INTENT_RUN)
			to_chat(M, "<span style='color:#FF0000'>You step on the snap pop!</span>")
...
/obj/item/toy/waterflower/afterattack(atom/A as mob|obj, mob/user as mob)
	...
	if (istype(A, /obj/item/storage/backpack ))
		...
		A.reagents.trans_to(src, 10)
		to_chat(user, "<span style='color:#0000FF'>You refill your flower!</span>")
		return

	else if (src.reagents.total_volume < 1)
		src.empty = 1
		to_chat(user, "<span style='color:#0000FF'>Your flower has run dry!</span>")
		return
		...
		spawn(0)
			for(var/i=0, i<1, i++)
				...
				for(var/atom/T in get_turf(D))
					...
					if(ismob(T) && T:client)
						to_chat(T:client, "<span style='color:#FF0000'>[user] has sprayed you with water!</span>")
				sleep(4)
			cdel(D)
