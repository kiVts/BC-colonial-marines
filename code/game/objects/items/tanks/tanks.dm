...
/obj/item/tank/examine(mob/user)
	...
	if (in_range(src, user))
		...
		to_chat(user, "<span style='color:#0000FF'>\The \icon[src][src] feels [descriptive]</span>")
...
/obj/item/tank/attackby(obj/item/W as obj, mob/user as mob)
	...
	if ((istype(W, /obj/item/device/analyzer)) && get_dist(user, src) <= 1)
		for (var/mob/O in viewers(user, null))
			to_chat(O, "<span style='color:#FF0000'>[user] has used [W] on \icon[src] [src]</span>")

		manipulated_by = user.real_name			//This person is aware of the contents of the tank.

		to_chat(user, "<span style='color:#0000FF'>Results of analysis of \icon[src]</span>")
		if (pressure>0)
	...
	if (href_list["stat"])
		if(istype(loc,/mob/living/carbon))
			...
			if(location.internal == src)
				...
				to_chat(usr, "<span style='color:#0000FF'>You close the tank release valve.</span>")
				...
				if(location.wear_mask && (location.wear_mask.flags_inventory & ALLOWINTERNALS))
					...
					to_chat(usr, "<span style='color:#0000FF'>You open \the [src] valve.</span>")
					if (location.hud_used && location.hud_used.internals)
						location.hud_used.internals.icon_state = "internal1"
				else
					to_chat(usr, "<span style='color:#0000FF'>You need something to connect to \the [src].</span>")
