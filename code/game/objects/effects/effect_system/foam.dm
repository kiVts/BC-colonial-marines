...
/obj/structure/foamedmetal/attack_hand(var/mob/user)
	if ((HULK in user.mutations) || (prob(75 - metal*25)))
		to_chat(user, "<span style='color:#0000FF'>You smash through the metal foam wall.</span>")
		for(var/mob/O in oviewers(user))
			if ((O.client && !( O.blinded )))
				to_chat(O, "<span style='color:#FF0000'>[user] smashes through the foamed metal.</span>")

		cdel(src)
	else
		to_chat(user, "<span style='color:#0000FF'>You hit the metal foam but bounce off it.</span>")
	return

/obj/structure/foamedmetal/attackby(var/obj/item/I, var/mob/user)

	if(prob(I.force*20 - metal*25))
		to_chat(user, "<span style='color:#0000FF'>You smash through the foamed metal with \the [I].</span>")
		for(var/mob/O in oviewers(user))
			if ((O.client && !( O.blinded )))
				to_chat(O, "<span style='color:#FF0000'>[user] smashes through the foamed metal.</span>")
		cdel(src)
	else
		to_chat(user, "<span style='color:#0000FF'>You hit the metal foam to no effect.</span>")
