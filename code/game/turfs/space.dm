...
/turf/open/space/attackby(obj/item/C, mob/user)
	...
	if (istype(C, /obj/item/stack/rods))
		...
		if (R.use(1))
			to_chat(user, "<span style='color:#0000FF'>Constructing support lattice ...</span>")
	...
	if (istype(C, /obj/item/stack/tile/plasteel))
		...
		if(L)
			...
			to_chat(user, "<span style='color:#FF0000'>The plating is going to need some support.</span>")
	return
			...
			if(!isemptylist(disk_search))
				if(istype(A, /mob/living))
					...
					if(MM.client && !MM.stat)
						to_chat(MM, "<span style='color:#FF0000'>Something you are carrying is preventing you from leaving. Don't play stupid; you know exactly what it is.</span>")
