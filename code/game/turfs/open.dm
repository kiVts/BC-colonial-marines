...
/turf/open/desertdam/river/is_weedable()
	return FALSE

/turf/open/desertdam/river/Entered(atom/movable/AM)
	..()
	if(iscarbon(AM))
		var/mob/living/carbon/C = AM
		var/river_slowdown = 1.75
...
/turf/open/desertdam/river/proc/cleanup(var/mob/living/carbon/human/M)
	if(!M || !istype(M)) return
...
/turf/open/desertdam/river/toxic/New()
	..()

/turf/open/desertdam/river/toxic/Entered(mob/living/M)
	..()
	if(istype(M)) M.apply_damage(15,TOX)
...
/turf/open/jungle/proc/Spread(var/probability, var/prob_loss = 50)
	...
	//to_chat(world, "<span style='color:#0000FF'>Spread([probability])</span>")
...
/turf/open/jungle/water/Entered(atom/movable/O)
	...
	if(istype(O, /mob/living/))
		...
		if(prob(10 + (M.m_intent == MOVE_INTENT_RUN ? 40 : 0)))
			to_chat(M, pick("<span style='color:#0000FF'>You slip on something slimy.","\blue You fall over into the murk.</span>"))
			M.Stun(2)
			M.KnockDown(1)

		//piranhas - 25% chance to be an omnipresent risk, although they do practically no damage
		if(prob(25))
			to_chat(M, "<span style='color:#0000FF'>You feel something slithering around your legs.</span>")
			if(prob(50))
				spawn(rand(25,50))
					var/turf/T = get_turf(M)
					if(istype(T, /turf/open/jungle/water))
						to_chat(M, pick("<span style='color:#FF0000'>Something sharp bites you!","\red Sharp teeth grab hold of you!","\red You feel something take a chunk out of your leg!</span>"))
