...
/turf/closed/wall/r_wall/attack_hand(mob/user)
	if (HULK in user.mutations)
		if (prob(10))
			to_chat(usr, text("<span style='color:#0000FF'>You smash through the wall.</span>"))
			usr.say(pick(";RAAAAAAAARGH!", ";HNNNNNNNNNGGGGGGH!", ";GWAAAAAAAARRRHHH!", "NNNNNNNNGGGGGGGGHH!", ";AAAAAAARRRGH!" ))
			dismantle_wall(1)
			return
		else
			to_chat(user, "<span style='color:#0000FF'>You punch the wall.</span>")
			return
...
/turf/closed/wall/r_wall/deadwood
	name = "facility wall"
	icon = 'icons/turf/deadwood.dmi'
	icon_state = "pend0"
	walltype = "pend"

/turf/closed/wall/r_wall/deadwood/handle_icon_junction(junction)
	if (!walltype)
		return
	...
	if(junction == 12)
		switch(r1)
			...
			if(9 to 10)
				icon_state = "pend_variant0"
	else
		icon_state = "[walltype][junction]"



/turf/closed/wall/r_wall/locers
	name = "Dam"
	icon = 'icons/turf/deadwood.dmi'
	icon_state = "pends0"
	walltype = "pends"

/turf/closed/wall/r_wall/locers/handle_icon_junction(junction)
	if (!walltype)
		return
	//lets make some detailed randomized shit happen.
	var/r1 = rand(0,10) //Make a random chance for this to happen
