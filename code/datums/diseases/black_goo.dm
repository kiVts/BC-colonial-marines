	...
	switch(stage)
		if(1)
			...
			if(goo_message_cooldown < world.time )
				if(prob(3))
					to_chat(affected_mob, "<span style='color:#FF0000'>You feel really warm...</span>")
					goo_message_cooldown = world.time + 100
		if(2)
			if(goo_message_cooldown < world.time)
				if (prob(3)) to_chat(affected_mob, "<span style='color:#FF0000'>Your throat is really dry...</span>")
				else if (prob(6)) to_chat(affected_mob, "<span style='color:#FF0000'>You feel really warm...</span>")
		...
		if(3)
			...
			if(goo_message_cooldown < world.time)
				if (prob(3))
					to_chat(affected_mob, "<span style='color:#FF0000'>You cough up some black fluid...</span>")
					goo_message_cooldown = world.time + 100
				else if (prob(6))
					to_chat(affected_mob, "<span style='color:#FF0000'>Your throat is really dry...</span>")
					goo_message_cooldown = world.time + 100
				else if (prob(9))
					to_chat(affected_mob, "<span style='color:#FF0000'>You feel really warm...</span>")
					goo_message_cooldown = world.time + 100
