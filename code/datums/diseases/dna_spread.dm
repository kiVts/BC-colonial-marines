...
/datum/disease/dnaspread/stage_act()
	...
	switch(stage)
		if(2 || 3) //Pretend to be a cold and give time to spread.
			...
			if(prob(1))
				to_chat(affected_mob, "<span style='color:#FF0000'>Your muscles ache.</span>")
				if(prob(20))
					affected_mob.take_limb_damage(1)
			if(prob(1))
				to_chat(affected_mob, "<span style='color:#FF0000'>Your stomach hurts.</span>")
				if(prob(20))
		...
		if(4)
			if(!src.transformed)
				...
				to_chat(affected_mob, "<span style='color:#FF0000'>You don't feel like yourself..</span>")
...
/datum/disease/dnaspread/Dispose()
	if ((original_dna["name"]) && (original_dna["UI"]) && (original_dna["SE"]))
		...
		to_chat(affected_mob, "<span style='color:#0000FF'>You feel more like yourself.</span>")
	. = ..()