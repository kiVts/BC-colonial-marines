...
client
	proc/debug_variables(datum/D in world)
		...
		if(!usr.client || !usr.client.holder || !(usr.client.holder.rights & R_MOD))
			to_chat(usr, "<span style='color:#FF0000'>You need to be a moderator or higher to access this.</span>")
			return
	...
	//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
		...
		switch(action_type)
			if("Strict type")
				...
				message_admins("<span style='color:#0000FF'>[key_name(usr)] deleted all objects of type [O_type] ([i] objects deleted) </span>")
			if("Type and subtypes")
				...
				message_admins("<span style='color:#0000FF'>[key_name(usr)] deleted all objects of type or subtype of [O_type] ([i] objects deleted) </span>")
		...
		if(amount != 0)
			...
			message_admins("<span style='color:#0000FF'>[key_name(usr)] dealt [amount] amount of [Text] damage to [L] </span>")
