...
/datum/game_mode/proc/initialize_emergency_calls()
	...
	if(!total_calls.len)
		to_chat(world, "<span style='color:#FF0000'>\b Error setting up emergency calls, no datums found.</span>")
		return 0
...
/datum/game_mode/proc/get_random_call()
	...
	if(!istype(chosen_call))
		to_chat(world, "<span style='color:#FF0000'>Something went wrong with emergency calls. Tell a coder!</span>")
		return null
	else
		return chosen_call
