	...
	if(mind)
		switch(mind.special_role)
			...
			if("mutineer")
				holder.icon_state = "hudmutineer"
			if("Commander")
				holder.icon_state = "fieldcommander"
			if("Executive Officer")
				holder.icon_state = "executiveofficer"
			if("Staff Officer")
				holder.icon_state = "staffofficer"
			if("Vehicle Crewman")
				holder.icon_state = "tankcrew"
			if("Corporate Liaison")
				holder.icon_state = "corporateliaison"
			if("Chief Engineer")
				holder.icon_state = "chiefengineer"
			if("Maintenance Tech")
				holder.icon_state = "maintenancetech"
			if("Requisitions Officer")
				holder.icon_state = "requisitionsoffice"
			if("Cargo Technician")
				holder.icon_state = "cargotechnician"
			if("Chief Medical Officer")
				holder.icon_state = "chiefmedicalofficer"
			if("Doctor")
				holder.icon_state = "doctor"
			if("Researcher")
				holder.icon_state = "researcher"
			if("Military Police")
				holder.icon_state = "warden"
			if("Chief MP")
				holder.icon_state = "chiefmp"
...
/mob/living/carbon/human/proc/hud_set_order()
	...
	if(command_aura)
		...
		switch(command_aura)
			if("move")
				holder.overlays += image('icons/mob/hud.dmi', src, "hudmove")
			if("hold")
				holder.overlays += image('icons/mob/hud.dmi', src, "hudhold")
			if("focus")
				holder.overlays += image('icons/mob/hud.dmi', src, "hudfocus")

/*		switch(leader_command_aura)
			if("move")
