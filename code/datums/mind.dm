			...
			switch(href_list["traitor"])
				if("clear")
					if(src in ticker.mode.traitors)
						...
						to_chat(current, "<span style='color:#FF0000'><FONT size = 3><B>You have been brainwashed! You are no longer a traitor!</B></FONT></span>")
				...
				if("autoobjectives")
					if (!config.objectives_disabled)
						...
						to_chat(usr, "<span style='color:#0000FF'>The objectives for traitor [key] have been generated. You can edit them and anounce manually.</span>")
			...
			switch(href_list["common"])
				...
				if("uplink")
					if (!ticker.mode.equip_traitor(current, !(src in ticker.mode.traitors)))
						to_chat(usr, "<span style='color:#FF0000'>Equipping a syndicate failed!</span>")

		else if (href_list["obj_announce"])
			var/obj_count = 1
			to_chat(current, "<span style='color:#0000FF'>Your current objectives:</span>")
			for(var/datum/objective/objective in objectives)
