...
/datum/teleport/instant/science
	...
	teleportChecks()
		if(istype(teleatom, /obj/item/disk/nuclear)) // Don't let nuke disks get teleported --NeoFite
			teleatom.visible_message("<span style='color:#FF0000'><B>The [teleatom] bounces off of the portal!</B></span>")
			return 0

		if(!isemptylist(teleatom.search_contents_for(/obj/item/disk/nuclear)))
			if(istype(teleatom, /mob/living))
				var/mob/living/MM = teleatom
				MM.visible_message("<span style='color:#FF0000'><B>The [MM] bounces off of the portal!</B>","\red Something you are carrying seems to be unable to pass through the portal. Better drop it if you want to go through.</span>")
			else
				teleatom.visible_message("<span style='color:#FF0000'><B>The [teleatom] bounces off of the portal!</B></span>")
			return 0
		...
		if(destination.z == 2) //centcomm z-level
			if(istype(teleatom, /obj/mecha))
				...
				to_chat(MM.occupant, "<span style='color:#FF0000'><B>The mech would not survive the jump to a location so far away!</B></span>")
				return 0
			if(!isemptylist(teleatom.search_contents_for(/obj/item/storage/backpack/holding)))
				teleatom.visible_message("<span style='color:#FF0000'><B>The Bag of Holding bounces off of the portal!</B></span>")
				return 0
