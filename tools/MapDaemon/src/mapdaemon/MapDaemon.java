...
public class MapDaemon {
    ...
    private static String default_config_dir = System.getProperty("user.home") + "/config.json";
    private static String default_log_dir = System.getProperty("user.home") + "/logs";
    ...
    public MapDaemon(String config_dir, boolean verbose) {
        ...
        if (!config.getBooleanFromConfig("prefs.enable")) {
            return; //Terminate if we are set to
        }
        this.verbose = verbose;
        String log_dir = config.getStringFromConfig("paths.crash_logs");
        if (log_dir.indexOf("ERROR") >= 0) {
        ...
        tick_delay = Float.parseFloat(config.getStringFromConfig("prefs.tick_delay")) * 1000;
    ...
    private void crashedEndProcess(String info) {
        ...
        if (!logger.close(true)) {
            ...
            System.out.println("Successfully wrote logs! Check relevant directory for crash logs!");
        }
        running = false;
        try {
            Thread.sleep(10000);
    ...
    private int processCommandStringAttempt(ByondResponse obj, int fails, String info_text) {
        ...
        String resp = (String) obj.getResponse();
    ...
    private boolean resumeRound(int fails) {
        ...
        String s = (String) obj.getResponse();
    ...
    private boolean getNextMap(int fails) {
        ...
        if (flag == PROCESS_COMMAND_SUCCESS) {
            ...
            next_map = (String) obj.getResponse();
    ...
    private boolean verifyRoundEnded(int fails) {
        ...
        int round_status = ((Float) obj.getResponse()).intValue();

        if (round_status == ROUND_STATUS_OVER) {
            return true;
 ...
 /*
    ...
    private boolean Compile() {
        ...
        String dme_path = project_base + File.separator + project_name + ".dme";
 ...
 /*
    ...
    private String compileDME(String dme_path) {
        logger.addLine("Executing DreamMaker:");

        String command = "/usr/local/bin/DreamMaker \"" + dme_path + "\"";
        if (OSCheck.getOperatingSystemType().equals(OSCheck.OSType.Windows)) {
            command = "\"" + byond_base + "\\bin\\dm.exe\" \"" + dme_path + "\"";
        }
        Process p = null;
        try {
            p = Runtime.getRuntime().exec(command);
        } catch (IOException e) {
            crashedEndProcess("Failed to execute DreamMaker, writing logs and terminating script");
            logger.addLine(returnStackTrace(e));
            return "ERROR";
        }
    ...
    private boolean verifyCompileSuccess(String data) {
        ...
        if (data.equals("")) {
            return true; // Debug stuff
        }
        String[] lines = data.split("\n");
        String last_line = lines[lines.length - 2];
    ...
    private boolean copyBinaries(String base_path, String targ_path) {
        ...
        for (int i = 0; i < binary_files.length; i++) {
            String base_file = base_path + File.separator + project_name + binary_files[i];
            String targ_file = targ_path + File.separator + project_name + binary_files[i];

            if (base_file.equals(targ_file)) {
                logger.addLine("Won't copy " + base_file + " is the same file.");
                continue;
            }
    ...
    private void CMDCopyFile(String source, String destination) throws Exception {
        String command = "cp -a \"" + source + "\" \"" + destination + "\"";
        if (OSCheck.getOperatingSystemType().equals(OSCheck.OSType.Windows)) {
            command = "cmd.exe /C copy \"" + source + "\" \"" + destination + "\" /Y";
        }
        Process p = null;
        try {
            p = Runtime.getRuntime().exec(command);
        ...
        // TODO: apply that block for linux
        if (OSCheck.getOperatingSystemType().equals(OSCheck.OSType.Windows)) {
            String data = "";
            BufferedReader reader = new BufferedReader(new InputStreamReader(p.getInputStream()));
            try {
