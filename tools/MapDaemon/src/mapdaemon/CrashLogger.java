...
public class CrashLogger {
    ...
    public CrashLogger(String directory, boolean verbose) {
        ...
        file_name_format = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss");
        closed = false;
    }
    ...
    private boolean writeInfo() {
        ...
        String file = directory + File.separator + file_name_format.format(date) + ".log";
        if (!dir.isDirectory() && !dir.mkdirs()) {
            addLine(MapDaemon.returnStackTrace(new Exception("Unable to create log directory")));
        }
        try {
            Files.write(Paths.get(file), data, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);
        } catch (IOException e) {
