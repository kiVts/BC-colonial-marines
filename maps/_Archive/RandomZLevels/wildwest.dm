		...
		switch(wish)
			if("Power")
				...
				if (!(LASER in user.mutations))
					...
					user << "<span style='color:#0000FF'>You feel pressure building behind your eyes.</span>"
				if (!(COLD_RESISTANCE in user.mutations))
					user.mutations.Add(COLD_RESISTANCE)
					user << "<span style='color:#0000FF'>Your body feels warm.</span>"
				if (!(XRAY in user.mutations))
					user.mutations.Add(XRAY)
					user.sight |= (SEE_MOBS|SEE_OBJS|SEE_TURFS)
					user.see_in_dark = 8
					user.see_invisible = SEE_INVISIBLE_LEVEL_TWO
					user << "<span style='color:#0000FF'>The walls suddenly disappear.</span>"
